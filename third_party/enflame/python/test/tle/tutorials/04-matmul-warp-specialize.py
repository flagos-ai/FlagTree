...
def get_autotune_config():
    ...
    for max_grid in [24, 48]:
        configs.append(triton.Config({'BLOCK_SIZE_M': 256, 'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_K': 256, 'MAX_GRID_DIM': max_grid}, num_stages=2, num_warps=4))
        configs.append(triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 256, 'MAX_GRID_DIM': max_grid}, num_stages=2, num_warps=4))
        configs.append(triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_K': 256, 'MAX_GRID_DIM': max_grid}, num_stages=2, num_warps=4))
        configs.append(triton.Config({'BLOCK_SIZE_M': 256, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 256, 'MAX_GRID_DIM': max_grid}, num_stages=2, num_warps=4))
    configs.append(triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 64, 'BLOCK_SIZE_K': 256, 'MAX_GRID_DIM': 24}, num_stages=2, num_warps=2))
    configs.append(triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 64, 'BLOCK_SIZE_K': 256, 'MAX_GRID_DIM': 48}, num_stages=2, num_warps=2))
    return configs
    ...
    tle.gpu.warp_specialize(
        [
            (_tle_consumer_default, (
                c_ptr, reader, M, N, K, stride_cm, stride_cn,
                tl.constexpr(BLOCK_SIZE_M), tl.constexpr(BLOCK_SIZE_N), tl.constexpr(BLOCK_SIZE_K),
                tl.constexpr(MAX_GRID_DIM), tl.constexpr(USE_LEAKY_RELU),
            )),
            (_tle_producer_worker, (
                a_ptr, b_ptr, writer, M, N, K, stride_am, stride_ak, stride_bk, stride_bn,
                tl.constexpr(BLOCK_SIZE_M), tl.constexpr(BLOCK_SIZE_N), tl.constexpr(BLOCK_SIZE_K),
...
def matmul_ws_tle_producer_default_kernel(
    ...
    a_buf = tle.gpu.alloc(
        [NUM_STAGES, BLOCK_SIZE_M, BLOCK_SIZE_K], dtype=tl.float16,  # type: ignore[arg-type]
        layout=None, scope=tle.gpu.smem, nv_mma_shared_layout=True)
    b_buf = tle.gpu.alloc(
        [NUM_STAGES, BLOCK_SIZE_K, BLOCK_SIZE_N], dtype=tl.float16,  # type: ignore[arg-type]
        layout=None, scope=tle.gpu.smem, nv_mma_shared_layout=True)
    p = tle.pipe(capacity=NUM_STAGES, a_buf=a_buf, b_buf=b_buf)
    writer = p.writer()
    reader = p.reader()
