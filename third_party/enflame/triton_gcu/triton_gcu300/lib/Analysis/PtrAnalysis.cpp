...
bool isPtrFromLoad(Value v, llvm::DenseMap<Value, bool>& valueFromLoads) {
  ...
  TypeSwitch<Operation *>(v.getDefiningOp())
      ...
      .Case<arith::AddIOp, arith::MulIOp, arith::CmpIOp,
            arith::OrIOp>([&](auto op) {
      ...
      .Case<arith::ExtSIOp, arith::ExtUIOp>(
          [&](auto op) {
        bypass = isPtrFromLoad(op.getIn(), valueFromLoads);
      })
      .Case<arith::SelectOp, arith::DivSIOp, arith::SubIOp,
...
bool isPtrCandidate(Value v, const gcu::AxisInfoEx *axisInfoEx,
  ...
  for (int i = 0; i < rank - 1; ++i) {
    ...
    if (tshape[i] == 1) {
      LLVM_DEBUG(llvm::dbgs()
          << "bypass load/store op shape including 1: \n");
          return false;
    }
  ...
  if (auto arg = dyn_cast<BlockArgument>(v)) {
    ...
    if (blockArgOp &&
      ...
      candidate = initValue.getDefiningOp() ?
                      isMaskCandidate(initValue, valueToCandiates) : true;
