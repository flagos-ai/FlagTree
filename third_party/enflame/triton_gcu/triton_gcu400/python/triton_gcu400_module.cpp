...
void pipelineAddPass(uintptr_t h, const std::string &name,
  ...
  gcu400_pipeline_add_pass(reinterpret_cast<Gcu400Pipeline>(h),
                           name.c_str(), options.c_str());
}

std::string pipelineRun(uintptr_t h, const std::string &input) {
  Gcu400String s = gcu400_pipeline_run(
      reinterpret_cast<Gcu400Pipeline>(h), input.data(), input.size());
  if (!s.data) {
    std::string err = gcu400_pipeline_last_error(
        reinterpret_cast<Gcu400Pipeline>(h));
...
std::string runOpt(const std::string &input,
  ...
  Gcu400String s = gcu400_run_opt(input.data(), input.size(),
                                  cargs.data(),
