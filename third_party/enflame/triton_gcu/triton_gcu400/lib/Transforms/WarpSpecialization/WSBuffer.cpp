...
#define DEBUG_TYPE "gcu-ws-buffer"
...
void getAccumCntsPreOrder(Operation *ctrlOp,
  ...
  ctrlOp->walk<WalkOrder::PreOrder>([&](Operation *subOp) {
    if (isa<scf::ForOp, scf::IfOp>(subOp)) {
      for (auto *op : regionsWithChannels) {
        if (subOp == op)
...
scf::IfOp rewriteIfOp(scf::IfOp ifOp, SmallVector<Operation *> &taskTopOps,
  ...
  // Update regionsWithChannels with newIfOp.
  ...
  if (tmpIter3 != regionsWithChannels.end()) {
    LDBG() << "rewrite ifOp: update regionsWithChannels " << ifOp.getOperation()
           << " --> " << newIfOp.getOperation();
  ...
  unsigned parentArgSize = 0;
  unsigned parentTCnts = 0;
...
scf::ForOp createNewLoop(scf::ForOp forOp, SmallVector<Value> &initialAccums) {
...
scf::ForOp createNewLoopWrapper(scf::ForOp origForOp,
  ...
  LLVM_DEBUG({
    ...
    origForOp.dump();
    // origForOp.print(llvm::dbgs(), OpPrintingFlags().skipRegions());
    // llvm::dbgs() << "\n";
  });

  scf::ForOp parentForOp = origForOp->getParentOfType<scf::ForOp>();

  // pCnts: number of PC channels in ForOp/IfOp in parentForOp
  unsigned pArgSize = 0;
  unsigned pCnts = 0;
  unsigned accumArgId = 0;
  if (parentForOp) {
  ...
  // tCnts: number of PC channels in ForOp/IfOp in origForOp
  SmallVector<Operation *> preOrderOps;
  getAccumCntsPreOrder(origForOp.getOperation(), regionsWithChannels,
                       preOrderOps);
  unsigned tCnts = preOrderOps.size();

  // accumArgId: index of first PC channel in parentForOp
  if (tCnts > 0 && parentForOp) {
    accumArgId =
        getAccumArgIdx(parentForOp, preOrderOps[0], regionsWithChannels);
  }
  ...
  scf::ForOp newForOp = createNewLoop(origForOp, initialAccums);
  LLVM_DEBUG({
  ...
  // Handle the loop body. Repalce accumArg in YieldOp
