...
Value ConfigGcuStore(OpBuilder &rewriter, Location loc, Value storeValue,
      ...
      mlir::ValueRange configShapes, mlir::ValueRange configOffsets,
      triton::gcu::TagInfo tag) {
    ...
    SmallVector<unsigned, 4> memOrderToSrc;  // memOrderToSrc[k] = src dim at memory pos k
    if (bDynamicStride) {
      ...
      for (unsigned k = 0; k < rank; ++k) {
        ...
        memOrderToSrc.push_back(
            ordConst ? static_cast<unsigned>(*ordConst) : k);
      }
    } else {
    ...
    auto gatherByMemOrder = [&](Value memOrder, ArrayRef<Value> srcDimVals) {
      ...
      for (unsigned d = 1; d < srcDimVals.size(); ++d) {
        ...
        auto eq = rewriter.create<arith::CmpIOp>(
            loc, arith::CmpIPredicate::eq, memOrder, idx);
    ...
    SmallVector<Value, 4> vSlicehape;
    SmallVector<Value, 4> vIntSlicehape;
    SmallVector<Value, 4> sliceOffsets;   // src (value buffer) slice offset, memory order
    SmallVector<Value, 4> dstOffsets;    // dst (global tile) write offset, memory order
    Value totalSize = one;
    for (unsigned k = 0; k < rank; ++k) {
      ...
      Value srcOff = rewriter.create<arith::MaxSIOp>(
          loc, zero,
          rewriter.create<arith::SubIOp>(loc, maskStart, warpBase));
      ...
      Value validEnd = rewriter.create<arith::MinSIOp>(
          loc, warpEnd, validRegionEnd);
      auto shape = rewriter.create<arith::MaxSIOp>(
          loc, zero,
          rewriter.create<arith::SubIOp>(loc, validEnd, dstOff));
      vSlicehape.push_back(shape);
    ...
    for (unsigned k = 0; k < rank; ++k) {
      auto dim_diff = rewriter.create<arith::SubIOp>(
          loc, vStoreShapesMem[k], vSlicehape[k]);
            ...
            childBuilder.create<scf::IfOp>(loc, isNotZero,
              [&](OpBuilder &child2Builder, Location loc) {
                child2Builder.create<memref_ext::DesliceStartOp>(
                  loc, dst, reshapeStoreValue, dstOffsets,
                  tag.getTag(), ValueRange{tag.getIdx()});
                child2Builder.create<scf::YieldOp>(loc);
              });
              childBuilder.create<scf::YieldOp>(loc);
