...
_HEADER_LINE_RE = re.compile(
    r'^\s+(\w+)(?:\s*=\s*\d+)?\s*,\s*//\s*(llvm\.\w[\w.]*\w)\s*$'
)
def _find_header_dir() -> str:
    ...
    src_backend = os.path.join(
        os.path.dirname(_THIS_DIR), "third_party", "enflame", "backend"
    )
...
def _get_forward_pattern():
    ...
    if _forward_pattern_cache is None:
        table = _get_table()
        _forward_pattern_cache = re.compile(
            r'@(' + '|'.join(
                re.escape(k) for k in sorted(table.keys(), key=len, reverse=True)
            ) + r')\.(\w+)'
        )
...
def _get_reverse_pattern():
    ...
    if _reverse_pattern_cache is None:
        ...
        _reverse_pattern_cache = re.compile(
            r'(' + '|'.join(
                re.escape(v) for v in sorted(table.values(), key=len, reverse=True)
            ) + r')___(\w+)'
        )
