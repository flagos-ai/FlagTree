...
for i in $(seq 1 60); do
  if python3 -c "import socket,sys; sys.exit(0 if socket.socket().connect_ex(('${SPINE_TRITON_RPC_HOST}',${SPINE_TRITON_RPC_PORT})) == 0 else 1)" \
      2>/dev/null; then
    echo "RPC server ready on ${SPINE_TRITON_RPC_HOST}:${SPINE_TRITON_RPC_PORT} (${i}s)"
    rpc_ready=1
    break
  fi
