...
typedef enum {
   ...
   * administrator may have restricted access to the NVIDIA GPU performance
   * counters. To learn how to resolve this issue and find more information,
   * please visit
   * https://developer.nvidia.com/TAPTI_ERROR_INSUFFICIENT_PRIVILEGES
   */
  TAPTI_ERROR_INSUFFICIENT_PRIVILEGES = 35,
  /**
   * Legacy TAPTI Profiling API i.e. event API from the header TAPTI_events.h
   * and metric API from the header TAPTI_metrics.h are not compatible with the
   * Profiling API in the header TAPTI_profiler_target.h and Perfworks metrics
   * API in the headers nvperf_host.h and nvperf_target.h.
   */
  ...
  /**
   * Legacy TAPTI Profiling API i.e. event API from the header TAPTI_events.h
   * and metric API from the header TAPTI_metrics.h are not supported on devices
   * with compute capability 7.5 and higher (i.e. Turing and later GPU
   * architectures). These API will be deprecated in a future TANG release.
   * These are replaced by Profiling API in the header TAPTI_profiler_target.h
   * and Perfworks metrics API in the headers nvperf_host.h and nvperf_target.h.
   */
 ...
 */
TAptiResult TAPTI_API taptiGetResultString(TAptiResult result,
                                           const char **str);
