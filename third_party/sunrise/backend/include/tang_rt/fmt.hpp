...
__device__ void FundamentalFillImpl(const uint32_t id, const T t, uint32_t *buf,
...
template <typename T>
__device__ void FundamentalFill(const uint32_t id, const T t, uint32_t *buf,
                                uint32_t &pos, const uint32_t mask) {
...
static __device__ inline void StringFillData(const char *s,
                                             ...
                                             uint32_t *const buf, uint32_t &pos,
                                             const uint32_t mask) {
...
template <typename T> struct FmtTraits;

template <unsigned N> struct FmtTraits<char[N]> {
  static __device__ unsigned int FmtLength(const char *) {
    return 4 + ArgAlign(N);
  }

  static __device__ void Fill(const char *s, uint32_t *buf, uint32_t &pos,
                              uint32_t mask) {
    //!< Fill header
...
template <unsigned N> struct FmtTraits<const char[N]> {
  static __device__ unsigned int FmtLength(const char *) {
    return 4 + ArgAlign(N);
  }
...
template <> struct FmtTraits<const char *> {
...
template <> struct ArgTraits<char> {
  ...
  static __device__ void Fill(const char ch, uint32_t *buf, uint32_t &pos,
                              uint32_t mask) {
    buf[pos++ & mask] = ch << 16 | id;
  }
};

template <> struct ArgTraits<signed char> {
  ...
  static __device__ void Fill(const signed char ch, uint32_t *buf,
                              uint32_t &pos, uint32_t mask) {
    buf[pos++ & mask] = ch << 16 | id;
  }
};
...
template <> struct ArgTraits<unsigned char> {
  ...
  static __device__ void Fill(const unsigned char ch, uint32_t *buf,
                              uint32_t &pos, uint32_t mask) {
    buf[pos++ & mask] = ch << 16 | id;
  }
};

template <> struct ArgTraits<short int> {
  ...
  static __device__ void Fill(const short int ch, uint32_t *buf, uint32_t &pos,
                              uint32_t mask) {
    buf[pos++ & mask] = (ch << 16) | id;
  }
};

template <> struct ArgTraits<unsigned short int> {
  ...
  static __device__ void Fill(const unsigned short ch, uint32_t *buf,
                              uint32_t &pos, uint32_t mask) {
    buf[pos++ & mask] = (ch << 16) | id;
  }
};
...
template <> struct ArgTraits<int> {
  ...
  static __device__ void Fill(const int s, uint32_t *buf, uint32_t &pos,
                              uint32_t mask) {
...
template <> struct ArgTraits<unsigned int> {
  ...
  static __device__ void Fill(const unsigned int s, uint32_t *buf,
                              uint32_t &pos, uint32_t mask) {
...
template <> struct ArgTraits<long> {
  ...
  static __device__ void Fill(const long s, uint32_t *buf, uint32_t &pos,
                              uint32_t mask) {
...
template <> struct ArgTraits<unsigned long> {
  ...
  static __device__ void Fill(const unsigned long s, uint32_t *buf,
                              uint32_t &pos, uint32_t mask) {
...
template <> struct ArgTraits<long long> {
  ...
  static __device__ void Fill(const long long s, uint32_t *buf, uint32_t &pos,
                              uint32_t mask) {
...
template <> struct ArgTraits<unsigned long long> {
  ...
  static __device__ void Fill(const unsigned long long s, uint32_t *buf,
                              uint32_t &pos, uint32_t mask) {
...
template <> struct ArgTraits<float> {
  ...
  static __device__ void Fill(const float s, uint32_t *buf, uint32_t &pos,
                              uint32_t mask) {
...
template <> struct ArgTraits<double> {
  ...
  static __device__ void Fill(const double s, uint32_t *buf, uint32_t &pos,
                              uint32_t mask) {
    detail::FundamentalFill(ArgId_double, s, buf, pos, mask);
  }
};

template <> struct ArgTraits<long double> {
  ...
  static __device__ void Fill(const long double s, uint32_t *buf, uint32_t &pos,
                              uint32_t mask) {
    detail::FundamentalFill(ArgId_long_double, s, buf, pos, mask);
  }
};

template <> struct ArgTraits<std::nullptr_t> {
  static const int id = ArgId_nullptr;

  static __device__ unsigned int ArgLength(...) { return 4; }

  static __device__ void Fill(const std::nullptr_t, uint32_t *buf,
                              uint32_t &pos, uint32_t mask) {
    buf[pos++ & mask] = ArgId_nullptr;
  }
};

template <typename T> struct ArgTraits<T *> {
  static const int id = ArgId_pointer;

  static __device__ unsigned int ArgLength(...) { return 4 + sizeof(T *); }

  static __device__ void Fill(T *const s, uint32_t *buf, uint32_t &pos,
                              uint32_t mask) {
...
template <unsigned N> struct ArgTraits<char[N]> {
...
template <unsigned N> struct ArgTraits<signed char[N]> {
...
template <unsigned N> struct ArgTraits<unsigned char[N]> {
...
namespace detail {
template <typename... Args> __device__ unsigned SumArgsLength(Args &&...args);

template <> __device__ constexpr unsigned SumArgsLength() { return 0; }
...
template <typename T> struct ArgTraitsHasFillFifoInfo {
  template <typename U> static auto Check(int) -> decltype(&U::FillFifoInfo);
...
template <typename AT, bool value> struct ArgTraitsFillProxy;

template <typename T> struct ArgTraitsFillProxy<T, true> {
  template <typename U>
  __device__ static void Fill(U &&u, uint32_t *fifobuf, const FifoInfo &msgInfo,
                              uint32_t &pos, uint32_t &mask) {
    ArgTraits<T>::FillFifoInfo(std::forward<U>(u), fifobuf, msgInfo, pos, mask);
  }
};

template <typename T> struct ArgTraitsFillProxy<T, false> {
  template <typename U>
  __device__ static void Fill(U &&u, uint32_t *fifobuf, const FifoInfo &msgInfo,
                              uint32_t &pos, uint32_t &mask) {
...
__device__ void FillArgs(uint32_t *fifobuf, const FifoInfo &msgInfo,
                         uint32_t &pos, uint32_t mask, Args &&...args);

template <>
__device__ inline void FillArgs(uint32_t *fifobuf, const FifoInfo &msgInfo,
                                uint32_t &pos, uint32_t mask) {}

template <typename T, typename... Args>
__device__ void FillArgs(uint32_t *fifobuf, const FifoInfo &msgInfo,
                         uint32_t &pos, uint32_t mask, T &&t, Args &&...args) {
  typedef typename std::remove_reference<T>::type _type;
  typedef typename std::remove_cv<_type>::type type;

  ArgTraitsFillProxy<
      type, ArgTraitsHasFillFifoInfo<ArgTraits<type>>::value>::Fill(t, fifobuf,
                                                                    msgInfo,
                                                                    pos, mask);
...
template <typename... Args> struct CountOfArgs;

template <> struct CountOfArgs<> {
  static const int value = 0;
};

template <typename T, typename... Args> struct CountOfArgs<T, Args...> {
...
template <> struct ArgTraits<debug::MsgBeg> {
  static const int id = ArgId_uint;

  __device__ static unsigned int ArgLength(debug::MsgBeg put) { return 8; }

  __device__ static void FillFifoInfo(debug::MsgBeg, uint32_t *fifobuf,
...
template <> struct ArgTraits<debug::MsgGet> {
  static const int id = ArgId_uint;

  __device__ static unsigned int ArgLength(debug::MsgGet put) { return 8; }

  __device__ static void FillFifoInfo(debug::MsgGet, uint32_t *fifobuf,
                                      const detail::FifoInfo &msgInfo,
                                      uint32_t &pos, uint32_t mask) {
...
template <> struct ArgTraits<debug::MsgSize> {
  static const int id = ArgId_uint;

  __device__ static unsigned int ArgLength(debug::MsgSize s) { return 8; }

  __device__ static void FillFifoInfo(debug::MsgSize, uint32_t *fifobuf,
                                      const detail::FifoInfo &msgInfo,
                                      uint32_t &pos, uint32_t mask) {
    detail::FundamentalFill<uint32_t>(ArgId_uint, msgInfo.size, fifobuf, pos,
                                      mask);
  }
};

template <> struct ArgTraits<debug::FifoSize> {
  static const int id = ArgId_uint;

  __device__ static unsigned int ArgLength(debug::FifoSize s) { return 8; }

  __device__ static void FillFifoInfo(debug::FifoSize, uint32_t *fifobuf,
                                      const detail::FifoInfo &msgInfo,
                                      uint32_t &pos, uint32_t mask) {
    detail::FundamentalFill<uint32_t>(ArgId_uint, msgInfo.fifoSize, fifobuf,
                                      pos, mask);
  }
};

template <> struct ArgTraits<debug::FifoAddress> {
  static const int id = ArgId_pointer;

  __device__ static unsigned int ArgLength(debug::FifoAddress s) { return 12; }

  __device__ static void FillFifoInfo(debug::FifoAddress, uint32_t *fifobuf,
                                      const detail::FifoInfo &msgInfo,
                                      uint32_t &pos, uint32_t mask) {
    detail::FundamentalFill<uint64_t>(ArgId_pointer, msgInfo.fifoAddress,
                                      fifobuf, pos, mask);
  }
};
...
inline __device__ uint32_t __ptPrintfFifoAlloc(struct fifo *fifo, uint32_t n,
                                               uint32_t *pGet) {
...
#else
  } while (!__atomic_compare_exchange_n(&fifo->put, &oldPut, newPut, true,
...
__ptPrintfFifoUpdateReady(struct fifo *fifo, uint32_t orig_pos, uint32_t pos) {
  [[maybe_unused]] unsigned int tmp;
...
#else
  } while (!__atomic_compare_exchange_n(&fifo->ready, &oldReady, pos, true,
