...
LinearLayout sunrisemmaDotOperandToLinearLayout(DotOperandEncodingAttr dotEncAttr, ArrayRef<int64_t> shape) {
  ...
  bool kContig = (dotOpIdx == 0 ? sunriseMmaAttr.getIsACol() == false : sunriseMmaAttr.getIsBCol() == true);
  ...
  case 4:
  default:
    llvm::report_fatal_error((std::string("linearlayout not implemented! elemBitWidth:") + std::to_string(elemBitWidth)).c_str());
    break;
  }
