...
static Value traceToBaseMemRef(Value v, unsigned maxDepth = 16) {
  ...
  if (auto op = dyn_cast<arith::ExtUIOp>(defOp))
    return traceToBaseMemRef(op.getIn(), maxDepth - 1);
  if (auto op = dyn_cast<UnrealizedConversionCastOp>(defOp))
    return traceToBaseMemRef(op.getOperand(0), maxDepth - 1);
...
static Value generateBaseAddr(Value baseMemRef, Operation *insertBefore,
  ...
  // offset, ...}).  Find a reinterpret_cast user that ranks it, following
  // through builtin.cast users (the reinterpret_cast may be applied to the
  // cast result rather than to the unranked base itself).  The base may have
  // multiple reinterpret_cast users in different regions (e.g. scf.if vs
  ...
  if (isa<UnrankedMemRefType>(current.getType())) {
    ...
    // Walk the user chain (through casts and strided-metadata extraction)
    // looking for a ranked memref view of the base that dominates
    // insertBefore; its aligned pointer is the buffer base.
    ...
    while (!worklist.empty()) {
      ...
      if (isa<MemRefType>(v.getType()) &&
          (isa<memref::CastOp>(def) || isa<memref::ReinterpretCastOp>(def) ||
           isa<memref::ExtractStridedMetadataOp>(def))) {
        if (domInfo.properlyDominates(v, insertBefore)) {
          extractFrom = v;
          break;
        }
      }
      if (isa<UnrealizedConversionCastOp>(def) || isa<memref::CastOp>(def) ||
          isa<memref::ExtractStridedMetadataOp>(def)) {
