...
struct AtomicRMWOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    FailureOr<Value> valBuffer =
        getBuffer(rewriter, atomicRMWOp.getVal(), options, bufState);
    if (failed(valBuffer))
      return failure();
    FailureOr<Value> outputBuffer =
        getBuffer(rewriter, atomicRMWOp.getDst(), options, bufState);
...
struct AtomicCASOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    FailureOr<Value> cmpBuffer =
        getBuffer(rewriter, atomicCASOp.getCmp(), options, bufState);
    if (failed(cmpBuffer))
      return failure();

    FailureOr<Value> valBuffer =
        getBuffer(rewriter, atomicCASOp.getVal(), options, bufState);
    if (failed(valBuffer))
      return failure();

    FailureOr<Value> outputBuffer =
        getBuffer(rewriter, atomicCASOp.getDst(), options, bufState);
    if (failed(outputBuffer))
      return failure();
...
struct BitCastOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    FailureOr<Value> srcBuffer =
        getBuffer(rewriter, bitcastOp.getSrc(), options, bufState);
    if (failed(srcBuffer))
      return failure();
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    if (isa<TensorType>(sendOp.getDstAddr().getType())) {
      FailureOr<Value> dstBuffer =
          getBuffer(rewriter, sendOp.getDstAddr(), options, bufState);
      if (failed(dstBuffer))
        return failure();
