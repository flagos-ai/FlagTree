...
// 所有 memref 的 ViewLikeOpInterface 实现均在此覆盖。新增 view op 时务必同步更新。
// ──────────────────────────────────────────────────────────────────────────
static bool isAllocBacked(Value v) {
    if (!v) return false;
    if (v.getDefiningOp<memref::AllocOp>()) return true;
    if (auto op = v.getDefiningOp<memref::SubViewOp>()) return isAllocBacked(op.getSource());
    if (auto op = v.getDefiningOp<memref::ReinterpretCastOp>()) return isAllocBacked(op.getSource());
    if (auto op = v.getDefiningOp<memref::CollapseShapeOp>()) return isAllocBacked(op.getSrc());
    if (auto op = v.getDefiningOp<memref::ExpandShapeOp>()) return isAllocBacked(op.getSrc());
    if (auto op = v.getDefiningOp<memref::CastOp>()) return isAllocBacked(op.getSource());
    if (auto op = v.getDefiningOp<memref::ReshapeOp>()) return isAllocBacked(op.getSource());
    if (auto op = v.getDefiningOp<memref::MemorySpaceCastOp>()) return isAllocBacked(op.getSource());
    if (auto op = v.getDefiningOp<memref::ViewOp>()) return isAllocBacked(op.getSource());
    if (auto op = v.getDefiningOp<memref::TransposeOp>()) return isAllocBacked(op.getIn());
    return false;
}
...
static memref::AllocOp findBaseAlloc(Value v) {
    ...
    if (auto op = v.getDefiningOp<memref::SubViewOp>()) return findBaseAlloc(op.getSource());
    if (auto op = v.getDefiningOp<memref::ReinterpretCastOp>()) return findBaseAlloc(op.getSource());
    if (auto op = v.getDefiningOp<memref::CollapseShapeOp>()) return findBaseAlloc(op.getSrc());
    if (auto op = v.getDefiningOp<memref::ExpandShapeOp>()) return findBaseAlloc(op.getSrc());
    if (auto op = v.getDefiningOp<memref::CastOp>()) return findBaseAlloc(op.getSource());
    if (auto op = v.getDefiningOp<memref::ReshapeOp>()) return findBaseAlloc(op.getSource());
    if (auto op = v.getDefiningOp<memref::MemorySpaceCastOp>()) return findBaseAlloc(op.getSource());
    if (auto op = v.getDefiningOp<memref::ViewOp>()) return findBaseAlloc(op.getSource());
    if (auto op = v.getDefiningOp<memref::TransposeOp>()) return findBaseAlloc(op.getIn());
    return {};
}
...
static Value getCopyTarget(Operation *copyOp) {
    if (auto mcpy = dyn_cast<memref::CopyOp>(copyOp)) return mcpy.getTarget();
    if (auto lcpy = dyn_cast<linalg::CopyOp>(copyOp)) return lcpy.getOutputs()[0];
    return Value();
}
static Value getCopySource(Operation *copyOp) {
    if (auto mcpy = dyn_cast<memref::CopyOp>(copyOp)) return mcpy.getSource();
    if (auto lcpy = dyn_cast<linalg::CopyOp>(copyOp)) return lcpy.getInputs()[0];
    return Value();
}
...
static SmallVector<LoadGroup> collectPipelinedLoads(scf::ForOp forOp) {
    ...
    for (Operation &op : forOp.getBody()->without_terminator()) {
        ...
        if (!copyOp) continue;
        if (!isAllocBacked(target)) continue;
        if (isAllocBacked(source)) continue; // skip SPM→SPM copies; only DDR→SPM loads are pipelined
        ...
        for (Operation *cur = copyOp->getPrevNode(); cur != nullptr; cur = cur->getPrevNode()) {
            ...
            cur->walk([&](Operation *inner) {
                ...
                if (auto cpy = dyn_cast<memref::CopyOp>(inner))
                    if (cpy.getOperation() != copyOp && findBaseAlloc(cpy.getTarget()) == baseAlloc) writesBase = true;
                if (auto lcpy = dyn_cast<linalg::CopyOp>(inner))
                    if (lcpy.getOperation() != copyOp && findBaseAlloc(lcpy.getOutputs()[0]) == baseAlloc) writesBase = true;
...
static void cloneDefChainInLoopBody(Operation *root, scf::ForOp forOp, IRMapping &mapping, OpBuilder &builder,
    ...
    if (auto sv = dyn_cast<memref::SubViewOp>(root)) {
        ...
        cloned = builder.create<memref::SubViewOp>(sv.getLoc(), resultTy, newSrc, offs, sizes, strides);
        ...
        if (failed(resultTy)) {
            ...
            cloned = builder.create<memref::ExpandShapeOp>(expand.getLoc(), *resultTy, newSrc, reassoc, remappedMixed);
        }
        ...
        cloned = builder.create<memref::CollapseShapeOp>(collapse.getLoc(), resultTy, newSrc, reassoc);
    } else {
        cloned = builder.clone(*root, mapping);
...
struct PipelineRewriter {
    ...
    void buildLoadOnlyOps() {
        for (Operation *c : copies) loadOnlyOps.insert(c);
        ...
        for (Operation *c : copies)
            if (auto base = findBaseAlloc(getCopyTarget(c))) pipelinedAllocBases.insert(base.getOperation());
    ...
    SmallVector<Value> cloneComputeOps(IRMapping &mapping) {
        ...
        for (Operation &op : forOp.getBody()->without_terminator()) {
            if (llvm::is_contained(copies, &op)) continue;
    ...
    LogicalResult run() {
                ...
                [&](OpBuilder &b, Location l) {
                    ...
                    for (auto &grp : loadGroups) {
                        ...
                        if (grp.isOuterAlloc) {
                            ...
                            slot = b.create<memref::SubViewOp>(l, subType, multi, offsets, sizes, strides);
                        }
        ...
        for (auto &grp : loadGroups) {
            ...
            Value origSource = getCopySource(copyOp);
            if (Operation *def = origSource.getDefiningOp())
                cloneDefChainInLoopBody(def, forOp, copyMapping, rewriter, kernelCopyProducersDone, kernelCopyCloneOf);
