...
TRITON=$WORKSPACE/nv_flagtree_v3.6.x
TX8_DEPS_ROOT=$WORKSPACE/tx8_deps
LLVM=$WORKSPACE/llvm-22
...
if [ "$args1" == "pytest" ]; then
    ...
    # Derive --rootdir from the first test path so pytest discovers
    # conftest.py even when CWD contains its own pyproject.toml.
    for _arg in "$@"; do
        case "$_arg" in -*) continue ;; esac
        _test_file="${_arg%%::*}"
        _search_dir="$(cd "$(dirname "$_test_file")" 2>/dev/null && pwd)"
        while [ "$_search_dir" != "/" ]; do
            if [ -f "$_search_dir/pytest.ini" ] || [ -f "$_search_dir/setup.cfg" ]; then
                pytest_rootdir="--rootdir=$_search_dir"
                break
            fi
...
echo "run cmd:python3 $pytest_cmd $pytest_rootdir $@"

USE_SIM_MODE=${USE_SIM_MODE} python3 $pytest_cmd $pytest_rootdir $@
