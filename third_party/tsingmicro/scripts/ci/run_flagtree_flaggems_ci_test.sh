...
if [ $skip_build -ne 1 ]; then
	rm -rf flagtree/build
	bash ./third_party/tsingmicro/scripts/build_tsingmicro.sh
	if [ $? -eq 0 ]; then
		echo "Build flagtree complete!"
	else
		echo "Build flagtree fail!!!"
		exit -1
	fi
fi
##4.运行测试(triton业务负责)
...
#测试任务相关环境变量
export JSON_FILE_PATH=$project_dir/toolchain/FlagGems/tests
...
if [ $skip_run -ne 1 ]; then
	...
	if [ $quick_mode -eq 1 ]; then
		python ./toolchain/FlagGems/tests/test_flag_gems_ci.py --test_set $test_set --device_count $device_count --skip_device $skip_device --quick
	else
		python ./toolchain/FlagGems/tests/test_flag_gems_ci.py --test_set $test_set --device_count $device_count --skip_device $skip_device
	fi
