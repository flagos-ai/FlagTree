...
def producer_consumer_kernel(
    ...
    if pid < NUM_PRODUCERS:
        ...
        while i < num_inputs:
            ...
            tle.dsa.thrive.wait(remote_signal, wait_value=rep * 2)
            tle.dsa.thrive.putmem(queue_ptr + qoff * BLOCK_SIZE, input_ptr + i * BLOCK_SIZE, BLOCK_SIZE, peer)
            tle.dsa.thrive.notify(signal_ptr + qoff, peer, signal=rep * 2 + 1, sig_op="set")
            i += NUM_PRODUCERS
        ...
        while i < num_inputs:
            qoff = i % queue_size
            rep = i // queue_size
            tle.dsa.thrive.wait(signal_ptr + qoff, wait_value=rep * 2 + 1)
            offs = tl.arange(0, BLOCK_SIZE)
            data = tl.load(queue_ptr + qoff * BLOCK_SIZE + offs)
            tl.store(output_ptr + i * BLOCK_SIZE + offs, data)
            tle.dsa.thrive.notify(signal_ptr + qoff, pe, signal=rep * 2 + 2, sig_op="set")
            i += NUM_CONSUMERS
