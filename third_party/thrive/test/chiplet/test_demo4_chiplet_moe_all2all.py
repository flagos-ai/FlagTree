...
def moe_all2all_kernel(
    ...
    tle.dsa.thrive.putmem(data_dst_ptr, data_src_ptr, num_rows * HIDDEN, pid, blocking=False)
    if MODE == 0:
        tle.dsa.thrive.putmem(split_dst, split_src, EXPERTS_PER_RANK + 1, pid, blocking=False)
    tle.dsa.thrive.fence()
    tle.dsa.thrive.notify(signal_ptr, pid, signal=call_count, sig_op="set")
    tle.dsa.thrive.wait(wait_ptr, wait_value=call_count)
