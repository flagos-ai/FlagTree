...
#define GEN_PASS_DEF_ENCAPSULATELINALGOP
...
class EncapsulateLinalgOpPass
    : public ::impl::EncapsulateLinalgOpBase<EncapsulateLinalgOpPass> {
public:
  ...
  bool CanFuseTo(const Cluster &clsA, const Cluster &clsB) {
    ...
    if (utils::getAnnotation(lastOpA))
      return false;
  ...
  void setMemscopeForAllocTensorOp(bufferization::AllocTensorOp allocOp,
                                   OpBuilder &b) {
    allocOp.setMemorySpaceAttr(
        b.getI64IntegerAttr((int64_t)mlir::triton::MemScope::MM));
    if (isScalarType(allocOp.getType())) {
      allocOp.setMemorySpaceAttr(
          b.getI64IntegerAttr((int64_t)mlir::triton::MemScope::DDR));
    }
  }
  ...
  void annotatePrefetchToSubKernel(bufferization::AllocTensorOp allocOp,
                                   func::CallOp callOp, OpBuilder &b) {
  ...
  func::FuncOp createFuncOpWithCluster(OpBuilder &b, StringRef subFnName,
    ...
    for (auto outputAndResult : llvm::zip(outputs, callOp.getResults())) {
      ...
      if (auto tensorAllocOp =
              dyn_cast<bufferization::AllocTensorOp>(dstInputOp)) {
...
std::unique_ptr<mlir::Pass> createEncapsulateLinalgOpPass() {
  return std::make_unique<EncapsulateLinalgOpPass>();
}
