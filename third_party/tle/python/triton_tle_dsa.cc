...
// Inject a three-operand dsa binary op builder: create_dsa_<name>(lhs, rhs, out).
...
static void init_triton_tle_ir(py::module m) {
  ...
  builder_cls
      ...
      // Vendor-neutral same-nbytes type/shape reinterpret (e.g. i64[N]→i32[2N]).
      // Backends lower this (Tsingmicro: mk.bitcast alias; others: tensor.bitcast).
      ...
      builder_cls
          .def("create_dsa_to_tensor",
                [](TritonOpBuilder &self, Type resultTy, Value src, bool writable) -> Value {
                  self.getContext()->getOrLoadDialect<dsa::DsaDialect>();
                  auto &b = self.getBuilder();
                  auto writableAttr = b.getBoolAttr(writable);
                  auto op = self.create<dsa::ToTensorOp>(resultTy, src, writableAttr);
                  return op.getResult();
                })
      ...
      builder_cls
          .def("create_dsa_extract_slice",
                [](TritonOpBuilder &self, Type resultTy, Value src,
                   ...
                   py::list dynOffsets, const std::vector<int64_t> &sizes,
                ...
                py::arg("resultTy"), py::arg("src"),
                py::arg("staticOffsets"), py::arg("dynOffsets"),
