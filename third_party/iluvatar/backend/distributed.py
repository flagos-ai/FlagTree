...
class FlagcxRuntimeConfig:
    ...
    def __init__(self, path_order=0):
        ...
        if self.is_available:
            ...
            bitcode_paths = self._get_bitcode_paths(required=False)
            self.bitcode_path = bitcode_paths[path_order] if bitcode_paths else None
            self.shared_lib_path = self._get_shared_lib_paths()[path_order]
            self.include_path = self._get_include_paths()[path_order]

    def _check_path_available(self, paths, name, required=True):
    ...
    def _get_bitcode_paths(self, required=True):
        ...
        return self._check_path_available(paths, self.bt_name, required=required)
...
class Distributed:
    ...
    def __init__(self):
        self.extern_libs = ({"libflagcx": str(flagcx_rt_conf.bitcode_path)}
                            if flagcx_rt_conf.is_available and flagcx_rt_conf.bitcode_path else {})
