...
from triton._internal_testing import is_corex, is_cuda, is_hip, is_hopper_or_newer, get_hip_lds_size
...
def _is_layout_applicable(layout) -> bool:
    if isinstance(layout, (ttgl.BlockedLayout, ttgl.SwizzledSharedLayout, ttgl.DistributedLinearLayout)):
        ...
        if layout in ["padded_shared_layout_single_interval", "padded_shared_layout_multi_interval"]:
            return True
        if isinstance(layout, ttgl.iluvatar.IluvatarMMALayout):
            return True
        if isinstance(layout, ttgl.DotOperandLayout) and isinstance(layout.parent, ttgl.iluvatar.IluvatarMMALayout):
            return True
        return False
    elif is_cuda():
    ...
    for (M, N) in shapes:
        for layout in layouts:
            if isinstance(layout,
                          (ttgl.amd.AMDMFMALayout, ttgl.NVMMADistributedLayout, ttgl.iluvatar.IluvatarMMALayout)):
...
_mma_pairs = [
    ...
    # Iluvatar TCU (ivcore11) layouts
    [
        ttgl.iluvatar.IluvatarMMALayout(version=[1, 0], warps_per_cta=[2, 2], instr_shape=[16, 16, 16]),
        ttgl.iluvatar.IluvatarMMALayout(version=[1, 0], warps_per_cta=[4, 1], instr_shape=[16, 16, 16]),
    ],
    [
        ttgl.iluvatar.IluvatarMMALayout(version=[1, 0], warps_per_cta=[1, 4], instr_shape=[16, 16, 16]),
        ttgl.iluvatar.IluvatarMMALayout(version=[1, 0], warps_per_cta=[4, 1], instr_shape=[16, 16, 16]),
    ],
]
...
_ld_st_dot_layouts = _filter_layouts([
    ...
    ttgl.DotOperandLayout(parent=ttgl.NVMMADistributedLayout(version=[2, 0], warps_per_cta=[4, 1], instr_shape=[16, 8]),
                          operand_index=1, k_width=2),
    ttgl.DotOperandLayout(
        parent=ttgl.iluvatar.IluvatarMMALayout(version=[1, 0], warps_per_cta=[4, 1], instr_shape=[16, 16, 16]),
        operand_index=0, k_width=2),
    ttgl.DotOperandLayout(
        parent=ttgl.iluvatar.IluvatarMMALayout(version=[1, 0], warps_per_cta=[4, 1], instr_shape=[16, 16, 16]),
        operand_index=1, k_width=2),
...
def test_gather_layouts(axis, src_layout, index_layout, src_shape, idx_shape, device):
    ...
    if is_corex():
        # These layouts are warp-local at 32 threads per warp but not at 64, so the
        # gather correctly takes the shared memory path rather than the shuffle one.
        # Iluvatar does have a warp-local gather; it emits llvm.bi.slb.shfl.idx.
        return
