...
def test_iluvatar_mma(dtype):
    torch.manual_seed(42)
    B = ttgl.constexpr(128)
    threads_per_warp = ttgl.constexpr(THREADS_PER_WARP)
    # int8 TCU uses {16,16,32}; fp16/bf16/fp32 use {16,16,16}.
    instr_k = ttgl.constexpr(32 if dtype == torch.int8 else 16)
    ...
    def kernel(a_ptr, b_ptr, c_ptr, out_ptr):
        ...
        b = ttgl.convert_layout(ttgl.load(b_ptr + offs), rhs_layout)
        c = ttgl.convert_layout(ttgl.load(c_ptr + offs), acc_layout)
        # TCU acc is i32 for int8, f32 for fp16/bf16/fp32.
        if a.dtype.is_int():
            out = ttgl.iluvatar.ivcore11.mma(a, b, c)
        elif c.dtype != ttgl.float32:
            out = ttgl.iluvatar.ivcore11.mma(a, b, c.to(ttgl.float32)).to(c.dtype)
        else:
            out = ttgl.iluvatar.ivcore11.mma(a, b, c)
        ttgl.store(out_ptr + offs, ttgl.convert_layout(out, layout))

    if dtype == torch.int8:
        a = torch.randint(-8, 8, (B, B), dtype=torch.int8, device="cuda")
        b = torch.randint(-8, 8, (B, B), dtype=torch.int8, device="cuda")
        c = torch.randint(-8, 8, (B, B), dtype=torch.int32, device="cuda")
        out = torch.empty((B, B), dtype=torch.int32, device="cuda")
        compiled = kernel.warmup(a, b, c, out, grid=(1, ))
        kernel[(1, )](a, b, c, out)
        ref = c.cpu() + torch.matmul(a.cpu().to(torch.int32), b.cpu().to(torch.int32))
...
@pytest.mark.skipif(not is_corex(), reason="Requires Iluvatar corex")
def test_iluvatar_sme_async_copy_lhs_mma():
    torch.manual_seed(42)
    B = ttgl.constexpr(64)
    threads_per_warp = ttgl.constexpr(THREADS_PER_WARP)
    ...
    def kernel(a_ptr, b_ptr, out_ptr):
        layout: ttgl.constexpr = ttgl.BlockedLayout([1, 1], [threads_per_warp, 1], [ttgl.num_warps(), 1], [1, 0])
        # One warp must cover exactly one 16 rows x 64 bytes hardware tile, i.e.
        # 16x32 fp16 elements, so that its registers hold the tile addresses the
        # engine is handed.
        ...
        lhs_layout: ttgl.constexpr = ttgl.DotOperandLayout(parent=acc_layout, operand_index=0, k_width=2, use_sme=1)
        rhs_layout: ttgl.constexpr = ttgl.DotOperandLayout(parent=acc_layout, operand_index=1, k_width=2)
        a_shared_layout: ttgl.constexpr = ttgl.SwizzledSharedLayout(vec=32, per_phase=1, max_phase=1, order=[1, 0],
                                                                    use_tcu=True)

        a_row = ttgl.arange(0, B, layout=ttgl.SliceLayout(1, a_sme_layout))[:, None]
        a_col = ttgl.arange(0, B, layout=ttgl.SliceLayout(0, a_sme_layout))[None, :]
        a_smem = ttgl.allocate_shared_memory(ttgl.float16, [B, B], a_shared_layout)
        ttgl.iluvatar.ivcore11.async_copy.async_copy_global_to_shared(a_smem, a_ptr + a_row * B + a_col, stride=B)
        ttgl.iluvatar.ivcore11.async_copy.commit_group()
        ttgl.iluvatar.ivcore11.async_copy.wait_group(0)
        a = a_smem.load(lhs_layout)

        row = ttgl.arange(0, B, layout=ttgl.SliceLayout(1, layout))[:, None]
        col = ttgl.arange(0, B, layout=ttgl.SliceLayout(0, layout))[None, :]
        offsets = row * B + col
        b = ttgl.convert_layout(ttgl.load(b_ptr + offsets), rhs_layout)
        acc = ttgl.full([B, B], 0.0, ttgl.float32, acc_layout)
        result = ttgl.iluvatar.ivcore11.mma(a, b, acc)
        ttgl.store(out_ptr + offsets, ttgl.convert_layout(result, layout))

    a = 0.1 * torch.randn((B, B), dtype=torch.float16, device="cuda")
    b = 0.1 * torch.randn((B, B), dtype=torch.float16, device="cuda")
    out = torch.empty((B, B), dtype=torch.float32, device="cuda")
    compiled = kernel.warmup(a, b, out, grid=(1, ), num_warps=4)
    assert "llvm.bi.sme.load.16x1b64.rowxfb16" in compiled.asm["llir"]
    assert "call void @llvm.bi.sl.waitcnt(i64 8)" in compiled.asm["llir"]
    assert "llvm.bi.sl.barrier.alu" in compiled.asm["llir"]

    kernel[(1, )](a, b, out, num_warps=4)
    torch.testing.assert_close(out, torch.matmul(a, b).float(), atol=1e-2, rtol=1e-2)


@pytest.mark.skipif(not is_corex(), reason="Requires Iluvatar corex")
def test_iluvatar_sme_async_copy_rhs_mma():
    torch.manual_seed(42)
    B = ttgl.constexpr(64)
    threads_per_warp = ttgl.constexpr(THREADS_PER_WARP)
    ...
    def kernel(a_ptr, b_ptr, out_ptr):
        ...
        # elements, which is one 16 rows x 64 bytes hardware tile transposed.
        b_sme_layout: ttgl.constexpr = ttgl.BlockedLayout([8, 1], [4, 16], [2, 2], [0, 1], is_sme=True,
                                                          sme_warps_per_cta=[2, 2])
        b_shared_layout: ttgl.constexpr = ttgl.SwizzledSharedLayout(vec=32, per_phase=1, max_phase=1, order=[0, 1],
                                                                    use_tcu=True)

        b_k = ttgl.arange(0, B, layout=ttgl.SliceLayout(1, b_sme_layout))[:, None]
        b_n = ttgl.arange(0, B, layout=ttgl.SliceLayout(0, b_sme_layout))[None, :]
        b_smem = ttgl.allocate_shared_memory(ttgl.float16, [B, B], b_shared_layout)
        ttgl.iluvatar.ivcore11.async_copy.async_copy_global_to_shared(b_smem, b_ptr + b_k + b_n * B, stride=B)
        ttgl.iluvatar.ivcore11.async_copy.commit_group()
        ttgl.iluvatar.ivcore11.async_copy.wait_group(0)
        b = b_smem.load(rhs_layout)

        row = ttgl.arange(0, B, layout=ttgl.SliceLayout(1, layout))[:, None]
        col = ttgl.arange(0, B, layout=ttgl.SliceLayout(0, layout))[None, :]
        offsets = row * B + col
        a = ttgl.convert_layout(ttgl.load(a_ptr + offsets), lhs_layout)
        acc = ttgl.full([B, B], 0.0, ttgl.float32, acc_layout)
        result = ttgl.iluvatar.ivcore11.mma(a, b, acc)
        ttgl.store(out_ptr + offsets, ttgl.convert_layout(result, layout))

    a = 0.1 * torch.randn((64, 64), dtype=torch.float16, device="cuda")
    b = 0.1 * torch.randn((64, 64), dtype=torch.float16, device="cuda")
    b_col_major = b.T.contiguous()
    out = torch.empty((64, 64), dtype=torch.float32, device="cuda")
    compiled = kernel.warmup(a, b_col_major, out, grid=(1, ), num_warps=4)
    assert "llvm.bi.sme.load.16x1b64.colxfb16" in compiled.asm["llir"]
    assert "llvm.bi.matrix.mad.f32x4.f16x4" in compiled.asm["llir"]

    kernel[(1, )](a, b_col_major, out, num_warps=4)
    torch.testing.assert_close(out, torch.matmul(a, b).float(), atol=1e-2, rtol=1e-2)


@pytest.mark.skipif(not is_corex(), reason="Requires Iluvatar corex")
@pytest.mark.parametrize(
    "dtype, col_major, intrinsic",
    [
        (torch.float16, False, "llvm.bi.sme.load.16x1b64.rowxfb16"),
        (torch.float32, True, "llvm.bi.sme.load.16x1b64.colxfb32"),
        (torch.int16, False, "llvm.bi.sme.load.16x1b64.rowxfb16"),
        (torch.int32, True, "llvm.bi.sme.load.16x1b64.colxfb32"),
        (torch.int8, False, "llvm.bi.sme.load.16x1b64.rowxfb8"),
        (torch.int8, True, "llvm.bi.sme.load.16x1b64.colxfb8"),
    ],
    ids=["fp16-row", "fp32-col", "int16-row", "int32-col", "int8-row", "int8-col"],
)
def test_iluvatar_sme_async_copy_g2s(dtype, col_major, intrinsic):
    """SME g2s → shared → blocked load (no MMA). Covers dtype and row/col order."""
    B = ttgl.constexpr(64)
    threads_per_warp = ttgl.constexpr(THREADS_PER_WARP)

    @gluon.jit
    def kernel(in_ptr, out_ptr, COL_MAJOR: ttgl.constexpr):
        element_ty: ttgl.constexpr = in_ptr.dtype.element_ty
        # One SME tile is 16 rows x 64 bytes, so the contiguous dimension holds
        # `vec` elements and only B // vec warps fit along it. One warp has to
        # cover exactly that tile, which fixes size_per_thread and
        # threads_per_warp: 16 lanes walk the 16 rows, 4 lanes x vec // 4
        # elements walk the contiguous bytes.
        vec: ttgl.constexpr = 512 // element_ty.primitive_bitwidth
        strided_lanes: ttgl.constexpr = threads_per_warp // 4
        contig_warps: ttgl.constexpr = B // vec
        strided_warps: ttgl.constexpr = ttgl.num_warps() // contig_warps
        if COL_MAJOR:
            src_layout: ttgl.constexpr = ttgl.BlockedLayout([vec // 4, 1], [4, strided_lanes],
                                                            [contig_warps, strided_warps], [0, 1], is_sme=True,
                                                            sme_warps_per_cta=[contig_warps, strided_warps])
            shared_layout: ttgl.constexpr = ttgl.SwizzledSharedLayout(vec=vec, per_phase=1, max_phase=1, order=[0, 1],
                                                                      use_tcu=True)
        else:
            src_layout: ttgl.constexpr = ttgl.BlockedLayout([1, vec // 4], [strided_lanes, 4],
                                                            [strided_warps, contig_warps], [1, 0], is_sme=True,
                                                            sme_warps_per_cta=[strided_warps, contig_warps])
            shared_layout: ttgl.constexpr = ttgl.SwizzledSharedLayout(vec=vec, per_phase=1, max_phase=1, order=[1, 0],
                                                                      use_tcu=True)
        dst_layout: ttgl.constexpr = ttgl.BlockedLayout([1, 1], [threads_per_warp, 1], [ttgl.num_warps(), 1], [1, 0])

        src_dim0 = ttgl.arange(0, B, layout=ttgl.SliceLayout(1, src_layout))[:, None]
        src_dim1 = ttgl.arange(0, B, layout=ttgl.SliceLayout(0, src_layout))[None, :]
        if COL_MAJOR:
            src_offsets = src_dim0 + src_dim1 * B
        else:
            src_offsets = src_dim0 * B + src_dim1
        smem = ttgl.allocate_shared_memory(element_ty, [B, B], shared_layout)
        ttgl.iluvatar.ivcore11.async_copy.async_copy_global_to_shared(smem, in_ptr + src_offsets, stride=B)
        ttgl.iluvatar.ivcore11.async_copy.commit_group()
        ttgl.iluvatar.ivcore11.async_copy.wait_group(0)

        value = smem.load(dst_layout)
        dst_row = ttgl.arange(0, B, layout=ttgl.SliceLayout(1, dst_layout))[:, None]
        dst_col = ttgl.arange(0, B, layout=ttgl.SliceLayout(0, dst_layout))[None, :]
        ttgl.store(out_ptr + dst_row * B + dst_col, value)

    if dtype.is_floating_point:
        expected = torch.arange(64 * 64, dtype=torch.float32, device="cuda").to(dtype).reshape(64, 64)
    else:
        expected = torch.arange(64 * 64, dtype=torch.int32, device="cuda").to(dtype).reshape(64, 64)
    inp = expected.T.contiguous() if col_major else expected
    out = torch.empty_like(expected)
    compiled = kernel.warmup(inp, out, col_major, grid=(1, ), num_warps=4)
    assert intrinsic in compiled.asm["llir"]
    assert "llvm.bi.sl.waitcnt" in compiled.asm["llir"]

    kernel[(1, )](inp, out, col_major, num_warps=4)
    torch.testing.assert_close(out, expected, atol=0, rtol=0)


@pytest.mark.skipif(not is_corex(), reason="Requires Iluvatar corex")
@pytest.mark.parametrize(
    "valid_cols,use_other,other",
    [
        (64, False, 0.0),
        (0, False, 0.0),
        (37, True, -7.0),
    ],
    ids=["all-true", "all-false", "partial-with-other"],
)
def test_iluvatar_sme_async_copy_masked(valid_cols, use_other, other):
    B = ttgl.constexpr(64)
    threads_per_warp = ttgl.constexpr(THREADS_PER_WARP)

    @gluon.jit
    def kernel(in_ptr, out_ptr, runtime_other, VALID_COLS: ttgl.constexpr, USE_OTHER: ttgl.constexpr):
        # The mask keeps the pointer distribution so the shared-memory fixup can
        # reuse it; only sme_mask differs, which is what stops its make_range from
        # collapsing to a tile address.
        src_layout: ttgl.constexpr = ttgl.BlockedLayout([1, 8], [16, 4], [2, 2], [1, 0], is_sme=True,
                                                        sme_warps_per_cta=[2, 2])
        mask_layout: ttgl.constexpr = ttgl.BlockedLayout([1, 8], [16, 4], [2, 2], [1, 0], is_sme=True, sme_mask=True,
                                                         sme_warps_per_cta=[2, 2])
        dst_layout: ttgl.constexpr = ttgl.BlockedLayout([1, 1], [threads_per_warp, 1], [ttgl.num_warps(), 1], [1, 0])
        shared_layout: ttgl.constexpr = ttgl.SwizzledSharedLayout(vec=32, per_phase=1, max_phase=1, order=[1, 0],
                                                                  use_tcu=True)

        src_row = ttgl.arange(0, B, layout=ttgl.SliceLayout(1, src_layout))[:, None]
        src_col = ttgl.arange(0, B, layout=ttgl.SliceLayout(0, src_layout))[None, :]
        mask_row = ttgl.arange(0, B, layout=ttgl.SliceLayout(1, mask_layout))[:, None]
        mask_col = ttgl.arange(0, B, layout=ttgl.SliceLayout(0, mask_layout))[None, :]
        mask = mask_col + mask_row * 0 < VALID_COLS
        smem = ttgl.allocate_shared_memory(ttgl.float16, [B, B], shared_layout)
        if USE_OTHER:
            ttgl.iluvatar.ivcore11.async_copy.async_copy_global_to_shared(smem, in_ptr + src_row * B + src_col,
                                                                          stride=B, mask=mask, other=runtime_other)
        else:
            ttgl.iluvatar.ivcore11.async_copy.async_copy_global_to_shared(smem, in_ptr + src_row * B + src_col,
                                                                          stride=B, mask=mask)
        ttgl.iluvatar.ivcore11.async_copy.commit_group()
    ...
    inp = torch.arange(64 * 64, dtype=torch.float32, device="cuda").to(torch.float16).reshape(64, 64)
    out = torch.empty_like(inp)
    compiled = kernel.warmup(inp, out, other, VALID_COLS=valid_cols, USE_OTHER=use_other, grid=(1, ), num_warps=4)
    if valid_cols == 0:
        assert "llvm.bi.sme.load.16x1b64.rowxfb16" not in compiled.asm["llir"]
    else:
        assert "llvm.bi.sme.load.16x1b64.rowxfb16" in compiled.asm["llir"]

    kernel[(1, )](inp, out, other, VALID_COLS=valid_cols, USE_OTHER=use_other, num_warps=4)
    fill = other if use_other else 0.0
    expected = torch.where(
        torch.arange(64, device="cuda")[None, :] < valid_cols,
        inp,
        torch.full_like(inp, fill),
    )
    torch.testing.assert_close(out, expected, atol=0, rtol=0)


@pytest.mark.skipif(not is_corex(), reason="Requires Iluvatar corex")
def test_iluvatar_sme_async_copy_pipelined_mma():
    torch.manual_seed(42)
    B = ttgl.constexpr(64)
    threads_per_warp = ttgl.constexpr(THREADS_PER_WARP)
    ...
    def kernel(a_ptr, b_ptr, out_ptr):
        ...
        a_row = ttgl.arange(0, B, layout=ttgl.SliceLayout(1, a_sme_layout))[:, None]
        a_col = ttgl.arange(0, B, layout=ttgl.SliceLayout(0, a_sme_layout))[None, :]
        a_offsets = a_row * (2 * B) + a_col
        row = ttgl.arange(0, B, layout=ttgl.SliceLayout(1, layout))[:, None]
        col = ttgl.arange(0, B, layout=ttgl.SliceLayout(0, layout))[None, :]
        offsets = row * B + col

        a_smem0 = ttgl.allocate_shared_memory(ttgl.float16, [B, B], a_shared_layout)
        a_smem1 = ttgl.allocate_shared_memory(ttgl.float16, [B, B], a_shared_layout)

        # Prime A[:, :B], then overlap A[:, B:] with the first mma.
        ttgl.iluvatar.ivcore11.async_copy.async_copy_global_to_shared(a_smem0, a_ptr + a_offsets, stride=2 * B)
        ttgl.iluvatar.ivcore11.async_copy.commit_group()
        ttgl.iluvatar.ivcore11.async_copy.wait_group(0)
        ttgl.iluvatar.ivcore11.async_copy.async_copy_global_to_shared(a_smem1, a_ptr + B + a_offsets, stride=2 * B)
        ttgl.iluvatar.ivcore11.async_copy.commit_group()

        a0 = a_smem0.load(lhs_layout)
        b0 = ttgl.convert_layout(ttgl.load(b_ptr + offsets), rhs_layout)
        acc = ttgl.full([B, B], 0.0, ttgl.float32, acc_layout)
        acc = ttgl.iluvatar.ivcore11.mma(a0, b0, acc)

        ttgl.iluvatar.ivcore11.async_copy.wait_group(0)
        a1 = a_smem1.load(lhs_layout)
        b1 = ttgl.convert_layout(ttgl.load(b_ptr + B * B + offsets), rhs_layout)
        result = ttgl.iluvatar.ivcore11.mma(a1, b1, acc)
        ttgl.store(out_ptr + offsets, ttgl.convert_layout(result, layout))

    a = 0.1 * torch.randn((64, 128), dtype=torch.float16, device="cuda")
    b = 0.1 * torch.randn((128, 64), dtype=torch.float16, device="cuda")
    out = torch.empty((64, 64), dtype=torch.float32, device="cuda")
    compiled = kernel.warmup(a, b, out, grid=(1, ), num_warps=4)
    llir = compiled.asm["llir"]
    assert llir.count("call void @llvm.bi.sme.load.16x1b64.rowxfb16") >= 2
    assert llir.count("call void @llvm.bi.sl.waitcnt") >= 2
    assert "call void @llvm.bi.sl.waitcnt(i64 8)" in llir
    assert "llvm.bi.matrix.mad.f32x4.f16x4" in llir
...
@pytest.mark.skipif(not (is_ampere_or_newer() or is_corex()), reason="Requires Ampere or CoreX")
