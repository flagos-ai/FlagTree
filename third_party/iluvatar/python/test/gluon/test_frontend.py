...
COREX_TARGET = GPUTarget("corex", 71, 64)

# Upstream wheels bundle every backend, so a target's golden IR can be checked on any host.
# Iluvatar wheels only ship their own, leaving HIP targets with nothing to compile them.
skip_hip_on_corex = pytest.mark.skipif(is_corex(), reason="AMD backend is not built into Iluvatar wheels")

ALL_TARGETS = [AMPERE_TARGET, HOPPER_TARGET, BLACKWELL_TARGET, pytest.param(HIP_TARGET_RDNA4, marks=skip_hip_on_corex)]
...
def anonymize_ir(ir):
    ...
    return spec_call("filecheck_anonymize_ir", ir) or ir
...
def test_iluvatar_layout_default_cache_compatibility():
    blocked = ttgl.BlockedLayout([1, 1], [1, 32], [4, 1], [1, 0])
    blocked_key = ((1, 1), (1, 32), (4, 1), (1, 0), ())
    assert hash(blocked) == hash(blocked_key)
    assert blocked.mangle() == "B1_1_1_32_4_1_1_0_B"

    dot = ttgl.DotOperandLayout(0, blocked, 2)
    assert hash(dot) == hash((0, blocked, 2))
    assert dot.mangle() == f"DO0_{blocked.mangle()}_2DO"

    shared = ttgl.SwizzledSharedLayout(1, 1, 1, [1, 0])
    shared_key = (1, 1, 1, (1, 0), ())
    assert hash(shared) == hash(shared_key)
    assert shared.mangle() == "SSS_1_1_1_1_0__SSS"

    assert blocked != ttgl.BlockedLayout([1, 1], [1, 32], [4, 1], [1, 0], is_sme=True, sme_warps_per_cta=[1, 1])
    assert dot != ttgl.DotOperandLayout(0, blocked, 2, use_sme=1)
    assert shared != ttgl.SwizzledSharedLayout(1, 1, 1, [1, 0], use_tcu=True)


@gluon.jit
def iluvatar_sme_layout_kernel():
    blocked: ttgl.constexpr = ttgl.BlockedLayout([1, 8], [16, 4], [4, 1], [1, 0], is_sme=True, sme_warps_per_cta=[1, 1])
    value = ttgl.full([16, 32], 1.0, ttgl.float16, blocked)
    reduced = ttgl.sum(value, axis=1)
    ttgl.static_assert(reduced.type.layout == ttgl.SliceLayout(1, blocked))
    ...
    shared: ttgl.constexpr = ttgl.SwizzledSharedLayout(1, 1, 1, [1, 0], use_tcu=True)
    ttgl.allocate_shared_memory(ttgl.float16, [16, 32], shared)


@pytest.mark.skipif(not is_corex(), reason="Requires Iluvatar corex")
def test_iluvatar_sme_layouts_ir():
    module = run_parser(
        iluvatar_sme_layout_kernel,
        target=COREX_TARGET,
    )
...
@gluon.jit
def iluvatar_sme_async_copy_kernel(ptr, STRIDE: ttgl.constexpr):
    blocked: ttgl.constexpr = ttgl.BlockedLayout([1, 8], [16, 4], [4, 1], [1, 0], is_sme=True, sme_warps_per_cta=[1, 1])
    mask_blocked: ttgl.constexpr = ttgl.BlockedLayout([1, 8], [16, 4], [4, 1], [1, 0], is_sme=True, sme_mask=True,
                                                      sme_warps_per_cta=[1, 1])
    shared: ttgl.constexpr = ttgl.SwizzledSharedLayout(32, 1, 1, [1, 0], use_tcu=True)

    row = ttgl.arange(0, 16, ttgl.SliceLayout(1, blocked))
    col = ttgl.arange(0, 32, ttgl.SliceLayout(0, blocked))
    mask_row = ttgl.arange(0, 16, ttgl.SliceLayout(1, mask_blocked))
    mask_col = ttgl.arange(0, 32, ttgl.SliceLayout(0, mask_blocked))
    pointers = ptr + row[:, None] * 32 + col[None, :]
    mask = mask_row[:, None] + mask_col[None, :] * 0 < 8
    smem = ttgl.allocate_shared_memory(ttgl.float16, [16, 32], shared)

    ttgl.iluvatar.ivcore11.async_copy.async_copy_global_to_shared(smem, pointers, stride=STRIDE, mask=mask, other=0.0)
    ttgl.iluvatar.ivcore11.async_copy.commit_group()
    ttgl.iluvatar.ivcore11.async_copy.wait_group(0)
    ttgl.iluvatar.ivcore11.async_copy.wait_group(1)


@pytest.mark.skipif(not is_corex(), reason="Requires Iluvatar corex")
def test_iluvatar_sme_async_copy_ir():
    module = run_parser(
        iluvatar_sme_async_copy_kernel,
        *make_args(MockTensor(ttgl.float16), 32),
        target=COREX_TARGET,
    )
    ir = module.str_nodebug()
    assert "ttg.async_copy_global_to_local" in ir
    assert "inputStride" in ir
    assert "ttg.async_commit_group" in ir
    assert "ttg.async_wait {num = 0" in ir
    assert "ttg.async_wait {num = 1" in ir
    assert "smeMask = true" in ir
    assert "smeWarpsPerCTA = [1, 1]" in ir
    assert "vec = 32" in ir
    assert "useTcu = true" in ir
...
@pytest.mark.skipif(not is_corex(), reason="Requires Iluvatar corex")
def test_iluvatar_sme_async_copy_rejects_unaligned_stride():
    with pytest.raises(CompilationError, match="stride must be 64-byte aligned"):
        run_parser(
            iluvatar_sme_async_copy_kernel,
            *make_args(MockTensor(ttgl.float16), 31),
            target=COREX_TARGET,
        )
...
def test_convert_layout_assert_trivial():
    ...
    parent_layout: ttgl.constexpr = ttgl.BlockedLayout([1, 128], [64, 1], [4, 1], [0, 1])
    slice_layout: ttgl.constexpr = ttgl.SliceLayout(1, parent_layout)
    equiv_layout: ttgl.constexpr = ttgl.BlockedLayout([1], [64], [4], [0])

    value = ttgl.arange(0, 256, layout=slice_layout)
...
def test_convert_layout_not_trivial(target):
    ...
    assert "layout conversion from IluvatarBlockedLayout(size_per_thread=[2]" in str(e.value.__cause__)
    assert "to IluvatarBlockedLayout(size_per_thread=[1]" in str(e.value.__cause__)
    ...
    assert "to IluvatarBlockedLayout(size_per_thread=[2]" in str(e.value.__cause__)
    assert "is not trivial" in str(e.value.__cause__)
...
def test_warp_specialize():
    # CHECK:       [[BLOCKED:#.*]] = #ttg.blocked<{sizePerThread = [1], threadsPerWarp = [64], warpsPerCTA = [4], order = [0]}>
...
def ws_body(num_warps: ttgl.constexpr):
    anchor(ttgl.arange(0, 256, layout=ttgl.BlockedLayout([1], [64], [num_warps], [0])))
...
def test_num_warps_caller_context():
    # CHECK-DAG: [[BLOCKED_NW4:#.*]] = #ttg.blocked<{sizePerThread = [1], threadsPerWarp = [64], warpsPerCTA = [4], order = [0]}>
    # CHECK-DAG: [[BLOCKED_NW2:#.*]] = #ttg.blocked<{sizePerThread = [1], threadsPerWarp = [64], warpsPerCTA = [2], order = [0]}>
    # CHECK-DAG: [[BLOCKED_NW1:#.*]] = #ttg.blocked<{sizePerThread = [1], threadsPerWarp = [64], warpsPerCTA = [1], order = [0]}>

    # CHECK: func private @{{.*}}ws_test_default{{.*}}() attributes {noinline = false}
    # CHECK: func private @{{.*}}ws_body{{.*}}() attributes {noinline = false}
    # CHECK: func private @{{.*}}anchor{{.*}}(%arg0: tensor<256xi32, [[BLOCKED_NW4]]>) attributes {noinline = false}
    ...
    # CHECK: func private @{{.*}}anchor{{.*}}_NW2(%arg0: tensor<256xi32, [[BLOCKED_NW2]]>) attributes {noinline = false, "ttg.num-warps" = 2 : i32}
    ...
    # CHECK: func private @{{.*}}anchor{{.*}}_NW1(%arg0: tensor<256xi32, [[BLOCKED_NW1]]>) attributes {noinline = false, "ttg.num-warps" = 1 : i32}
    ttgl.warp_specialize([
        (ws_test_default, ()),
        (ws_test_worker0, ()),
        (ws_test_worker1, ()),
...
def test_mlir_attr_error():
    ...
    @gluon.jit
    def kernel():
        ttgl.arange(0, 1, layout=ttgl.BlockedLayout([1], [64], [4], [1]))
...
def test_tensor_layout_type_changed():
    ...
    @gluon.jit
    def kernel():
        layout: ttgl.constexpr = ttgl.BlockedLayout(size_per_thread=[1, 1], threads_per_warp=[1, 64],
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "...", "ttg.threads-per-warp" = 64 : i32} {
  tt.func public @kernel() attributes {noinline = false} {
    ...
    tt.call @"test_frontend.smem_and_layout_user__MDi32S32_32SLSSS_1_1_1_1_0__SSSLAS[32, 32]ASMD__(1,)cconstexpr_IluvatarSwizzledSharedLayout(vec=1, per_phase=1, max_phase=1, order=(1, 0), cga_layout=__, use_tcu=False)_"(%0) : (!ttg.memdesc<32x32xi32, #shared, #smem, mutable>) -> ()
    tt.return
  }
  tt.func private @"test_frontend.smem_and_layout_user__MDi32S32_32SLSSS_1_1_1_1_0__SSSLAS[32, 32]ASMD__(1,)cconstexpr_IluvatarSwizzledSharedLayout(vec=1, per_phase=1, max_phase=1, order=(1, 0), cga_layout=__, use_tcu=False)_"(%arg0: !ttg.memdesc<32x32xi32, #shared, #smem, mutable>) attributes {noinline = false} {
    tt.return
  }
}
""")
...
@pytest.mark.skipif(is_corex(), reason="NVMMADistributedLayout is 32 threads per warp")
@filecheck_test
@gluon.jit
...
def test_tensor_permute():
    # CHECK-DAG: [[BLOCKED:#.*]] = #ttg.blocked<{sizePerThread = [1, 2], threadsPerWarp = [4, 16], warpsPerCTA = [4, 1], order = [1, 0]}>
    # CHECK-DAG: [[BLOCKED1:#.*]] = #ttg.blocked<{sizePerThread = [2, 1], threadsPerWarp = [16, 4], warpsPerCTA = [1, 4], order = [0, 1]}>
    layout: ttgl.constexpr = ttgl.BlockedLayout([1, 2], [4, 16], [4, 1], [1, 0])
    a = ttgl.full([32, 32], 0, ttgl.int32, layout=layout)
    # CHECK: tt.trans{{.*}} : tensor<32x32xi32, [[BLOCKED]]> -> tensor<32x32xi32, [[BLOCKED1]]>
    res = ttgl.permute(a, [1, 0])
    permuted_layout: ttgl.constexpr = ttgl.BlockedLayout([2, 1], [16, 4], [1, 4], [0, 1])
...
@gluon.jit
def test_split_join():
    # CHECK: [[BLOCKED:#.*]] = #ttg.blocked<{sizePerThread = [2], threadsPerWarp = [64], warpsPerCTA = [4], order = [0]}>
    # CHECK: [[BLOCKED1:#.*]] = #ttg.blocked<{sizePerThread = [2, 2], threadsPerWarp = [64, 1], warpsPerCTA = [4, 1], order = [1, 0]}>
    layout: ttgl.constexpr = ttgl.BlockedLayout([2], [64], [4], [0])
    ...
    res = ttgl.join(a, b)
    expect_layout: ttgl.constexpr = ttgl.BlockedLayout([2, 2], [64, 1], [4, 1], [1, 0])
...
def test_reshape_linear_layout():
    # CHECK: [[BLOCKED:#.*]] = #ttg.blocked<{sizePerThread = [1, 1], threadsPerWarp = [64, 1], warpsPerCTA = [4, 1], order = [0, 1]}>
    # CHECK: [[LINEAR:#.*]] = #ttg.linear
    layout: ttgl.constexpr = ttgl.BlockedLayout([1, 1], [64, 1], [4, 1], [0, 1])
...
def test_tensor_reshape():
    # CHECK: [[BLOCKED:#.*]] = #ttg.blocked<{sizePerThread = [2], threadsPerWarp = [64], warpsPerCTA = [4], order = [0]}>
    # CHECK: [[BLOCKED1:#.*]] = #ttg.blocked<{sizePerThread = [1, 1, 2], threadsPerWarp = [4, 4, 4], warpsPerCTA = [4, 1, 1], order = [2, 1, 0]}>
    layout: ttgl.constexpr = ttgl.BlockedLayout([2], [64], [4], [0])
    a = ttgl.full([256], 1, ttgl.int32, layout)
    # CHECK: tt.reshape {{.*}} : tensor<256xi32, [[BLOCKED]]> -> tensor<8x4x8xi32, [[BLOCKED1]]>
    v = a.reshape([8, 4, 8])
    expect_layout: ttgl.constexpr = ttgl.BlockedLayout([1, 1, 2], [4, 4, 4], [4, 1, 1], [2, 1, 0])
...
def test_zeros():
    ...
    layout: ttgl.constexpr = ttgl.BlockedLayout([2], [64], [4], [0])
    layout_2d: ttgl.constexpr = ttgl.BlockedLayout([1, 2], [4, 16], [4, 1], [1, 0])
...
def test_auto_layout():
    ...
    ttgl.set_auto_layout(i, ttgl.BlockedLayout([1], [64], [4], [0]))
...
def test_auto_layout_broadcast():
    ...
    y = ttgl.full([1, 16], 2, ttgl.int32, layout=ttgl.BlockedLayout([1, 1], [1, 64], [4, 1], [1, 0]))
...
def test_layout_zeros():
    ...
    ttgl.zeros([128], ttgl.float32, layout=ttgl.BlockedLayout([1], [64], [4], [0]))
...
def test_mismatch_shape_and_layout_rank():
    ...
    @gluon.jit
    def kernel():
        layout: ttgl.constexpr = ttgl.BlockedLayout([1, 1], [1, 64], [1, 4], [1, 0])
...
def test_non_scalar_loop_bounds():
    ...
    @gluon.jit
    def kernel():
        x = ttgl.full([32], 0, ttgl.int32, layout=ttgl.BlockedLayout([1], [64], [1], [0]))
...
def test_auto_layout_convert_store_val():
    ...
    def kernel(out_ptr,  #
        ...
        blocked: ttgl.constexpr = ttgl.BlockedLayout([1, 4], [64, 1], [2, 2], [1, 0])
    ...
    expecttest.assert_expected_inline(
        anonymize_ir(module.str_nodebug()), """\
#blocked = #ttg.blocked<{sizePerThread = [1, 4], threadsPerWarp = [64, 1], warpsPerCTA = [2, 2], order = [1, 0]}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "...", "ttg.threads-per-warp" = 64 : i32} {
