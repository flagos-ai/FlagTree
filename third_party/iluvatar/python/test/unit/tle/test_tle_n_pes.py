...
def test_n_pes_lowers_to_flagcx_abi():
    ...
    assert "flagcxDevCommGetIntraSize" in llir, llir
    assert re.search(r"(?:declare|define).*i32 @flagcxDevCommGetIntraSize\(", llir), llir


def test_n_pes_composes_with_get_device_id():
    llir = _compile_peer_rank().asm["llir"]
    assert "flagcxDevCommGetIntraRank" in llir, llir
    assert "flagcxDevCommGetIntraSize" in llir, llir
