...
class TestDeviceSpaceBarrier:
    ...
    def test_device_barrier_carries_device_space_attrs(self):
        ...
        assert 'order = "acqrel"' in ttir, ttir
    ...
    def test_device_barrier_lowers_to_flagcx_abi(self, barrier_kind, symbol):
        ...
        assert re.search(rf"(?:declare|define).*void @{symbol}\(", llir), llir
    ...
    def test_device_barrier_call_matches_corex_bitcode_abi(self):
        ...
        define = re.search(r"(?:declare|define)[^\n]*@flagcxIntraBarrierSyncS\([^\n]*", llir)
        assert define, llir
        assert "ptr addrspace(1)" in define.group(0), define.group(0)
