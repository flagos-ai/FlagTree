...
class TestDeviceSpaceRemote:
    ...
    def test_remote_lowers_to_flagcx_abi(self):
        ...
        assert re.search(r"(?:declare|define).*@flagcxGetIntraPointerC\(", llir), llir
    ...
    def test_remote_call_matches_corex_bitcode_abi(self):
        ...
        assert call, llir
        assert "ptr addrspace(1)" in call.group(0), call.group(0)
    ...
    def test_remote_result_is_already_global(self):
        llir = _compile_remote_read().asm["llir"]
        assert re.search(r"(?:declare|define)[^\n]*ptr addrspace\(1\) @flagcxGetIntraPointerC\(", llir), llir
        # The adapter does its own generic-to-global cast on the way out, so
        # scope this to the kernel: a cast there means the two sides disagree
        # about the address space again.
        kernel = re.search(r"define[^\n]*@_remote_read_kernel\b.*?\n}", llir, re.S)
        assert kernel, llir
        assert "addrspacecast" not in kernel.group(0), kernel.group(0)
