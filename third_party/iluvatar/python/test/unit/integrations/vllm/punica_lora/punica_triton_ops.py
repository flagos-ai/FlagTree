...
@triton.jit
def mm_k(a_ptr, b_ptr, ak_stride, bn_stride, bk_stride, offset_k,
         K: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr,
         BLOCK_K: tl.constexpr, EVEN_K: tl.constexpr, SPLIT_K: tl.constexpr,
         CAST_TYPE: tl.constexpr, b_dtype: tl.constexpr,
...
def do_expand_kernel(
    ...
    SPLIT_K = 1
    accumulator = mm_k(a_ptr, b_ptr, input_d2_stride, cur_lora_d1_stride,
                       cur_lora_d2_stride, offset_k, K, BLOCK_M, BLOCK_N,
                       BLOCK_K, EVEN_K, SPLIT_K, CAST_TYPE,
                       cur_lora_ptr.dtype.element_ty, USE_STRIDE_LOAD)
...
def do_shrink_kernel(
    ...
    accumulator = mm_k(a_ptr, b_ptr, input_d1_stride, lora_d1_stride,
                       lora_d2_stride, offset_k, K, BLOCK_M, BLOCK_N, BLOCK_K,
                       EVEN_K, SPLIT_K, False, cur_lora_ptr.dtype.element_ty,
                       USE_STRIDE_LOAD)
...
def lora_expand(
    ...
    if same_stride:
        ...
        use_stride_load = all((w.stride(1) * elem_size) % 64 == 0
