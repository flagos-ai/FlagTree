...
def _make_bench_runner(
    ...
    if _is_iluvatar_backend() and provider in ("triton_atomic_fused", "tle_cluster_fused"):
        raise ValueError(f"{provider} is disabled on Iluvatar: unsupported cooperative grid/CTA cluster semantics")
...
def _make_yiakwy_cuda_runner(
    ...
    if _is_iluvatar_backend():
        # CoreX does not support cooperative_groups::grid_group/this_grid.
        return None
...
def run_realistic_benchmark(block_size: int, num_experts: int) -> None:
    ...
    # CoreX does not support cooperative_groups::grid_group/this_grid.
    yiakwy_available = not _is_iluvatar_backend() and num_experts <= 256 and _load_embedded_yiakwy_cuda_moe_align(
    ) is not None
