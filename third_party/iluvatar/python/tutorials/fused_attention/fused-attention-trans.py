...
def _fwd_kernel(
    ...
    if IS_CAUSAL:
        ...
        start_m = tl.where(start_h % 2 == 0, num_m_blocks - pid_m - 1, pid_m)
    else:
        start_m = pid_m
    ...
    off_q = offs_zh * stride_qh + offs_m[None, :] * stride_qm + offs_d[:, None] * stride_qk
    off_k = kv_offs_zh * stride_kh + offs_n[:, None] * stride_kn + offs_d[None, :] * stride_kk
...
def test_attention(Z, H, KV_H, N_CTX, HEAD_DIM, dtype, is_causal, test_backward):
    ...
    def tol(ref):
        ...
        # bf16 keeps 8 mantissa bits, so summing the KV loop in a different order
        # than the reference is worth a few ULP at the tensor's scale -- also on the
        # elements the reference cancels to ~0, where a relative tolerance grants
        # nothing.  1e-2 covers the tensors small enough to be all cancellation
        # (|dq| peaks at 0.01 while its terms are far larger), and 8 ULP at the
        # tensor's own scale covers dv, which in the GQA shapes accumulates 8 query
        # heads, reaches |dv| ~ 12.5 and lands 0.125 away on ~5e-5 of its elements.
        return dict(atol=max(1e-2, 8 * torch.finfo(dtype).eps * ref.abs().max().item()), rtol=1e-2)

    torch.testing.assert_close(ref_out, tri_out, **tol(ref_out), equal_nan=True)
    if test_backward:
        torch.testing.assert_close(ref_dv, tri_dv, **tol(ref_dv), equal_nan=True)
        torch.testing.assert_close(ref_dk, tri_dk, **tol(ref_dk), equal_nan=True)
        torch.testing.assert_close(ref_dq, tri_dq, **tol(ref_dq), equal_nan=True)
