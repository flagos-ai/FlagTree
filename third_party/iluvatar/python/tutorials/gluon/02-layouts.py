...
# ```
#
# Where `R` is a power of 2. `THREADS_PER_WARP` comes from the current target
# (32 on NVIDIA / RDNA, 64 on Iluvatar / CDNA).
...
def test_memcpy_1d(XBLOCK, xnumel, num_warps):
    ...
    input = torch.randn(xnumel, device="cuda")
    output = torch.empty_like(input)
    layout = gl.BlockedLayout([1], [THREADS_PER_WARP], [num_warps], [0])
...
if __name__ == "__main__" and _enabled("R_vs_throughput"):
    ...
    for i in range(0, 5):
        R = 2**i
        layout = gl.BlockedLayout([R], [THREADS_PER_WARP], [num_warps], [0])
...
if __name__ == "__main__" and _enabled("LDG_STG_instructions") and not is_corex():
...
def test_memcpy_2d(XBLOCK, YBLOCK, xnumel, ynumel, transposed, num_warps):
    ...
    layout = gl.BlockedLayout([1, 1], [1, THREADS_PER_WARP], [1, num_warps], [1, 0])
...
if __name__ == "__main__" and _enabled("memcpy_2d_layout"):
    ...
    XBLOCK = 1
    YBLOCK = 2048
    layout = gl.BlockedLayout([1, 1], [1, THREADS_PER_WARP], [1, 4], [1, 0])
...
if __name__ == "__main__" and _enabled("memcpy_2d_layout"):
    layout = gl.BlockedLayout([1, 1], [THREADS_PER_WARP, 1], [4, 1], [0, 1])
...
def get_layout_for_gmem_access(tensor, num_warps):
    if len(tensor.shape) == 1:
        return gl.BlockedLayout([1], [THREADS_PER_WARP], [num_warps], [0])
    ...
    if tensor.stride(1) == 1:
        return gl.BlockedLayout([1, 1], [1, THREADS_PER_WARP], [1, num_warps], [1, 0])
    else:
        return gl.BlockedLayout([1, 1], [THREADS_PER_WARP, 1], [num_warps, 1], [0, 1])
