...
#include "Transforms/RemoveRedundantCopy.h"
...
#define GEN_PASS_DEF_TRITONILUVATARTLEREMOVEREDUNDANTCOPY
...
struct TritonIluvatarTleRemoveRedundantCopy
    : public iluvatar_tle::impl::TritonIluvatarTleRemoveRedundantCopyBase<
          TritonIluvatarTleRemoveRedundantCopy> {
...
ForOpArgConversion::matchAndRewrite(scf::ForOp forOp,
  ...
  for (auto &op : forOp.getBody()->without_terminator()) {
    if (isa<iluvatar_tle::DSLRegionOp>(op)) {
      if (dslRegionOp)
        return failure();
      dslRegionOp = dyn_cast<iluvatar_tle::DSLRegionOp>(op);
    }
  }

  if (!dslRegionOp || !iluvatar_tle::isSingleForLoop(forOp))
    return failure();
...
void mlir::triton::iluvatar_tle::populateRemoveRedundantCopyPatterns(
    RewritePatternSet &patterns) {
  patterns.add<ForOpArgConversion>(patterns.getContext());
}

void TritonIluvatarTleRemoveRedundantCopy::runOnOperation() {
  RewritePatternSet patterns(&getContext());
  iluvatar_tle::populateRemoveRedundantCopyPatterns(patterns);
