  ...
  builderCls
      ...
      .def("create_distributed_barrier",
           [](TritonOpBuilder &self) -> void {
             self.create<iluvatar_tle::DistributedBarrierOp>(
                 ...
                 mlir::StringAttr(), mlir::StringAttr(), mlir::IntegerAttr(),
                 mlir::IntegerAttr(), mlir::StringAttr(), mlir::IntegerAttr(),
      ...
      .def(
          ...
          [](TritonOpBuilder &self, const std::string &groupKind,
            ...
            self.create<iluvatar_tle::DistributedBarrierOp>(
                mlir::Value(), mlir::StringAttr(), mlir::StringAttr(),
                mlir::StringAttr(), kindAttr, mlir::IntegerAttr(),
                mlir::IntegerAttr(), mlir::StringAttr(), rankAttr, shapeAttr,
                axesAttr, maskAttr);
          },
...
//
// Only the two enums the barrier path needs are exposed. The signal enums
...
//
// Values must track flagcxTeamKind_t / flagcxCoopKind_t, the same source the
// lowering in tle/dialect/lib/Conversion/TleToLLVM/DistributedBarrierOpToLLVM.
// cpp switches on.
namespace {

enum class FlagCXTeamKind : int32_t { INTRA = 0, INTER = 1, WORLD = 2 };
enum class FlagCXCoopKind : int32_t { THREAD = 0, WARP = 1, BLOCK = 2 };
...
void init_triton_iluvatar_tle_compat(py::module &&m) {
  ...
  py::enum_<FlagCXTeamKind>(attr, "FlagCXTeamKind")
      .value("Intra", FlagCXTeamKind::INTRA)
      .value("Inter", FlagCXTeamKind::INTER)
      .value("World", FlagCXTeamKind::WORLD)
      .def_static(
          "from_str",
          [](const std::string &name) -> std::optional<FlagCXTeamKind> {
            if (name == "intra")
              return FlagCXTeamKind::INTRA;
            if (name == "inter")
              return FlagCXTeamKind::INTER;
            if (name == "world")
              return FlagCXTeamKind::WORLD;
            return std::nullopt;
          },
          py::arg("name"))
      .def_static(
          "from_int",
          [](int value) -> std::optional<FlagCXTeamKind> {
            if (value < static_cast<int>(FlagCXTeamKind::INTRA) ||
                value > static_cast<int>(FlagCXTeamKind::WORLD))
              return std::nullopt;
            return static_cast<FlagCXTeamKind>(value);
          },
          py::arg("value"));

  py::enum_<FlagCXCoopKind>(attr, "FlagCXCoopKind")
      .value("Thread", FlagCXCoopKind::THREAD)
      .value("Warp", FlagCXCoopKind::WARP)
      .value("Block", FlagCXCoopKind::BLOCK)
      .def_static(
          "from_str",
          [](const std::string &name) -> std::optional<FlagCXCoopKind> {
            if (name == "thread")
              return FlagCXCoopKind::THREAD;
            if (name == "warp")
              return FlagCXCoopKind::WARP;
            if (name == "block")
              return FlagCXCoopKind::BLOCK;
            return std::nullopt;
          },
          py::arg("name"));
