...
def _batch_benchmark(self, kernel_dict, rep=10, quantiles=None):
    ...
    if self.do_bench.__module__ == "triton.testing":
        ...
        if torch.npu.is_available() and enable_bench_npu:
            ...
            tiling_dict_temp = do_bench_multiple_kernel_npu(kernel_dict_temp, active=max(30, rep), prof_dir=None,
                                                            keep_res=False)
...
def _profile(autotuner, *args, config, **meta):
    ...
    do_bench_npu(kernel_call, prof_dir=autotuner.auto_profile_dir, keep_res=True)
...
def set_Config_extra_options(config, bishengir_options):
    ...
    config.limit_auto_multi_buffer_only_for_local_buffer = bishengir_options.get(
        "limit_auto_multi_buffer_only_for_local_buffer", None)  # Compiler Default False
    config.limit_auto_multi_buffer_of_local_buffer = bishengir_options.get("limit_auto_multi_buffer_of_local_buffer",
    ...
    config.enable_hivm_auto_cv_balance = bishengir_options.get("enable_hivm_auto_cv_balance",
