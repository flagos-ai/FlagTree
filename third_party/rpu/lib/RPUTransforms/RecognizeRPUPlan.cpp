...
DictionaryAttr mapStringIntToDictAttr(MLIRContext *ctx,
...
DictionaryAttr signatureToDictAttr(MLIRContext *ctx, const RPUPlan &plan) {
  ...
  attrs.append(
      builder.getNamedAttr("return_type", builder.getStringAttr(plan.returnType)));
  return attrs.getDictionary(ctx);
}
...
FailureOr<plan::KernelOp> createRPUPlanKernelOp(ModuleOp module,
  ...
  if (!signature || !layout || !mask || !emission) {
    module.emitError("failed to convert RPUPlan JSON payload to MLIR attributes");
    return failure();
  }
  ...
  auto op = builder.create<plan::KernelOp>(
      source.getLoc(),
      builder.getStringAttr(planSymbol),
      builder.getStringAttr(plan.kernelName),
      FlatSymbolRefAttr::get(ctx, source.getSymName()),
      builder.getI32IntegerAttr(plan.version),
      builder.getStringAttr(plan.pattern),
      signature,
      mapStringIntToDictAttr(ctx, plan.shape),
      mapStringIntToDictAttr(ctx, plan.args),
      layout,
      mask,
      requiredDslFeaturesToArrayAttr(ctx, plan.requiredDslFeatures),
      emission);
  return op;
}
...
void setPlanTraceAttr(ModuleOp module, std::optional<StringRef> selected,
                      ...
                      ArrayRef<TraceAttempt> attempts) {
  module->setAttr("rpu.plan.trace",
                  StringAttr::get(module.getContext(),
                                  serializePlanTraceToJson(
                                      selected, functionName, functionLocation,
                                      attempts)));
}
...
RPUPlan makeAddPlan(triton::FuncOp func, const AddOperands &operands) {
  ...
  if (operands.masked) {
    ...
    plan.requiredDslFeatures = {
        "rpu.make_tensor", "rpu.local_tile", "ctx.load", "tile.add",
        "ctx.store"};
    ...
    plan.requiredDslFeatures = {
        "ctx.load_contig", "tile.add", "ctx.store_contig"};
  } else {
    plan.layout = llvm::json::Object{
        {"memory", "tensor"},
    ...
    plan.mask = llvm::json::Object{{"masked", false}};
    plan.requiredDslFeatures = {
        "rpu.make_tensor", "rpu.local_tile", "ctx.tile_frame", "ctx.load",
        "tile.add", "ctx.store"};
  }
...
RPUPlan makeGemmPlan(triton::FuncOp func, const GemmOperands &operands) {
  ...
  plan.mask = llvm::json::Object{{"masked", false}};
  plan.requiredDslFeatures = {
      "rpu.Array", "ctx.load", "ctx.zeros", "ctx.mma", "ctx.store"};
  plan.emission = llvm::json::Object{
      {"kind", "gemm"},
      {"m", operands.m},
      {"n", operands.n},
...
RPUPlan makeConvKxKPlan(triton::FuncOp func,
                        const ConvKxKOperands &operands) {
  RPUPlan plan;
  plan.kernelName = func.getSymName().str();
...
RPUPlan makeResNet50BottleneckPlan(
    triton::FuncOp func, const ResNet50BottleneckOperands &operands) {
  RPUPlan plan;
  plan.kernelName = func.getSymName().str();
...
public:
  ...
  void runOnOperation() final {
    ...
    attempts.push_back(
        failedAttempt(kAddPattern, kAddFailureReason,
    ...
    attempts.push_back(
        failedAttempt(kGemmPattern, kGemmFailureReason,
    ...
    attempts.push_back(
        failedAttempt(kReduceSumAllPattern, kReduceSumAllFailureReason,
    ...
    attempts.push_back(failedAttempt(kResNet50BottleneckPattern,
                                     kResNet50BottleneckFailureReason,
                                     resNet50BottleneckFailureAnchor(
                                         *maybeFunc)));
