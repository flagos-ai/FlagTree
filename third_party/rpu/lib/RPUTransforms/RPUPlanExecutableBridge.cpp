...
getStringAttrValue(DictionaryAttr dict, llvm::StringRef name) {
  if (!dict)
    return std::nullopt;
...
static void buildGemmExecutableIntoModule(PatternRewriter &rewriter,
  ...
  auto a = rewriter.create<exec::LoadMatrixOp>(loc, tileMK, lhs, shape.m,
                                               shape.k);
  auto b = rewriter.create<exec::LoadMatrixOp>(loc, tileKN, rhs, shape.k,
                                               shape.n);
...
static void buildConvKxKExecutableIntoModule(PatternRewriter &rewriter,
  ...
  for (int64_t ky = 0; ky < kernelSize; ++ky) {
    for (int64_t kx = 0; kx < kernelSize; ++kx) {
      ...
      acc = rewriter.create<exec::MmaOp>(loc, tile16x16, x.getResult(),
                                         w.getResult(), acc)
                .getResult();
    }
  }
...
static void buildResNetBlock16ExecutableIntoModule(
    PatternRewriter &rewriter, ModuleOp executableModule,
    llvm::StringRef kernelName, int64_t hidden) {
  MLIRContext *context = executableModule.getContext();
  loadExecutableDialects(*context);
  Location loc = UnknownLoc::get(context);
  Type f16 = rewriter.getF16Type();
  ...
  if (hidden == 32) {
    ...
    Value conv2Acc =
        rewriter.create<exec::ZeroOp>(loc, tile16x16, 16, 16).getResult();
    conv2Acc = rewriter.create<exec::MmaOp>(
                   loc, tile16x16, relu1Lo.getResult(), w2Lo.getResult(),
                   conv2Acc)
                   .getResult();
    auto w2Hi =
        rewriter.create<exec::LoadMatrixOp>(loc, tile16x16, w2Ptr, 16, 16);
    ...
    conv2Acc = rewriter.create<exec::MmaOp>(
                   loc, tile16x16, relu1Hi.getResult(), w2Hi.getResult(),
                   conv2Acc)
                   .getResult();

    auto residual = rewriter.create<exec::AddOp>(
        loc, tile16x16, conv2Acc, x.getResult());
  ...
  auto w1 =
      rewriter.create<exec::LoadMatrixOp>(loc, tile16xHidden, w1Ptr, 16, hidden);
  auto conv1Zero =
      rewriter.create<exec::ZeroOp>(loc, tile16xHidden, 16, hidden);
  auto conv1 = rewriter.create<exec::MmaOp>(
      loc, tile16xHidden, x.getResult(), w1.getResult(),
      conv1Zero.getResult());
  ...
  auto w2 =
      rewriter.create<exec::LoadMatrixOp>(loc, tileHiddenx16, w2Ptr, hidden, 16);
  auto conv2Zero = rewriter.create<exec::ZeroOp>(loc, tile16x16, 16, 16);
  auto conv2 = rewriter.create<exec::MmaOp>(
      loc, tile16x16, relu1.getResult(), w2.getResult(),
      conv2Zero.getResult());
  auto residual = rewriter.create<exec::AddOp>(
...
static void buildResNet50Bottleneck16ExecutableIntoModule(
  ...
  if (bottleneck == 32) {
    ...
    for (int64_t ky = 0; ky < 3; ++ky) {
      for (int64_t kx = 0; kx < 3; ++kx) {
        ...
        conv2AccLo = rewriter.create<exec::MmaOp>(
                         loc, tile16x16, relu1Lo.getResult(),
    ...
    auto relu2Lo = rewriter.create<exec::MaxOp>(
        loc, tile16x16, conv2AccLo, zero2Lo.getResult());
    auto zero2Hi = rewriter.create<exec::ZeroOp>(loc, tile16x16, 16, 16);
    auto relu2Hi = rewriter.create<exec::MaxOp>(
        loc, tile16x16, conv2AccHi, zero2Hi.getResult());
    auto w3Lo =
    ...
    Value conv3 = rewriter.create<exec::MmaOp>(
                      loc, tile16x16, relu2Lo.getResult(), w3Lo.getResult(),
                      conv3Zero.getResult())
                      .getResult();
    auto w3Hi =
        rewriter.create<exec::LoadMatrixOp>(loc, tile16x16, w3Ptr, 16, 16);
    setMatrixOffsets(rewriter, w3Hi.getOperation(), 16, 0);
    conv3 = rewriter.create<exec::MmaOp>(
                loc, tile16x16, relu2Hi.getResult(), w3Hi.getResult(), conv3)
                .getResult();
    auto residual =
  ...
  for (int64_t ky = 0; ky < 3; ++ky) {
    for (int64_t kx = 0; kx < 3; ++kx) {
      ...
      auto conv1Zero =
          rewriter.create<exec::ZeroOp>(loc, tile16x16, 16, 16);
      auto conv1 = rewriter.create<exec::MmaOp>(
          loc, tile16x16, x.getResult(), w1.getResult(),
          conv1Zero.getResult());
  ...
  auto zero2 = rewriter.create<exec::ZeroOp>(loc, tile16x16, 16, 16);
  auto relu2 = rewriter.create<exec::MaxOp>(
      loc, tile16x16, conv2Acc, zero2.getResult());
  auto w3 = rewriter.create<exec::LoadMatrixOp>(loc, tile16x16, w3Ptr, 16, 16);
  auto conv3Zero = rewriter.create<exec::ZeroOp>(loc, tile16x16, 16, 16);
  auto conv3 = rewriter.create<exec::MmaOp>(
      loc, tile16x16, relu2.getResult(), w3.getResult(),
      conv3Zero.getResult());
  auto residual = rewriter.create<exec::AddOp>(
...
getAddContiguousExecutableSpec(plan::KernelOp op) {
  ...
  if (!view.isPattern("add") || !kind || *kind != "add" || !n ||
      !logicalN || !masked || !maskMasked || !out || *out != 0 || !lhs ||
      *lhs != 1 || !rhs || *rhs != 2 || !isPositiveMultipleOf16(*n))
    return std::nullopt;
...
getGemmExecutableSpec(plan::KernelOp op) {
  ...
  std::optional<int64_t> lhs = view.emissionInteger("lhs");
  std::optional<int64_t> rhs = view.emissionInteger("rhs");
  if (!view.isPattern("gemm") || !kind || *kind != "gemm" || !m || !n ||
      !k || !isPositiveMultipleOf16(*m) || !isPositiveMultipleOf16(*n) ||
      !isPositiveMultipleOf16(*k) || !out || *out != 0 || !lhs ||
      *lhs != 1 || !rhs || *rhs != 2)
    return std::nullopt;
...
getSoftmaxExecutableSpec(plan::KernelOp op) {
  ...
  if (!view.isPattern("softmax") || !kind || *kind != "softmax" || !n ||
      !isPositiveMultipleOf16(*n) || !out || *out != 0 || !input ||
      *input != 1)
    return std::nullopt;
...
getResNetBlock16ExecutableSpec(plan::KernelOp op) {
  ...
  if (!view.isPattern("resnet_block") || !kind ||
      *kind != "resnet_block" || !m || *m != 16 || !channels ||
      *channels != 16 || !hidden || (*hidden != 16 && *hidden != 32) ||
      !out || *out != 0 || !x || *x != 1 || !w1 || *w1 != 2 || !w2 ||
      *w2 != 3)
    return std::nullopt;
...
getResNet50Bottleneck16ExecutableSpec(plan::KernelOp op) {
  ...
  if (!view.isPattern("resnet50_bottleneck") || !kind ||
      *kind != "resnet50_bottleneck" || !kernelSize || *kernelSize != 3 ||
      !m || *m != 16 || !channels || *channels != 16 || !bottleneck ||
      (*bottleneck != 16 && *bottleneck != 32) || !inputWidth ||
      *inputWidth != 16 || !out || *out != 0 || !input || *input != 1 ||
      !w1 || *w1 != 2 || !w2 || *w2 != 3 || !w3 || *w3 != 4)
    return std::nullopt;
  return ResNet50BottleneckExecutableSpec{view.kernelName().str(),
                                          *bottleneck};
}
...
using BuildFn = LogicalResult (*)(plan::KernelOp op,
                                  PatternRewriter &rewriter,
                                  ModuleOp module,
...
static LogicalResult buildGemm(plan::KernelOp op, PatternRewriter &rewriter,
                               ...
                               const ExecutableBuildSpec &spec) {
  eraseModuleBodyExceptPlanRoot(module, op, rewriter);
  buildGemmExecutableIntoModule(
      rewriter, module, spec.kernelName,
      ExecutableGemmShape{spec.gemmShape.m, spec.gemmShape.n,
                          spec.gemmShape.k});
  rewriter.eraseOp(op);
  if (failed(verify(module)))
    return module.emitError(
...
static std::optional<ExecutableBuildSpec> matchSoftmax(plan::KernelOp op) {
  if (std::optional<SoftmaxExecutableSpec> softmax =
    ...
    ExecutableBuildSpec spec{ExecutableBuildKind::Softmax,
                             softmax->kernelName};
...
static std::optional<ExecutableBuildSpec> matchResNet50Bottleneck(
    plan::KernelOp op) {
...
static LogicalResult buildResNet50Bottleneck(
    plan::KernelOp op, PatternRewriter &rewriter, ModuleOp module,
    const ExecutableBuildSpec &spec) {
  eraseModuleBodyExceptPlanRoot(module, op, rewriter);
...
static constexpr ExecutableBuildEntry kExecutableBuildEntries[] = {
    ...
    {"resnet50_bottleneck", matchResNet50Bottleneck,
     buildResNet50Bottleneck},
};

static std::optional<std::pair<const ExecutableBuildEntry *,
                               ExecutableBuildSpec>>
...
static std::optional<llvm::StringRef>
getPlanEmissionKind(plan::KernelOp op) {
...
class RPUPlanExecutableBuildPattern
    : public OpRewritePattern<plan::KernelOp> {
public:
  ...
  LogicalResult matchAndRewrite(plan::KernelOp op,
    ...
    std::optional<std::pair<const ExecutableBuildEntry *,
                            ExecutableBuildSpec>>
