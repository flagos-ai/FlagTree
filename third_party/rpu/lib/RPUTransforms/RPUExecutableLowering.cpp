...
FailureOr<plan::KernelOp> getSinglePlanKernel(ModuleOp module) {
  ...
  if (plans.empty()) {
    module.emitError("RPU executable lowering requires one rpu_plan.kernel, found none");
    return failure();
  }
  if (plans.size() > 1) {
    module.emitError("RPU executable lowering requires one rpu_plan.kernel, found multiple");
    return failure();
  }
  return plans.front();
}
