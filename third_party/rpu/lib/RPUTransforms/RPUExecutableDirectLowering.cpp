...
static FailureOr<bool> lowerCanonicalElementwise16ValueMapToExecutableOps(
  ...
  FailureOr<exec::ExecutableKernelScaffold> scaffold =
      exec::buildExecutableF16PointerKernelScaffold(
          ...
          "direct Elementwise16 value-map lowering requires executable kernel scaffold");
  if (failed(scaffold))
    return failure();

  if (failed(materializeElementwise16ValueMapLoweringPlan(
          rewriter, loc, scaffold->kernel, plan,
          "direct Elementwise16 value-map lowering requires IR-owned body materialization")))
    return failure();
  ...
  if (failed(verify(module))) {
    module.emitError(
        "failed to verify direct Elementwise16 value-map executable RPU module");
    return failure();
  }
  return true;
}
...
static LogicalResult lowerMatchedElementwise1DToExecutable(
  ...
  if (!isPositiveMultipleOf16(request.n) || request.n > 256)
    return module.emitError(
        "RPU executable Elementwise1D direct lowering requires 0 < n <= 256 and 16-aligned n");
  if (request.masked) {
    if (request.logicalN <= 0 || request.logicalN > request.n)
      return module.emitError(
          "RPU executable Elementwise1D direct lowering requires 0 < logical_n <= n");
  ...
  for (size_t i = 0, e = request.inputArgIndices.size(); i < e; ++i) {
    if (request.inputArgIndices[i] != static_cast<unsigned>(i + 1))
      return module.emitError(
          "RPU executable Elementwise1D direct lowering requires ordered input args");
  }
  ...
  if (failed(exec::buildExecutableCompactElementwise1DBody(
          ...
          "direct Elementwise1D lowering requires IR-owned compact elementwise materialization")))
    return failure();
...
static LogicalResult lowerMatchedGemmToExecutable(
    PatternRewriter &rewriter, ModuleOp module, triton::FuncOp func,
...
static exec::ExecutableSoftmaxBodyBuildSpec buildExecutableSoftmaxBodySpec(
...
static LogicalResult lowerMatchedSoftmaxToExecutable(
  ...
  if (failed(exec::buildExecutableSoftmaxBody(
          rewriter, loc, kernel, body,
          "direct Softmax lowering requires IR-owned Softmax body materialization")))
    return failure();
...
static LogicalResult lowerMatchedSqrtToExecutable(
    PatternRewriter &rewriter, ModuleOp module, triton::FuncOp func,
...
static LogicalResult lowerMatchedReduceSumAllToExecutable(
  ...
  FailureOr<exec::ExecutableKernelScaffold> scaffold =
      exec::buildExecutableF16PointerKernelScaffold(
          ...
          /*numArgs=*/2,
          "direct ReduceSumAll lowering requires IR-owned executable kernel scaffold");
  if (failed(scaffold))
    return failure();
  exec::KernelOp kernel = scaffold->kernel;
  ...
  if (failed(exec::buildExecutableReduceSumAllBody(
          rewriter, loc, kernel, body,
          "direct ReduceSumAll lowering requires IR-owned ReduceSumAll body materialization")))
    return failure();
...
static LogicalResult lowerMatchedReluToExecutable(
    PatternRewriter &rewriter, ModuleOp module, triton::FuncOp func,
...
validateExecutableMaximumOperands(ModuleOp module,
  ...
  if (!isPositiveMultipleOf16(source.n))
    return module.emitError(
        "RPU executable Maximum direct lowering requires positive 16-aligned n");
  return success();
}
...
static LogicalResult lowerMatchedMaximumToExecutable(
  ...
  FailureOr<exec::ExecutableKernelScaffold> scaffold =
      exec::buildExecutableF16PointerKernelScaffold(
          ...
          "direct Maximum lowering requires IR-owned executable kernel scaffold");
  if (failed(scaffold))
    return failure();
  exec::KernelOp kernel = scaffold->kernel;
  ...
  if (failed(exec::buildExecutableMaximumBody(
          rewriter, loc, kernel, body,
          "direct Maximum lowering requires IR-owned Maximum body materialization")))
    return failure();
...
static LogicalResult lowerMatchedReduceSumAxisToExecutable(
  ...
  FailureOr<exec::ExecutableKernelScaffold> scaffold =
      exec::buildExecutableF16PointerKernelScaffold(
          ...
          "direct ReduceSumAxis lowering requires IR-owned executable kernel scaffold");
  if (failed(scaffold))
    return failure();
  exec::KernelOp kernel = scaffold->kernel;
  ...
  if (failed(exec::buildExecutableReduceSumAxisBody(
          rewriter, loc, kernel, body,
          "direct ReduceSumAxis lowering requires IR-owned body materialization")))
    return failure();
...
validateExecutableBroadcastAddOperands(ModuleOp module,
  ...
  if (source.out != 0 || source.lhs != 1 || source.rhs != 2)
    return module.emitError(
        "RPU executable BroadcastAdd direct lowering requires out=0 lhs=1 rhs=2");
  if (source.rows <= 0 || source.cols <= 0 || source.rows % 16 != 0 ||
      source.cols % 16 != 0)
    return module.emitError(
...
static LogicalResult lowerMatchedBroadcastAddToExecutable(
  ...
  FailureOr<exec::ExecutableKernelScaffold> scaffold =
      exec::buildExecutableF16PointerKernelScaffold(
          ...
          /*numArgs=*/3,
          "direct BroadcastAdd lowering requires IR-owned executable kernel scaffold");
  if (failed(scaffold))
    return failure();
  exec::KernelOp kernel = scaffold->kernel;
  ...
  if (failed(exec::buildExecutableBroadcastAddBody(
          rewriter, loc, kernel, body,
          "direct BroadcastAdd lowering requires IR-owned body materialization")))
    return failure();
...
static exec::ExecutableConvKxKBodyBuildSpec buildExecutableConvKxKBodySpec(
...
validateExecutableConvKxKOperands(ModuleOp module,
  ...
  if (source.m != 16 || source.inChannels != 16 ||
      source.outChannels != 16 || source.inputWidth != 16)
    return module.emitError(
...
static LogicalResult lowerMatchedConvKxKToExecutable(
  ...
  if (failed(exec::buildExecutableConvKxKBody(
          rewriter, loc, kernel, body,
          "direct ConvKxK lowering requires IR-owned ConvKxK body materialization")))
    return failure();
...
static exec::ExecutableResNetBlockBodyBuildSpec buildExecutableResNetBlockBodySpec(
...
static LogicalResult
lowerMatchedResNetBlockToExecutable(PatternRewriter &rewriter, ModuleOp module,
                                    triton::FuncOp func,
                                    const exec::ExecutableResNetBlockBodyBuildSpec &body) {
  if (body.outputArgIndex != 0 || body.xArgIndex != 1 ||
      body.w1ArgIndex != 2 || body.w2ArgIndex != 3)
    return module.emitError(
        "RPU executable residual direct lowering requires out=0 x=1 w1=2 w2=3");
  ...
  if (failed(exec::buildExecutableResNetBlockBody(
          rewriter, loc, kernel, body,
          "direct residual lowering requires IR-owned residual body materialization")))
    return failure();
...
static LogicalResult lowerMatchedResNet50BottleneckToExecutable(
  ...
  if (body.bottleneck != 16 && body.bottleneck != 32)
    return module.emitError(
        "RPU executable bottleneck direct lowering requires bottleneck in {16,32}");
  ...
  if (failed(exec::buildExecutableResNet50BottleneckBody(
          rewriter, loc, kernel, body,
          "direct bottleneck lowering requires IR-owned bottleneck body materialization")))
    return failure();
...
LogicalResult emitUnsupportedDirectExecutablePattern(ModuleOp module) {
  ...
  os << "; ReduceSumAll lowering: C++ recognizer " << kReduceSumAllFailureReason;
...
public:
  ...
  LogicalResult matchAndRewrite(triton::FuncOp func,
    ...
    FailureOr<bool> loweredElementwise16 = lowerCanonicalElementwise16ValueMapToExecutableOps(
...
public:
  ...
  LogicalResult matchAndRewrite(triton::FuncOp func,
    ...
    return lowerMatchedElementwise1DToExecutable(rewriter, module, func, *request);
  }
};
...
public:
  ...
  LogicalResult matchAndRewrite(triton::FuncOp func,
    ...
    exec::ExecutableAddBodyBuildSpec body = buildExecutableAddBodySpec(*operands);
    StringRef executableKind = getExecutableAddKind(*operands);
    return lowerMatchedAddToExecutable(rewriter, module, func, body, executableKind);
  }
};
...
public:
  ...
  LogicalResult matchAndRewrite(triton::FuncOp func,
    ...
    exec::ExecutableGemmBodyBuildSpec body = buildExecutableGemmBodySpec(*operands);
    return lowerMatchedGemmToExecutable(rewriter, module, func, body);
  }
};

class SoftmaxToRPUExecutablePattern
    : public OpRewritePattern<triton::FuncOp> {
public:
  using OpRewritePattern<triton::FuncOp>::OpRewritePattern;
  ...
  LogicalResult matchAndRewrite(triton::FuncOp func,
    ...
    if (failed(validateExecutableSoftmaxOperands(module, *operands)))
      return failure();
    exec::ExecutableSoftmaxBodyBuildSpec body = buildExecutableSoftmaxBodySpec(*operands);
...
class ConvKxKToRPUExecutablePattern
    : public OpRewritePattern<triton::FuncOp> {
public:
  using OpRewritePattern<triton::FuncOp>::OpRewritePattern;
  ...
  LogicalResult matchAndRewrite(triton::FuncOp func,
    ...
    if (failed(validateExecutableConvKxKOperands(module, *operands)))
      return failure();
    exec::ExecutableConvKxKBodyBuildSpec body = buildExecutableConvKxKBodySpec(*operands);
...
public:
  ...
  LogicalResult matchAndRewrite(triton::FuncOp func,
    ...
    if (failed(validateExecutableResNetBlockOperands(module, *operands)))
      return failure();
    exec::ExecutableResNetBlockBodyBuildSpec body = buildExecutableResNetBlockBodySpec(*operands);
...
public:
  ...
  LogicalResult matchAndRewrite(triton::FuncOp func,
    ...
    if (failed(validateExecutableResNet50BottleneckOperands(module, *operands)))
      return failure();
    exec::ExecutableResNet50BottleneckBodyBuildSpec body = buildExecutableResNet50BottleneckBodySpec(*operands);
    return lowerMatchedResNet50BottleneckToExecutable(rewriter, module, func, body);
  }
};
...
void populateSupportedTTIRToRPUExecutablePatterns(RewritePatternSet &patterns) {
  ...
  patterns.add<ResNetBlockToRPUExecutablePattern>(patterns.getContext(),
                                                  PatternBenefit(1));
  patterns.add<ResNet50BottleneckToRPUExecutablePattern>(
