...
void appendAllFailedAttempts(SmallVectorImpl<TraceAttempt> &attempts,
  ...
  attempts.push_back(
      failedAttempt(kResNet50BottleneckPattern, reason, anchor));
}
...
bool isAllowedSoftmaxTopLevelOp(Operation &op) {
  return isa<triton::MakeRangeOp, triton::LoadOp, triton::ReduceOp,
             ...
             triton::ReturnOp>(&op) ||
         isa<math::ExpOp>(&op) ||
         op.getName().getStringRef() == "math.exp";
}
...
bool isAllowedReduceSumAxisTopLevelOp(Operation &op) {
  return isa<triton::MakeRangeOp, triton::ExpandDimsOp, triton::BroadcastOp,
             triton::LoadOp, triton::ReduceOp, triton::StoreOp,
             triton::SplatOp, triton::AddPtrOp, triton::ReturnOp,
             arith::ConstantOp, arith::MulIOp, arith::AddIOp,
             arith::TruncFOp>(&op);
}
...
TraceAnchor rankedFailureAnchor(
    triton::FuncOp func, llvm::function_ref<bool(Operation &)> isAllowed,
...
TraceAnchor addFailureAnchor(triton::FuncOp func) {
  return rankedFailureAnchor(
      func, isAllowedAddTopLevelOp, [](Operation &op) -> int {
...
TraceAnchor gemmFailureAnchor(triton::FuncOp func) {
  return rankedFailureAnchor(
      func, isAllowedGemmTopLevelOp, [](Operation &op) -> int {
...
TraceAnchor sqrtFailureAnchor(triton::FuncOp func) {
  return rankedFailureAnchor(
      func, isAllowedSqrtTopLevelOp, [](Operation &op) -> int {
        if (isa<triton::StoreOp>(&op))
          return 0;
...
TraceAnchor reduceSumAllFailureAnchor(triton::FuncOp func) {
  return rankedFailureAnchor(
      func, isAllowedReduceSumAllTopLevelOp, [](Operation &op) -> int {
        if (isa<triton::StoreOp>(&op))
          return 0;
        if (isa<triton::ReduceOp>(&op))
          return 1;
...
TraceAnchor reduceSumAxisFailureAnchor(triton::FuncOp func) {
  return rankedFailureAnchor(
      func, isAllowedReduceSumAxisTopLevelOp, [](Operation &op) -> int {
        if (isa<triton::StoreOp>(&op))
          return 0;
        if (isa<triton::ReduceOp>(&op))
          return 1;
...
TraceAnchor broadcastAddFailureAnchor(triton::FuncOp func) {
  return rankedFailureAnchor(
      func, isAllowedBroadcastAddTopLevelOp, [](Operation &op) -> int {
        if (isa<triton::StoreOp>(&op))
          return 0;
        if (isa<arith::AddFOp>(&op))
          return 1;
...
TraceAnchor convKxKFailureAnchor(triton::FuncOp func) {
  return rankedFailureAnchor(
      func, isAllowedNNTopLevelOp, [](Operation &op) -> int {
        if (isa<triton::StoreOp>(&op))
          return 0;
        if (isa<triton::DotOp>(&op))
          return 1;
...
bool isRankedTensorOf(Value value, Type elementType, ArrayRef<int64_t> shape) {
  auto type = dyn_cast<RankedTensorType>(value.getType());
  return type && type.getShape() == shape && type.getElementType() == elementType;
}
...
bool isMakeRange(Value value, int64_t nElements) {
  ...
  return range && range.getStartAttr().getInt() == 0 &&
         ...
         isRankedVectorOf(range.getResult(),
                          IntegerType::get(value.getContext(), 32),
                          nElements);
}
...
std::optional<LoadRole> matchRowMajorF16PointerAccess(Value pointer,
                                                      Block &entry,
                                                      int64_t rows,
                                                      int64_t cols,
                                                      int64_t rowStride) {
  ...
  auto colBroadcast =
      finalAdd.getOffset().getDefiningOp<triton::BroadcastOp>();
...
getLoadedF16MatrixRole(Value value, Block &entry, int64_t rowStride) {
  ...
  return matchRowMajorF16PointerAccess(load.getPtr(), entry,
...
std::optional<Value> getReluResult(Value source, ArrayRef<int64_t> shape) {
  ...
  for (Operation *user : source.getUsers()) {
    ...
    if (!isRankedTensorOf(result, Float32Type::get(source.getContext()),
                          shape))
      return std::nullopt;
...
FailureOr<AddOperands> recognizeAdd(triton::FuncOp func) {
  ...
  if (lhs == memory->loadToArg.end() ||
      rhs == memory->loadToArg.end() || out == memory->ptrToArg.end())
    return failure();
...
recognizeElementwise1DValueGraph(triton::FuncOp func) {
  ...
  // 256 = one full V256 (the hardware vector width), the largest contig load_contig<1>
  // on real hardware via the board NVEC fallback (see RPU_BOARD_LOAD_CONTIG_NVEC
  // gate in RPUExecutableEmitter::emitLoadContig). The default legacy path
  // emits load_contig<n> with n in element count, so 256 still fits in that
  // path's tile shape.
  ...
  for (Operation *op : binaryOps) {
    ...
    valueToSlot[resultValue] = graph.inputArgIndices.size() + graph.ops.size() - 1;
...
FailureOr<GemmOperands> recognizeGemm(triton::FuncOp func) {
  ...
  for (Operation &op : entry.getOperations()) {
    ...
    if (auto load = dyn_cast<triton::LoadOp>(&op))
      ...
      stores.push_back(store);
    else if (isa<triton::MakeRangeOp, triton::ExpandDimsOp,
                 triton::SplatOp, triton::AddPtrOp, triton::BroadcastOp,
                 triton::ReturnOp, arith::ConstantOp, arith::MulIOp,
                 arith::TruncFOp>(&op))
      continue;
    else
      return failure();
...
FailureOr<SoftmaxOperands> recognizeSoftmax(triton::FuncOp func) {
  ...
  if (ranges.size() != 1 || loads.size() != 1 || reductions.size() != 2 ||
      exts.size() != 1 ||
      subs.size() != 1 || exps.size() != 1 || truncs.size() != 1 ||
      stores.size() != 1)
    return failure();
  ...
  if (div.getLhs() != exp->getResult(0) ||
      div.getRhs() != sumSplat.getResult())
    return failure();
...
FailureOr<ConvKxKOperands> recognizeConvKxK(triton::FuncOp func) {
  ...
  if (!firstInput || !firstWeight ||
      firstInput->rows != m || firstInput->cols != inChannels ||
      firstInput->rowBase != 0 || firstWeight->rows != inChannels ||
      firstWeight->cols != outChannels || firstWeight->rowBase != 0)
    return failure();
  ...
  for (auto [dotIndex, dot] : llvm::enumerate(dots)) {
    ...
    if (!input || !weight ||
        input->arg != firstInput->arg || input->rows != m ||
        input->cols != inChannels ||
  ...
  if (!trunc || trunc.getIn() != *acc ||
      !isRankedTensorOf(trunc.getResult(),
                        Float16Type::get(func.getContext()),
                        {m, outChannels}))
    return failure();

  std::optional<LoadRole> outAccess =
      matchRowMajorF16PointerAccess(store.getPtr(), entry, m, outChannels,
                                    outChannels);
...
FailureOr<ResNetBlockOperands> recognizeResNetBlock(triton::FuncOp func) {
  ...
  if (dot2Shape->m != m || dot2Shape->k != hidden ||
      dot2Shape->n != channels)
    return failure();
  ...
  if (store.getMask() || store.getValue() != *storeValue)
    return failure();
  std::optional<LoadRole> outAccess =
      matchRowMajorF16PointerAccess(store.getPtr(), entry, m, channels,
                                    channels);
  if (!outAccess || outAccess->rowBase != 0)
    return failure();
...
recognizeResNet50Bottleneck(triton::FuncOp func) {
  ...
  for (int64_t pairIndex = 0; pairIndex < kernelArea; ++pairIndex) {
    ...
    if (!input || !conv1W || !conv2W ||
        input->arg != firstInput->arg || input->rows != m ||
        input->cols != channels ||
        conv1W->arg != w1->arg || conv1W->rows != channels ||
        conv1W->cols != bottleneck || conv1W->rowBase != 0 ||
        conv2W->rows != bottleneck || conv2W->cols != bottleneck ||
        conv2W->rowBase != pairIndex * bottleneck)
      return failure();
    inputRowBases.push_back(input->rowBase);
    if (pairIndex == 0) {
      skipInput = input;
  ...
  for (int64_t pairIndex = 0; pairIndex < kernelArea; ++pairIndex) {
    std::optional<LoadRole> conv2W =
        getLoadedF16MatrixRole(dots[pairIndex * 2 + 1].getB(), entry,
                               bottleneck);
    if (!conv2W || conv2W->arg != w2->arg ||
        conv2W->rows != bottleneck || conv2W->cols != bottleneck ||
        conv2W->rowBase != pairIndex * bottleneck)
      return failure();
  }
  ...
  if (!finalShape || finalShape->m != m || finalShape->k != bottleneck ||
      finalShape->n != channels || finalDot.getA() != *relu2Fp16 ||
      !w3 || w3->rows != bottleneck || w3->cols != channels ||
      w3->rowBase != 0)
    return failure();
...
FailureOr<SqrtOperands> recognizeSqrt(triton::FuncOp func) {
  ...
  if (sqrt->getNumOperands() != 1 || sqrt->getNumResults() != 1 ||
      ...
      !isRankedVectorOf(sqrt->getResult(0),
                        Float32Type::get(func.getContext()), nElements))
    return failure();
  ...
  if (inputArg == memory->loadToArg.end() ||
      outArg == memory->ptrToArg.end())
    return failure();
...
FailureOr<ReduceSumAllOperands>
recognizeReduceSumAll(triton::FuncOp func) {
  FunctionType functionType = func.getFunctionType();
  if (functionType.getNumInputs() != 2 || functionType.getNumResults() != 0)
    return failure();
...
FailureOr<ReduceSumAxisOperands>
recognizeReduceSumAxis(triton::FuncOp func, int64_t axis) {
  if (axis != 0 && axis != 1)
    return failure();
  FunctionType functionType = func.getFunctionType();
  if (functionType.getNumInputs() != 2 || functionType.getNumResults() != 0)
...
FailureOr<BroadcastAddOperands> recognizeBroadcastAdd(triton::FuncOp func) {
  ...
  if (add.getLhs() != matLoad.getResult())
    return failure();
  if (!isRankedTensorOf(add.getResult(), f16,
  ...
  if (!expand)
    return failure();
  if (!isRankedTensorOf(expand.getResult(), f16,
                         ArrayRef<int64_t>{rows, 1}))
    return failure();
