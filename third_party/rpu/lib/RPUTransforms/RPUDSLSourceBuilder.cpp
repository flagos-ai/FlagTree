...
std::string buildRPUDSLProgram(llvm::StringRef kernelName,
                               llvm::StringRef args,
                               llvm::StringRef body) {
  std::string result;
...
std::string buildAddBody(int64_t n, int64_t logicalN, int64_t out,
                         int64_t lhs, int64_t rhs, bool masked) {
  ...
  if (masked) {
    os << llvm::formatv(
        "    auto lhs_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{0}, rpu::make_shape(1, {1}), rpu::make_stride({2}, 1));\n"
        "    auto rhs_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{3}, rpu::make_shape(1, {1}), rpu::make_stride({2}, 1));\n"
        "    auto out_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{4}, rpu::make_shape(1, {1}), rpu::make_stride({2}, 1));\n"
        "    auto lhs = static_cast<rpu::Tile<half, 16, {2}>>(ctx.load<half, 16, {2}>(rpu::local_tile<16, {2}>(lhs_tensor, rpu::make_coord(0, 0))));\n"
        "    auto rhs = static_cast<rpu::Tile<half, 16, {2}>>(ctx.load<half, 16, {2}>(rpu::local_tile<16, {2}>(rhs_tensor, rpu::make_coord(0, 0))));\n"
        "    auto result = lhs + rhs;\n"
        "    ctx.store(rpu::local_tile<16, {2}>(out_tensor, rpu::make_coord(0, 0)), result);",
  ...
  os << llvm::formatv(
      "    auto lhs_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{0}, rpu::make_shape({1}, 16), rpu::make_stride(16, 1));\n"
      "    auto rhs_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{2}, rpu::make_shape({1}, 16), rpu::make_stride(16, 1));\n"
      "    auto out_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{3}, rpu::make_shape({1}, 16), rpu::make_stride(16, 1));\n",
  ...
  for (int64_t rowOffset = 0; rowOffset < totalRows;
    ...
    os << llvm::formatv(
        "        auto lhs = static_cast<rpu::Tile<half, {0}, 16>>(ctx.load<half, {0}, 16>(rpu::local_tile<{0}, 16>(lhs_tensor, rpu::make_coord({1}, 0))));\n"
        "        auto rhs = static_cast<rpu::Tile<half, {0}, 16>>(ctx.load<half, {0}, 16>(rpu::local_tile<{0}, 16>(rhs_tensor, rpu::make_coord({1}, 0))));\n"
        "        auto result = lhs + rhs;\n"
        "        ctx.store(rpu::local_tile<{0}, 16>(out_tensor, rpu::make_coord({1}, 0)), result);\n",
        chunkRows, rowOffset);
    os << "    }\n";
  }
  return os.str();
...
std::string buildSoftmaxBody(int64_t n, int64_t input, int64_t out) {
  int64_t nvec = n / 16;
  return llvm::formatv(
             "    auto x = ctx.load_contig<{0}>(arg{1});\n"
...
std::string buildConvKxKBody(int64_t kernelSize, int64_t m,
                             int64_t inChannels, int64_t outChannels,
                             int64_t inputWidth, int64_t input,
                             int64_t weight, int64_t out) {
  int64_t inputRows =
      (kernelSize - 1) * inputWidth + (kernelSize - 1) + m;
  ...
  os << "    rpu::Array<half, 2> x_arr{arg" << input << ", " << inputRows
     << ", " << inChannels << "};\n"
     << "    rpu::Array<half, 2> w_arr{arg" << weight << ", "
     << weightRows << ", " << outChannels << "};\n"
  ...
  for (int64_t ky = 0; ky < kernelSize; ++ky) {
    for (int64_t kx = 0; kx < kernelSize; ++kx) {
      ...
      os << "    auto x_" << ky << "_" << kx << " = ctx.load<half, " << m
         ...
         << ">(w_arr, rpu::IndexList{" << wRow << ", 0});\n"
         << "    ctx.mma<" << m << ", " << inChannels << ", "
         << outChannels << ">(x_" << ky << "_" << kx << ", w_" << ky
         << "_" << kx << ", acc);\n";
    }
  }
...
std::string buildResNetBlockBody(int64_t m, int64_t channels, int64_t hidden,
  ...
  os
     << "    auto w1 = ctx.load<half, " << channels << ", " << hidden
...
std::string buildResNet50BottleneckBody(int64_t kernelSize, int64_t m,
                                        ...
                                        int64_t input, int64_t w1,
                                        int64_t w2, int64_t w3) {
  int64_t inputRows =
      (kernelSize - 1) * inputWidth + (kernelSize - 1) + m;
  ...
  if (bottleneck == 32) {
    ...
    for (int64_t ky = 0; ky < kernelSize; ++ky) {
      for (int64_t kx = 0; kx < kernelSize; ++kx) {
        ...
        os << "    auto x_" << ky << "_" << kx << " = ctx.load<half, "
           << m << ", " << channels << ">(x_arr, rpu::IndexList{" << xRow
           << ", 0});\n"
           << "    auto conv1_" << ky << "_" << kx
           << "_0 = ctx.zeros<half, " << m << ", 16>();\n"
           << "    ctx.mma<" << m << ", " << channels << ", 16>(x_" << ky
           << "_" << kx << ", w1_0, conv1_" << ky << "_" << kx
           << "_0);\n"
           << "    auto zero1_" << ky << "_" << kx
           << "_0 = ctx.zeros<half, " << m << ", 16>();\n"
           << "    auto relu1_" << ky << "_" << kx
           << "_0 = rpu::max_binop(conv1_" << ky << "_" << kx
           << "_0, zero1_" << ky << "_" << kx << "_0);\n"
           << "    auto conv1_" << ky << "_" << kx
           << "_1 = ctx.zeros<half, " << m << ", 16>();\n"
           << "    ctx.mma<" << m << ", " << channels << ", 16>(x_" << ky
           << "_" << kx << ", w1_1, conv1_" << ky << "_" << kx
           << "_1);\n"
           << "    auto zero1_" << ky << "_" << kx
           << "_1 = ctx.zeros<half, " << m << ", 16>();\n"
           << "    auto relu1_" << ky << "_" << kx
           << "_1 = rpu::max_binop(conv1_" << ky << "_" << kx
           << "_1, zero1_" << ky << "_" << kx << "_1);\n"
           << "    auto w2_" << ky << "_" << kx
           << "_00 = ctx.load<half, 16, 16>(w2_arr, rpu::IndexList{"
           << w2Row << ", 0});\n"
           ...
           << "    auto w2_" << ky << "_" << kx
           << "_01 = ctx.load<half, 16, 16>(w2_arr, rpu::IndexList{"
           << w2Row << ", 16});\n"
           << "    ctx.mma<" << m << ", 16, 16>(relu1_" << ky << "_" << kx
  ...
  os
     << "    auto w1 = ctx.load<half, " << channels << ", " << bottleneck
     << ">(w1_arr, rpu::IndexList{0, 0});\n"
  ...
  for (int64_t ky = 0; ky < kernelSize; ++ky) {
    for (int64_t kx = 0; kx < kernelSize; ++kx) {
      ...
      os << "    auto x_" << ky << "_" << kx << " = ctx.load<half, " << m
         ...
         << ", 0});\n"
         << "    auto conv1_" << ky << "_" << kx
         << " = ctx.zeros<half, " << m << ", " << bottleneck << ">();\n"
         << "    ctx.mma<" << m << ", " << channels << ", " << bottleneck
         << ">(x_" << ky << "_" << kx << ", w1, conv1_" << ky << "_"
         << kx << ");\n"
         << "    auto zero1_" << ky << "_" << kx
         << " = ctx.zeros<half, " << m << ", " << bottleneck << ">();\n"
         << "    auto relu1_" << ky << "_" << kx
         << " = rpu::max_binop(conv1_" << ky << "_" << kx << ", zero1_"
         << ky << "_" << kx << ");\n"
         << "    auto w2_" << ky << "_" << kx << " = ctx.load<half, "
         << bottleneck << ", " << bottleneck
         << ">(w2_arr, rpu::IndexList{" << w2Row << ", 0});\n"
         << "    ctx.mma<" << m << ", " << bottleneck << ", "
         << bottleneck << ">(relu1_" << ky << "_" << kx << ", w2_" << ky
         << "_" << kx << ", conv2_acc);\n";
    }
  }
