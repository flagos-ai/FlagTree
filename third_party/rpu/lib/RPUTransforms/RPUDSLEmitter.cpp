...
std::vector<std::string> directRPUDSLSupportedPatterns() {
  return {"add",     "gemm",         "softmax",      "sqrt",
          "reduce_sum_all", "convkxk", "resnet_block", "resnet50_bottleneck"};
}
...
formatSignature(plan::KernelOp op, const RPUPlan &plan) {
...
static FailureOr<std::string> emitAddBody(plan::KernelOp op,
  ...
  FailureOr<int64_t> logicalN =
      getIntegerField(op, plan.emission, "logical_n");
...
static FailureOr<std::string> emitResNetBlockBody(plan::KernelOp op,
                                                  const RPUPlan &plan) {
  FailureOr<int64_t> m = getIntegerField(op, plan.emission, "m");
  FailureOr<int64_t> channels =
      getIntegerField(op, plan.emission, "channels");
...
static FailureOr<std::string>
emitResNet50BottleneckBody(plan::KernelOp op, const RPUPlan &plan) {
  FailureOr<int64_t> kernelSize =
      getIntegerField(op, plan.emission, "kernel_size");
  FailureOr<int64_t> m = getIntegerField(op, plan.emission, "m");
  FailureOr<int64_t> channels =
...
getRPUPlanKernelSummaryFromModule(ModuleOp module) {
  ...
  if (plans.size() > 1) {
    module.emitError(
        "RPU rpu_plan.kernel summary requires one rpu_plan.kernel, found multiple");
    return failure();
  }
...
FailureOr<RPUDSLEmissionResult> emitRPUDSLFromKernelOp(plan::KernelOp op) {
  ...
  if (!converted) {
    op.emitError("failed to convert rpu_plan.kernel to RPUPlan DTO for .rc emission");
    return failure();
  }
  ...
  if (plan.pattern == "add") {
    ...
    if (failed(body))
      return failure();
    return RPUDSLEmissionResult{plan.kernelName, plan.pattern,
                                buildRPUDSLProgram(plan.kernelName, *args,
                                                   *body)};
  }
...
FailureOr<RPUDSLEmissionResult> emitRPUDSLFromModule(ModuleOp module) {
  ...
  module.walk([&](plan::KernelOp op) { plans.push_back(op); });
  if (plans.empty()) {
    module.emitError("RPU direct .rc emission requires one rpu_plan.kernel, found none");
    return failure();
  }
  if (plans.size() > 1) {
    module.emitError("RPU direct .rc emission requires one rpu_plan.kernel, found multiple");
    return failure();
  }
