...
static bool isTile16x16F16(Type type) {
  return isTileMatrixF16(type, 16, 16);
}
...
static int64_t getOptionalI32AttrOrZero(Operation *op,
                                        llvm::StringRef name) {
...
public:
  ExecutableOpSequenceRenderState(
      ...
      : kernel_(kernel), os_(os), bindings_(bindings),
        diagnostic_(diagnostic), valueNameHints_(valueNameHints) {}
...
private:
  ...
  std::optional<exec::ExecutableVectorMaskInfo>
  valueMask(Value value) const {
  ...
  void bind(Value value, llvm::StringRef name,
            std::optional<exec::ExecutableVectorMaskInfo> mask =
                std::nullopt) {
  ...
  requireMaskedTensorName(const ExecutableMemoryBinding &binding,
    ...
    nameOs.flush();
    os_ << "    auto " << name
        << " = rpu::make_tensor<half, 2, rpu::MemScope::Local>("
        << binding.name << ", rpu::make_shape(1, " << mask.logicalN
        << "), rpu::make_stride(" << mask.blockN << ", 1));\n";
  ...
  LogicalResult emitLoadContig(exec::LoadContigOp op) {
    ...
    FailureOr<exec::ExecutableVectorMaskInfo> maskInfo =
        exec::getExecutableVectorMaskInfo(
            op.getOperation(), op.getN(),
    ...
    if (maskInfo->masked) {
      ...
      if (failed(tensorName))
        return failure();
      os_ << "    auto " << resultName
          << " = static_cast<rpu::Tile<half, 16, " << maskInfo->blockN
          << ">>(ctx.load<half, 16, " << maskInfo->blockN
          << ">(rpu::local_tile<16, " << maskInfo->blockN << ">("
          << *tensorName << ", rpu::make_coord(0, 0))));\n";
  ...
  LogicalResult emitLoadMatrix(exec::LoadMatrixOp op) {
    ...
    if ((*binding)->physicalB) {
      ...
      os_ << "    auto " << storageName << " = ctx.load<half, "
          << op.getCols() << ", " << op.getRows() << ">(" << (*binding)->name
  ...
  LogicalResult emitStoreMatrix(exec::StoreMatrixOp op) {
    ...
    os_ << "    ctx.store<half, " << op.getRows() << ", " << op.getCols()
        << ">(" << (*binding)->name << ", rpu::IndexList{" << row << ", "
        << col << "}, " << *valueName << ");";
    return success();
  }
  ...
  LogicalResult emitZero(exec::ZeroOp op) {
    std::string resultName = nextTileName(op.getResult());
    os_ << "    auto " << resultName << " = ctx.zeros<half, "
        << op.getRows() << ", " << op.getCols() << ">();\n";
    bind(op.getResult(), resultName);
    return success();
  }
  ...
  LogicalResult emitBinaryElementwise(Value lhs, Value rhs, Value result,
    ...
    if (failed(lhsName) || failed(rhsName))
      return failure();
    std::optional<exec::ExecutableVectorMaskInfo> lhsMask =
        valueMask(lhs);
    std::optional<exec::ExecutableVectorMaskInfo> rhsMask =
        valueMask(rhs);
  ...
  LogicalResult emitReduceMaxAll(exec::ReduceMaxAllOp op) {
    ...
    os_ << "    auto " << resultName << " = ctx.reduce_max_all("
        << *inputName << ");\n";
    bind(op.getResult(), resultName);
    return success();
  }
  ...
  LogicalResult emitReduceSumAll(exec::ReduceSumAllOp op) {
    ...
    if (failed(inputName))
      return failure();
    std::string resultName = nextScalarName(op.getResult());
    os_ << "    auto " << resultName << " = ctx.reduce_sum_all("
        << *inputName << ");\n";
    bind(op.getResult(), resultName);
    return success();
  }
...
static FailureOr<std::string> emitGenericExecutableOpSequenceBody(
    ...
    ArrayRef<ExecutableValueNameHint> valueNameHints = {}) {
  if (failed(exec::verifyGenericRenderableExecutableOpSequence(
          kernel, ops, diagnostic)))
    return failure();
  std::string body;
...
static FailureOr<std::string>
emitGenericSimpleAddContigOpSequenceBody(exec::KernelOp kernel,
                                         ArrayRef<Operation *> ops,
                                         exec::ExecutableAddOpSequenceInfo info,
                                         unsigned outArg, unsigned lhsArg,
                                         unsigned rhsArg) {
...
static FailureOr<std::string>
emitAddOpSequenceBody(exec::KernelOp kernel, ArrayRef<Operation *> ops) {
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          ...
          "executable add op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
  ...
  if (masked) {
    os << llvm::formatv(
        "    auto arg{0}_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{0}, rpu::make_shape(1, {1}), rpu::make_stride({2}, 1));\n"
        "    auto arg{3}_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{3}, rpu::make_shape(1, {1}), rpu::make_stride({2}, 1));\n"
        "    auto arg{4}_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{4}, rpu::make_shape(1, {1}), rpu::make_stride({2}, 1));\n"
        "    auto tile0 = static_cast<rpu::Tile<half, 16, {2}>>(ctx.load<half, 16, {2}>(rpu::local_tile<16, {2}>(arg{0}_tensor, rpu::make_coord(0, 0))));\n"
        "    auto tile1 = static_cast<rpu::Tile<half, 16, {2}>>(ctx.load<half, 16, {2}>(rpu::local_tile<16, {2}>(arg{3}_tensor, rpu::make_coord(0, 0))));\n"
        "    auto tile2 = tile0 + tile1;\n"
        "    ctx.store(rpu::local_tile<16, {2}>(arg{4}_tensor, rpu::make_coord(0, 0)), tile2);",
  ...
  os << llvm::formatv(
      "    auto arg{0}_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{0}, rpu::make_shape({1}, 16), rpu::make_stride(16, 1));\n"
      "    auto arg{2}_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{2}, rpu::make_shape({1}, 16), rpu::make_stride(16, 1));\n"
      "    auto arg{3}_tensor = rpu::make_tensor<half, 2, rpu::MemScope::Local>(arg{3}, rpu::make_shape({1}, 16), rpu::make_stride(16, 1));\n",
  ...
  for (int64_t rowOffset = 0; rowOffset < totalRows;
    ...
    int64_t chunkRows = std::min<int64_t>(kMaxRowsPerFrame,
                                          totalRows - rowOffset);
    os << "    {\n";
    os << "        auto frame = ctx.tile_frame();\n";
    os << llvm::formatv(
        "        auto tile0 = static_cast<rpu::Tile<half, {0}, 16>>(ctx.load<half, {0}, 16>(rpu::local_tile<{0}, 16>(arg{1}_tensor, rpu::make_coord({2}, 0))));\n"
        "        auto tile1 = static_cast<rpu::Tile<half, {0}, 16>>(ctx.load<half, {0}, 16>(rpu::local_tile<{0}, 16>(arg{3}_tensor, rpu::make_coord({2}, 0))));\n"
        "        auto tile2 = tile0 + tile1;\n"
        "        ctx.store(rpu::local_tile<{0}, 16>(arg{4}_tensor, rpu::make_coord({2}, 0)), tile2);\n",
...
emitAddFromExecutableContract(const exec::ExecutableKernelContractInfo &contract) {
  ...
  if (n <= 0 || info.rhsLoad.getN() != n || info.store.getN() != n ||
    ...
    kernel.emitError(
        "executable add contract-view emission requires positive !rpu.tile<Nxf16>");
    return failure();
  }
...
emitGemmOpSequenceBody(exec::KernelOp kernel, ArrayRef<Operation *> ops) {
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          kernel,
          {info.store.getPtr(), info.lhsLoad.getPtr(), info.rhsLoad.getPtr()},
          "executable gemm op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
  ...
  llvm::SmallVector<ExecutableMemoryBinding, 3> bindings;
  bindings.push_back(ExecutableMemoryBinding{
      info.lhsLoad.getPtr(), lhsArg, matrixArgName(lhsArg),
      MatrixTileShape{m, k}});
  bindings.push_back(ExecutableMemoryBinding{
      info.rhsLoad.getPtr(), rhsArg, matrixArgName(rhsArg),
      MatrixTileShape{n, k}, true});
  bindings.push_back(ExecutableMemoryBinding{
      info.store.getPtr(), outArg, matrixArgName(outArg),
      MatrixTileShape{m, n}});
...
static FailureOr<RPUExecutableEmissionResult>
emitGemmFromExecutableContract(const exec::ExecutableKernelContractInfo &contract) {
  exec::KernelOp kernel = contract.kernel;
  ArrayRef<Operation *> ops = contract.bodyOps;
  ...
  if (info.rhsLoad.getRows() != k || info.zero.getRows() != m ||
    ...
    kernel.emitError(
        "executable gemm contract-view emission requires canonical matrix tile shapes");
    return failure();
  }
...
emitSoftmaxOpSequenceBody(exec::KernelOp kernel, ArrayRef<Operation *> ops) {
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          ...
          "executable softmax op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
...
static FailureOr<RPUExecutableEmissionResult>
emitSoftmaxFromExecutableContract(
  ...
  FailureOr<exec::ExecutableSoftmaxOpSequenceInfo> maybeInfo =
      exec::getExecutableSoftmaxOpSequenceInfo(
          kernel, ops,
          "executable softmax contract-view emission requires verified IR info");
  if (failed(maybeInfo))
    return failure();
  exec::ExecutableSoftmaxOpSequenceInfo info = *maybeInfo;
  ...
  if (nvec <= 0 || info.store.getN() != nvec ||
    ...
    kernel.emitError(
        "executable softmax contract-view emission requires positive !rpu.tile<Nxf16>");
    return failure();
  }
...
emitConvKxKOpSequenceBody(exec::KernelOp kernel, ArrayRef<Operation *> ops) {
  ...
  int64_t windows = *kernelSize * *kernelSize;
  size_t zeroIndex =
      layout == ConvKxKPrimitiveBodyLayout::Canonical ? 0 : 2;
  size_t firstXIndex =
      layout == ConvKxKPrimitiveBodyLayout::Canonical ? 1 : 0;
  size_t firstWIndex =
      layout == ConvKxKPrimitiveBodyLayout::Canonical ? 2 : 1;
  size_t storeIndex =
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          ...
          "executable convkxk op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
  ...
  for (int64_t window = 0; window < windows; ++window) {
    size_t loadBase =
        layout == ConvKxKPrimitiveBodyLayout::Canonical
            ...
            : (window == 0
                   ? 0
                   : static_cast<size_t>(4 + (window - 1) * 3));
    auto loadX = cast<exec::LoadMatrixOp>(ops[loadBase]);
    auto loadW = cast<exec::LoadMatrixOp>(ops[loadBase + 1]);
    int64_t xRow =
        getOptionalI32AttrOrZero(loadX.getOperation(), "row_offset");
    int64_t wRow =
        getOptionalI32AttrOrZero(loadW.getOperation(), "row_offset");
  ...
  os << "    rpu::Array<half, 2> x_arr{arg" << inputArg << ", "
     << inputRows << ", " << k << "};\n";
  os << "    rpu::Array<half, 2> w_arr{arg" << weightArg << ", "
     << weightRows << ", " << n << "};\n";
  ...
  llvm::SmallVector<ExecutableMemoryBinding, 3> bindings;
  bindings.push_back(
      ExecutableMemoryBinding{firstX.getPtr(), inputArg, "x_arr",
                              MatrixTileShape{inputRows, k}});
  bindings.push_back(
      ExecutableMemoryBinding{firstW.getPtr(), weightArg, "w_arr",
                              MatrixTileShape{weightRows, n}});
  bindings.push_back(
      ExecutableMemoryBinding{store.getPtr(), outArg, "out_arr",
                              MatrixTileShape{store.getRows(),
                                              store.getCols()}});
  ...
  return emitGenericExecutableOpSequenceBody(
      kernel, ops, bindings,
      "generic ConvKxK emission requires bound executable operands",
      prelude, valueNameHints);
}

static FailureOr<RPUExecutableEmissionResult>
emitConvKxKFromExecutableContract(
    const exec::ExecutableKernelContractInfo &contract) {
  exec::KernelOp kernel = contract.kernel;
  ArrayRef<Operation *> ops = contract.bodyOps;
  ...
  if (!kernelSize ||
    ...
    kernel.emitError(
        "executable convkxk contract-view emission requires supported convkxk op count");
    return failure();
  }
...
emitResNetBlockOpSequenceBody(exec::KernelOp kernel,
  ...
  auto store =
      cast<exec::StoreMatrixOp>(hidden32 ? ops[19] : ops[12]);

  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          kernel, {store.getPtr(), x.getPtr(), firstW1.getPtr(),
                   firstW2.getPtr()},
          "executable residual op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
  ...
  bindings.push_back(
      ExecutableMemoryBinding{x.getPtr(), xArg, "x_arr",
                              MatrixTileShape{xRows, xCols}});
  bindings.push_back(
      ExecutableMemoryBinding{firstW1.getPtr(), w1Arg, "w1_arr",
                              MatrixTileShape{w1Rows, w1Cols}});
  bindings.push_back(
      ExecutableMemoryBinding{firstW2.getPtr(), w2Arg, "w2_arr",
                              MatrixTileShape{w2Rows, w2Cols}});
  bindings.push_back(
      ExecutableMemoryBinding{store.getPtr(), outArg, "out_arr",
                              MatrixTileShape{outRows, outCols}});
  ...
  if (hidden32) {
    ...
    return emitGenericExecutableOpSequenceBody(
        kernel, ops, bindings,
        "generic Residual emission requires bound executable operands",
        prelude, valueNameHints);
  }
...
emitResNetBlock16FromExecutableContract(
  ...
  FailureOr<std::string> body =
      emitResNetBlockOpSequenceBody(kernel, ops);
  if (failed(body))
    return failure();
  std::string source =
      buildRPUDSLProgram(contract.kernelName, *prototype, *body);
...
emitResNet50BottleneckOpSequenceBody(exec::KernelOp kernel,
  ...
  auto firstW3 = cast<exec::LoadMatrixOp>(
      ops[bottleneck32 ? tail + 4 : tail + 2]);
  auto store =
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          kernel,
          {store.getPtr(), xSkip.getPtr(), firstW1.getPtr(),
           firstW2.getPtr(), firstW3.getPtr()},
          "executable bottleneck op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
  ...
  if (bottleneck32) {
    ...
    updateMatrixExtent(cast<exec::LoadMatrixOp>(ops[tail + 4]), w3Rows,
                       w3Cols);
    updateMatrixExtent(cast<exec::LoadMatrixOp>(ops[tail + 7]), w3Rows,
                       w3Cols);
  } else {
    updateMatrixExtent(cast<exec::LoadMatrixOp>(ops[1]), w1Rows, w1Cols);
    for (int64_t window = 0; window < 9; ++window) {
    ...
    updateMatrixExtent(cast<exec::LoadMatrixOp>(ops[tail + 2]), w3Rows,
                       w3Cols);
  }
  ...
  bindings.push_back(
      ExecutableMemoryBinding{xSkip.getPtr(), xArg, "x_arr",
                              MatrixTileShape{xRows, xCols}});
  bindings.push_back(
      ExecutableMemoryBinding{firstW1.getPtr(), w1Arg, "w1_arr",
                              MatrixTileShape{w1Rows, w1Cols}});
  bindings.push_back(
      ExecutableMemoryBinding{firstW2.getPtr(), w2Arg, "w2_arr",
                              MatrixTileShape{w2Rows, w2Cols}});
  bindings.push_back(
      ExecutableMemoryBinding{firstW3.getPtr(), w3Arg, "w3_arr",
  ...
  if (bottleneck32) {
    ...
    for (int64_t ky = 0; ky < 3; ++ky) {
      for (int64_t kx = 0; kx < 3; ++kx) {
        ...
        std::string conv10 =
            llvm::formatv("conv1_{0}_{1}_0", ky, kx).str();
        std::string zero10 =
            llvm::formatv("zero1_{0}_{1}_0", ky, kx).str();
        std::string relu10 =
            llvm::formatv("relu1_{0}_{1}_0", ky, kx).str();
        std::string conv11 =
            llvm::formatv("conv1_{0}_{1}_1", ky, kx).str();
        std::string zero11 =
            llvm::formatv("zero1_{0}_{1}_1", ky, kx).str();
        std::string relu11 =
            llvm::formatv("relu1_{0}_{1}_1", ky, kx).str();
        std::string w200 =
            llvm::formatv("w2_{0}_{1}_00", ky, kx).str();
        std::string w210 =
            llvm::formatv("w2_{0}_{1}_10", ky, kx).str();
        std::string w201 =
            llvm::formatv("w2_{0}_{1}_01", ky, kx).str();
        std::string w211 =
            llvm::formatv("w2_{0}_{1}_11", ky, kx).str();
...
emitSqrtOpSequenceBody(exec::KernelOp kernel, ArrayRef<Operation *> ops) {
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          kernel, {info.load.getPtr(), info.store.getPtr()},
          "executable sqrt op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
...
static FailureOr<RPUExecutableEmissionResult>
emitSqrtFromExecutableContract(
    const exec::ExecutableKernelContractInfo &contract) {
  exec::KernelOp kernel = contract.kernel;
  ArrayRef<Operation *> ops = contract.bodyOps;
  ...
  if (nvec <= 0 || info.store.getN() != nvec ||
      ...
      !isTileVectorF16(info.store.getValue().getType(), nvec)) {
    kernel.emitError(
        "executable sqrt contract-view emission requires positive !rpu.tile<Nxf16>");
    return failure();
  }
...
emitReduceSumAllOpSequenceBody(exec::KernelOp kernel,
  ...
  FailureOr<exec::ExecutableReduceSumAllOpSequenceInfo> maybeInfo =
      exec::getExecutableReduceSumAllOpSequenceInfo(
          kernel, ops,
          "executable reduce_sum_all sequence emission requires verified IR info");
  if (failed(maybeInfo))
    return failure();
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          kernel, {info.load.getPtr(), info.store.getPtr()},
          "executable reduce_sum_all op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
  ...
  return emitGenericExecutableOpSequenceBody(
      kernel, ops, bindings,
      "generic compact ReduceSumAll emission requires bound executable operands");
}
...
emitReduceSumAllFromExecutableContract(
  ...
  FailureOr<exec::ExecutableReduceSumAllOpSequenceInfo> maybeInfo =
      exec::getExecutableReduceSumAllOpSequenceInfo(
          kernel, ops,
          "executable reduce_sum_all contract-view emission requires verified IR info");
  if (failed(maybeInfo))
    return failure();
  exec::ExecutableReduceSumAllOpSequenceInfo info = *maybeInfo;
  ...
  if (nvec <= 0 || info.store.getN() != nvec ||
      ...
      !isTileVectorF16(info.store.getValue().getType(), nvec)) {
    kernel.emitError(
        "executable reduce_sum_all contract-view emission requires positive !rpu.tile<Nxf16>");
    return failure();
  }
...
emitReluOpSequenceBody(exec::KernelOp kernel, ArrayRef<Operation *> ops) {
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          kernel, {info.load.getPtr(), info.store.getPtr()},
          "executable relu op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
  unsigned inputArg = (*args)[0];
  unsigned outArg = (*args)[1];
...
static FailureOr<RPUExecutableEmissionResult>
emitReluFromExecutableContract(
    const exec::ExecutableKernelContractInfo &contract) {
  exec::KernelOp kernel = contract.kernel;
  ArrayRef<Operation *> ops = contract.bodyOps;
  FailureOr<exec::ExecutableReluOpSequenceInfo> maybeInfo =
  ...
  if (nvec <= 0 || info.store.getN() != nvec ||
      ...
      !isTileVectorF16(info.store.getValue().getType(), nvec)) {
    kernel.emitError(
        "executable relu contract-view emission requires positive !rpu.tile<Nxf16>");
    return failure();
  }
...
emitMaximumOpSequenceBody(exec::KernelOp kernel, ArrayRef<Operation *> ops) {
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          ...
          "executable maximum op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
...
static FailureOr<RPUExecutableEmissionResult>
emitMaximumFromExecutableContract(
    const exec::ExecutableKernelContractInfo &contract) {
  exec::KernelOp kernel = contract.kernel;
  ArrayRef<Operation *> ops = contract.bodyOps;
  FailureOr<exec::ExecutableMaximumOpSequenceInfo> maybeInfo =
      exec::getExecutableMaximumOpSequenceInfo(
          kernel, ops,
          "executable maximum contract-view emission requires verified IR info");
  if (failed(maybeInfo))
    return failure();
  exec::ExecutableMaximumOpSequenceInfo info = *maybeInfo;
  ...
  if (n <= 0 || info.lhsLoad.getN() != n || info.rhsLoad.getN() != n ||
      ...
      !isTileVectorF16(info.store.getValue().getType(), n)) {
    kernel.emitError(
        "executable maximum contract-view emission requires positive !rpu.tile<Nxf16>");
    return failure();
  }
...
emitReduceSumAxisOpSequenceBody(exec::KernelOp kernel,
  ...
  FailureOr<exec::ExecutableReduceSumAxisOpSequenceInfo> maybeInfo =
      exec::getExecutableReduceSumAxisOpSequenceInfo(
          kernel, ops,
          "executable reduce_sum_axis sequence emission requires verified IR info");
  if (failed(maybeInfo))
    return failure();
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          kernel, {info.load.getPtr(), info.store.getPtr()},
          "executable reduce_sum_axis op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
  unsigned inputArg = (*args)[0];
  unsigned outArg = (*args)[1];
  ...
  if (mChunks == 1) {
    ...
    os << "    auto tile0 = ctx.load<half, " << M << ", " << N
       << ">(arg" << inputArg << "_arr, rpu::IndexList{0, 0});\n";
    os << "    auto tile1 = ctx.reduce_sum<" << axis << ">(tile0);\n";
    os << "    auto tile2 = ctx.zeros<half, " << M << ", " << N
       << ">() + tile1;\n";
    os << "    ctx.store<half, " << M << ", " << N
       << ">(arg" << outArg << "_arr, rpu::IndexList{0, 0}, tile2);";
  } else if (axis == 0) {
    ...
    for (int64_t c = 0; c < mChunks; ++c) {
      int64_t rowOff = c * 16;
      os << "    auto tile_x" << c << " = ctx.load<half, 16, " << N
         << ">(arg" << inputArg << "_arr, rpu::IndexList{" << rowOff
         << ", 0});\n";
    ...
    for (int64_t c = 0; c < mChunks; ++c) {
      int64_t rowOff = c * 16;
      os << "    {\n";
      os << "        auto tile_x = ctx.load<half, 16, " << N
         << ">(arg" << inputArg << "_arr, rpu::IndexList{" << rowOff
         << ", 0});\n";
      os << "        auto tile_r = ctx.reduce_sum<1>(tile_x);\n";
      os << "        auto full = ctx.zeros<half, 16, " << N << ">() + tile_r;\n";
      os << "        ctx.store<half, 16, " << N << ">(arg" << outArg
...
emitReduceSumAxisFromExecutableContract(
  ...
  FailureOr<exec::ExecutableReduceSumAxisOpSequenceInfo> maybeInfo =
      exec::getExecutableReduceSumAxisOpSequenceInfo(
          kernel, ops,
          "executable reduce_sum_axis contract-view emission requires verified IR info");
  if (failed(maybeInfo))
    return failure();
  exec::ExecutableReduceSumAxisOpSequenceInfo info = *maybeInfo;
...
emitBroadcastAddOpSequenceBody(exec::KernelOp kernel,
  ...
  FailureOr<exec::ExecutableBroadcastAddOpSequenceInfo> maybeInfo =
      exec::getExecutableBroadcastAddOpSequenceInfo(
          kernel, ops,
          "executable broadcast_add sequence emission requires verified IR info");
  if (failed(maybeInfo))
    return failure();
  ...
  FailureOr<llvm::SmallVector<unsigned, 4>> args =
      exec::getExecutableKernelArgumentIndices(
          kernel,
          {info.lhsLoad.getPtr(), info.rhsLoad.getPtr(), info.store.getPtr()},
          "executable broadcast_add op-sequence emission requires block argument pointers");
  if (failed(args))
    return failure();
  unsigned lhsArg = (*args)[0];
  unsigned rhsArg = (*args)[1];
...
emitBroadcastAddFromExecutableContract(
  ...
  FailureOr<exec::ExecutableBroadcastAddOpSequenceInfo> maybeInfo =
      exec::getExecutableBroadcastAddOpSequenceInfo(
          kernel, ops,
          "executable broadcast_add contract-view emission requires verified IR info");
  if (failed(maybeInfo))
    return failure();
  exec::ExecutableBroadcastAddOpSequenceInfo info = *maybeInfo;
  int64_t rows = info.rows;
...
static FailureOr<std::string>
emitGenericVectorOpSequenceBody(
    const exec::ExecutableKernelContractInfo &contract) {
  ...
  if (failed(exec::verifyExecutableElementwise1DOpSequence(
          ...
          "generic Elementwise1D emission requires verifier-only executable IR contract")))
    return failure();

  llvm::SmallVector<ExecutableMemoryBinding, 4> bindings;
  for (const exec::ExecutableKernelArgumentInfo &arg : contract.arguments) {
    bindings.push_back(ExecutableMemoryBinding{
        arg.value, arg.index, arg.name, std::nullopt});
  }
...
static FailureOr<RPUExecutableEmissionResult>
emitGenericFromExecutableContract(
    const exec::ExecutableKernelContractInfo &contract) {
  FailureOr<std::string> prototype =
      buildKernelArgumentPrototype(contract.arguments);
  if (failed(prototype))
