...
FailureOr<KernelOp> mlir::rpu::exec::getSingleExecutableKernelFromModule(
  ...
  if (kernels.size() > 1) {
    module.emitError() << consumer << " requires one rpu.kernel, found multiple";
    return failure();
  }
...
mlir::rpu::exec::getVerifiedExecutableKernelContract(
...
mlir::rpu::exec::getNonEmptyExecutableKernelBodyOps(
    KernelOp kernel, llvm::StringRef consumer) {
...
static bool hasLegalizableGemmBody(ArrayRef<Operation *> ops) {
  return ops.size() == 5 && isOpAt<LoadMatrixOp>(ops, 0) &&
         ...
         isOpAt<DotOp>(ops, 2) && isOpAt<StoreMatrixOp>(ops, 3) &&
         isOpAt<ReturnOp>(ops, 4);
}
...
getConvKxKKernelSizeFromOpCount(size_t opCount) {
...
static bool hasConvKxKBody(ArrayRef<Operation *> ops) {
  ...
  for (int64_t i = 0; i < windows; ++i) {
    ...
    if (!isOpAt<LoadMatrixOp>(ops, base) ||
        !isOpAt<LoadMatrixOp>(ops, base + 1) ||
        !isOpAt<MmaOp>(ops, base + 2))
      return false;
  }
...
static bool hasLegalizableResNetBlockBody(ArrayRef<Operation *> ops) {
  ...
  return ops.size() == 18 && isOpAt<LoadMatrixOp>(ops, 0) &&
         ...
         isOpAt<MaxOp>(ops, 4) && isOpAt<LoadMatrixOp>(ops, 5) &&
         isHighLevelLegalizableExecutableOp(ops[6]) &&
         isOpAt<DotOp>(ops, 6) && isOpAt<ZeroOp>(ops, 7) &&
         isOpAt<MaxOp>(ops, 8) && isOpAt<LoadMatrixOp>(ops, 9) &&
...
static bool hasResNet50BottleneckBody(ArrayRef<Operation *> ops) {
  if (ops.size() == 76) {
    if (!isOpAt<LoadMatrixOp>(ops, 0) ||
        !isOpAt<LoadMatrixOp>(ops, 1) || !isOpAt<ZeroOp>(ops, 2))
      return false;
    for (size_t i = 0; i < 9; ++i) {
      size_t base = 3 + i * 7;
      if (!isOpAt<LoadMatrixOp>(ops, base) ||
          !isOpAt<ZeroOp>(ops, base + 1) ||
          !isOpAt<MmaOp>(ops, base + 2) ||
          !isOpAt<ZeroOp>(ops, base + 3) ||
          !isOpAt<MaxOp>(ops, base + 4) ||
          !isOpAt<LoadMatrixOp>(ops, base + 5) ||
          !isOpAt<MmaOp>(ops, base + 6))
        return false;
    }
    size_t tail = 66;
    return isOpAt<ZeroOp>(ops, tail) && isOpAt<MaxOp>(ops, tail + 1) &&
           isOpAt<LoadMatrixOp>(ops, tail + 2) &&
           isOpAt<ZeroOp>(ops, tail + 3) &&
           isOpAt<MmaOp>(ops, tail + 4) &&
           isOpAt<AddOp>(ops, tail + 5) &&
           isOpAt<ZeroOp>(ops, tail + 6) &&
  ...
  for (size_t i = 0; i < 9; ++i) {
    ...
    if (!isOpAt<LoadMatrixOp>(ops, base) ||
        ...
        !isOpAt<ZeroOp>(ops, base + 3) ||
        !isOpAt<MaxOp>(ops, base + 4) ||
        !isOpAt<ZeroOp>(ops, base + 5) ||
        !isOpAt<MmaOp>(ops, base + 6) ||
        !isOpAt<ZeroOp>(ops, base + 7) ||
        !isOpAt<MaxOp>(ops, base + 8) ||
        !isOpAt<LoadMatrixOp>(ops, base + 9) ||
        ...
        !isOpAt<LoadMatrixOp>(ops, base + 15) ||
        !isOpAt<MmaOp>(ops, base + 16))
      return false;
  }
  size_t tail = 158;
  return isOpAt<ZeroOp>(ops, tail) && isOpAt<MaxOp>(ops, tail + 1) &&
         isOpAt<ZeroOp>(ops, tail + 2) &&
         isOpAt<MaxOp>(ops, tail + 3) &&
         isOpAt<LoadMatrixOp>(ops, tail + 4) &&
         isOpAt<ZeroOp>(ops, tail + 5) &&
         isOpAt<MmaOp>(ops, tail + 6) &&
         isOpAt<LoadMatrixOp>(ops, tail + 7) &&
         isOpAt<MmaOp>(ops, tail + 8) &&
         isOpAt<AddOp>(ops, tail + 9) &&
         isOpAt<ZeroOp>(ops, tail + 10) &&
         isOpAt<MaxOp>(ops, tail + 11) &&
         isOpAt<StoreMatrixOp>(ops, tail + 12) &&
         isOpAt<ReturnOp>(ops, tail + 13);
}

static bool
hasLegalizableResNet50BottleneckBody(ArrayRef<Operation *> ops) {
  if (ops.size() == 66) {
    if (!isOpAt<LoadMatrixOp>(ops, 0) ||
        !isOpAt<LoadMatrixOp>(ops, 1) || !isOpAt<ZeroOp>(ops, 2))
      return false;
    for (size_t i = 0; i < 9; ++i) {
      ...
      if (!isOpAt<LoadMatrixOp>(ops, base) ||
          ...
          !isOpAt<DotOp>(ops, base + 1) ||
          !isOpAt<ZeroOp>(ops, base + 2) ||
          !isOpAt<MaxOp>(ops, base + 3) ||
          !isOpAt<LoadMatrixOp>(ops, base + 4) ||
          !isOpAt<MmaOp>(ops, base + 5))
        return false;
    }
    size_t tail = 57;
    return isOpAt<ZeroOp>(ops, tail) && isOpAt<MaxOp>(ops, tail + 1) &&
           ...
           isOpAt<DotOp>(ops, tail + 3) &&
           isOpAt<AddOp>(ops, tail + 4) &&
           isOpAt<ZeroOp>(ops, tail + 5) &&
           isOpAt<MaxOp>(ops, tail + 6) &&
  ...
  for (size_t i = 0; i < 9; ++i) {
    ...
    if (!isOpAt<LoadMatrixOp>(ops, base) ||
        ...
        !isOpAt<DotOp>(ops, base + 4) ||
        !isOpAt<ZeroOp>(ops, base + 5) ||
        !isOpAt<MaxOp>(ops, base + 6) ||
        !isOpAt<LoadMatrixOp>(ops, base + 7) ||
        !isOpAt<MmaOp>(ops, base + 8) ||
        !isOpAt<LoadMatrixOp>(ops, base + 9) ||
        !isOpAt<MmaOp>(ops, base + 10) ||
        !isOpAt<LoadMatrixOp>(ops, base + 11) ||
        !isOpAt<MmaOp>(ops, base + 12) ||
  ...
  return isOpAt<ZeroOp>(ops, tail) && isOpAt<MaxOp>(ops, tail + 1) &&
         ...
         isOpAt<DotOp>(ops, tail + 5) &&
         isOpAt<LoadMatrixOp>(ops, tail + 6) &&
         isOpAt<MmaOp>(ops, tail + 7) &&
         isOpAt<AddOp>(ops, tail + 8) &&
         isOpAt<ZeroOp>(ops, tail + 9) &&
         isOpAt<MaxOp>(ops, tail + 10) &&
...
static LogicalResult verifyAddDataflow(KernelOp op, Block &block,
  ...
  if (!logicalNAttr || !blockNAttr)
    return op.emitOpError("kind add masked store requires logical_n and block_n");
...
mlir::rpu::exec::getExecutableAddOpSequenceInfo(
  ...
  return ExecutableAddOpSequenceInfo{lhsLoad, rhsLoad, add, store, n,
                                     logicalN, masked};
}
...
mlir::rpu::exec::getExecutableGemmOpSequenceInfo(
    KernelOp kernel, ArrayRef<Operation *> ops, llvm::StringRef consumer) {
  if (!hasGemmBody(ops)) {
    kernel.emitError() << consumer << " requires canonical gemm executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
  ...
  return ExecutableGemmOpSequenceInfo{lhsLoad, rhsLoad, zero, mma, store,
                                      m, n, k};
}
...
mlir::rpu::exec::getExecutableSoftmaxOpSequenceInfo(
    KernelOp kernel, ArrayRef<Operation *> ops, llvm::StringRef consumer) {
  if (!hasSoftmaxBody(ops)) {
    kernel.emitError()
        << consumer << " requires canonical softmax executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
  ...
  auto mul = cast<MulScalarOp>(ops[6]);
  auto store = cast<StoreContigOp>(ops[7]);
  return ExecutableSoftmaxOpSequenceInfo{load, reduceMax, sub, exp,
                                         reduceSum, reciprocal, mul, store,
                                         load.getN()};
}
...
mlir::rpu::exec::getExecutableSqrtOpSequenceInfo(
    KernelOp kernel, ArrayRef<Operation *> ops, llvm::StringRef consumer) {
  if (!hasSqrtBody(ops)) {
    kernel.emitError() << consumer << " requires canonical sqrt executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
...
mlir::rpu::exec::getExecutableReduceSumAllOpSequenceInfo(
  ...
  if (!hasReduceSumAllBody(ops)) {
    kernel.emitError()
        << consumer << " requires canonical reduce_sum_all executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
...
verifyReduceSumAxisDataflow(KernelOp op, Block &block,
                            ArrayRef<Operation *> ops) {
...
static LogicalResult
verifyBroadcastAddDataflow(KernelOp op, Block &block,
                           ArrayRef<Operation *> ops) {
  auto lhsLoad = cast<LoadMatrixOp>(ops[0]);
  auto rhsLoad = cast<LoadContigOp>(ops[1]);
...
mlir::rpu::exec::getExecutableReluOpSequenceInfo(KernelOp kernel,
                                                 ...
                                                 llvm::StringRef consumer) {
  if (!hasReluBody(ops)) {
    kernel.emitError() << consumer << " requires canonical relu executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
...
mlir::rpu::exec::getExecutableMaximumOpSequenceInfo(
    KernelOp kernel, ArrayRef<Operation *> ops, llvm::StringRef consumer) {
  if (!hasMaximumBody(ops)) {
    kernel.emitError()
        << consumer << " requires canonical maximum executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
...
mlir::rpu::exec::getExecutableReduceSumAxisOpSequenceInfo(
  ...
  if (!hasReduceSumAxisBody(ops)) {
    kernel.emitError()
        << consumer << " requires canonical reduce_sum_axis executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
  ...
  return ExecutableReduceSumAxisOpSequenceInfo{
      load, reduce, store, load.getRows(), load.getCols(),
      static_cast<int64_t>(reduce.getAxis())};
}
...
mlir::rpu::exec::getExecutableBroadcastAddOpSequenceInfo(
  ...
  if (!hasBroadcastAddBody(ops)) {
    kernel.emitError()
        << consumer << " requires canonical broadcast_add executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
...
mlir::rpu::exec::getExecutableConvKxKOpSequenceInfo(
    KernelOp kernel, ArrayRef<Operation *> ops, llvm::StringRef consumer) {
  ConvKxKBodyLayout layout = getConvKxKBodyLayout(ops);
  if (layout != ConvKxKBodyLayout::CanonicalPrimitive &&
      layout != ConvKxKBodyLayout::ExpandedDotSeedPrimitive) {
    kernel.emitError()
        << consumer << " requires primitive convkxk executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
  ...
  size_t zeroIndex =
      layout == ConvKxKBodyLayout::CanonicalPrimitive ? 0 : 2;
  ...
  for (int64_t i = 0; i < windows; ++i) {
    size_t base =
        layout == ConvKxKBodyLayout::CanonicalPrimitive
    ...
    size_t mmaIndex = layout == ConvKxKBodyLayout::ExpandedDotSeedPrimitive &&
                              i == 0
                          ? 3
                          : base + 2;
  ...
  auto store = cast<StoreMatrixOp>(ops[storeIndex]);
  return ExecutableConvKxKOpSequenceInfo{zero, inputLoads, weightLoads, mmas,
                                         store, *kernelSize};
}
...
static LogicalResult verifyResNetBlockDataflow(KernelOp op, Block &block,
  ...
  if (hasLegalizableResNetBlockBody(ops)) {
    if (ops.size() == 18) {
      ...
      if (!is16x16Load(x) || !is16x16Load(w1Lo) || !is16x16Load(w1Hi) ||
          !is16x16Load(w2Lo) || !is16x16Load(w2Hi) ||
          !is16x16Zero(zero1Lo) || !is16x16Zero(zero1Hi) ||
    ...
    if (x.getPtr() != block.getArgument(1) ||
        ...
        zero2.getRows() != 16 || zero2.getCols() != 16 ||
        store.getRows() != 16 || store.getCols() != 16 ||
        conv1.getLhs() != x.getResult() ||
  ...
  if (ops.size() == 21) {
    ...
    if (!is16x16Load(x) || !is16x16Load(w1Lo) || !is16x16Load(w1Hi) ||
        !is16x16Load(w2Lo) || !is16x16Load(w2Hi) ||
        !is16x16Zero(conv1LoZero) || !is16x16Zero(zero1Lo) ||
        !is16x16Zero(conv1HiZero) || !is16x16Zero(zero1Hi) ||
        !is16x16Zero(conv2Zero) || !is16x16Zero(zero2) ||
  ...
  if (x.getPtr() != block.getArgument(1) ||
      ...
      x.getCols() != 16 || w1.getRows() != 16 || hidden != 16 ||
      w2.getRows() != hidden ||
      w2.getCols() != 16 || conv1Zero.getRows() != 16 ||
      conv1Zero.getCols() != hidden || zero1.getRows() != 16 ||
      zero1.getCols() != hidden ||
...
mlir::rpu::exec::getExecutableResNetBlockOpSequenceInfo(
  ...
  if (!hasResNetBlockBody(ops)) {
    kernel.emitError()
        << consumer << " requires canonical residual executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
  ...
  SmallVector<ZeroOp, 5> zeros{
      cast<ZeroOp>(ops[2]), cast<ZeroOp>(ops[4]), cast<ZeroOp>(ops[7]),
      cast<ZeroOp>(ops[10])};
...
static LogicalResult verifyResNet50BottleneckDataflow(
    KernelOp op, Block &block, ArrayRef<Operation *> ops) {
  auto fail = [&]() {
    return op.emitOpError(
        "kind resnet50_bottleneck requires canonical resnet50 bottleneck dataflow");
  };
  auto is16x16Load = [](LoadMatrixOp load) {
    return load.getRows() == 16 && load.getCols() == 16;
  };
  ...
  if (hasLegalizableResNet50BottleneckBody(ops)) {
    if (ops.size() == 153) {
      ...
      if (hasInvalidMatrixOffsetAttr(xSkip.getOperation()) ||
          ...
          hasInvalidMatrixOffsetAttr(w1Hi.getOperation()))
        return fail();
      if (!is16x16Load(xSkip) || !is16x16Load(w1Lo) ||
          !is16x16Load(w1Hi) || !is16x16Zero(conv2LoZero) ||
          !is16x16Zero(conv2HiZero) ||
          xSkip.getPtr() != block.getArgument(1) ||
          w1Lo.getPtr() != block.getArgument(2) ||
          w1Hi.getPtr() != block.getArgument(2) ||
          !hasOffsets(xSkip, 0, 0) || !hasOffsets(w1Lo, 0, 0) ||
          !hasOffsets(w1Hi, 0, 16))
        return fail();
      ...
      for (int64_t i = 0; i < 9; ++i) {
        ...
        if (hasInvalidMatrixOffsetAttr(x.getOperation()) ||
          ...
          return fail();
        if (!is16x16Load(x) || !is16x16Zero(zero1Lo) ||
      ...
      if (!is16x16Zero(zero2Lo) || !is16x16Zero(zero2Hi) ||
          !is16x16Load(w3Lo) || !is16x16Load(w3Hi) ||
          !is16x16Zero(zero3) || w3Lo.getPtr() != block.getArgument(4) ||
    ...
    if (hasInvalidMatrixOffsetAttr(xSkip.getOperation()) ||
      ...
      return fail();
    if (!is16x16Load(xSkip) || !is16x16Load(w1) ||
        !is16x16Zero(conv2Zero) || xSkip.getPtr() != block.getArgument(1) ||
    ...
    for (int64_t i = 0; i < 9; ++i) {
      ...
      if (hasInvalidMatrixOffsetAttr(x.getOperation()) ||
        ...
        return fail();
      if (!is16x16Load(x) || !is16x16Zero(zero1) ||
    ...
    if (hasInvalidMatrixOffsetAttr(w3.getOperation()))
      return fail();
    if (!is16x16Zero(zero2) || !is16x16Load(w3) ||
  ...
  if (ops.size() == 172) {
    ...
    if (hasInvalidMatrixOffsetAttr(w3Lo.getOperation()) ||
      ...
      return fail();
    if (!is16x16Zero(zero2Lo) || !is16x16Zero(zero2Hi) ||
        !is16x16Load(w3Lo) || !is16x16Zero(conv3Zero) ||
        !is16x16Load(w3Hi) || !is16x16Zero(zero3) ||
  ...
  for (int64_t i = 0; i < 9; ++i) {
    ...
    if (hasInvalidMatrixOffsetAttr(x.getOperation()) ||
        hasInvalidMatrixOffsetAttr(w2.getOperation()))
      return fail();
    if (!is16x16Load(x) || !is16x16Zero(conv1Zero) ||
        !is16x16Zero(zero1) || !is16x16Load(w2) ||
        x.getPtr() != block.getArgument(1) ||
        w2.getPtr() != block.getArgument(3) ||
        getI32AttrOrZero(x.getOperation(), "row_offset") != expectedXRow ||
        getI32AttrOrZero(x.getOperation(), "col_offset") != 0 ||
  ...
  if (hasInvalidMatrixOffsetAttr(w3.getOperation()))
    return fail();
  if (!is16x16Zero(zero2) || !is16x16Load(w3) ||
      !is16x16Zero(conv3Zero) || !is16x16Zero(zero3) ||
...
mlir::rpu::exec::getExecutableResNet50BottleneckOpSequenceInfo(
  ...
  if (!hasResNet50BottleneckBody(ops)) {
    kernel.emitError()
        << consumer << " requires canonical resnet50 bottleneck executable body";
    return failure();
  }
  if (kernel.getBody().empty()) {
    kernel.emitError() << consumer << " requires executable body";
  ...
  if (ops.size() == 172) {
    ...
    SmallVector<ZeroOp, 42> zeros{cast<ZeroOp>(ops[3]),
                                  cast<ZeroOp>(ops[4])};
...
static constexpr ExecutableVerifierEntry kExecutableVerifierEntries[] = {
    ...
    {"resnet_block", hasResNetBlockExecutableBody,
     "kind resnet_block requires canonical or legalizable residual block executable body",
     verifyResNetBlockDataflow},
    {"resnet50_bottleneck", hasResNet50BottleneckExecutableBody,
     "kind resnet50_bottleneck requires canonical or legalizable resnet50 bottleneck executable body",
    ...
    {"relu", hasReluBody,
     "kind relu requires canonical relu executable body",
     verifyReluDataflow},
...
FailureOr<SmallVector<Operation *>>
mlir::rpu::exec::getCanonicalExecutableKernelBodyOps(
    KernelOp kernel, llvm::StringRef consumer) {
...
bool mlir::rpu::exec::isGenericRenderableExecutableKernelKind(StringRef kind) {
  return kind == "generic" || kind == "add" || kind == "gemm" ||
         kind == "softmax" || kind == "convkxk" ||
         kind == "resnet_block" || kind == "resnet50_bottleneck" ||
         kind == "sqrt" || kind == "reduce_sum_all" ||
         kind == "relu" || kind == "maximum" ||
...
static LogicalResult verifyCompactElementwise1DContract(
    Operation *op, OperandRange inputs, Type resultType,
    ArrayRef<int64_t> opcodes, ArrayRef<int64_t> lhsSlots,
...
mlir::rpu::exec::getExecutableVectorMaskInfo(Operation *op, int64_t n,
  ...
  if (!logicalNAttr || !blockNAttr) {
    op->emitError() << consumer
                    << " masked op requires logical_n and block_n";
    return failure();
  }
  if (!logicalNAttr.getType().isInteger(32) ||
      !blockNAttr.getType().isInteger(32)) {
    op->emitError() << consumer
                    << " masked logical_n and block_n must be i32";
    return failure();
  }
  int64_t logicalN = logicalNAttr.getInt();
  int64_t blockN = blockNAttr.getInt();
  ...
  if (logicalN <= 0 || logicalN > blockN) {
    op->emitError() << consumer
                    << " masked logical_n must be in (0, block_n]";
    return failure();
  }
...
LogicalResult mlir::rpu::exec::buildExecutableConvKxKBody(
  ...
  builder.create<StoreMatrixOp>(
      loc, entry.getArgument(spec.outputArgIndex), acc, 16, 16);
...
LogicalResult mlir::rpu::exec::buildExecutableResNetBlockBody(
    OpBuilder &builder, Location loc, KernelOp kernel,
    const ExecutableResNetBlockBodyBuildSpec &spec,
    llvm::StringRef consumer) {
  ...
  unsigned argIndices[] = {spec.outputArgIndex, spec.xArgIndex,
                           spec.w1ArgIndex, spec.w2ArgIndex};
  for (unsigned index : argIndices) {
    if (index >= entry.getNumArguments())
      return kernel.emitError() << consumer << " arg index is out of range";
  }
  ...
  Type tile16xHidden =
      TileType::get(builder.getContext(), shape16xHidden, f16);
  Type tileHiddenx16 =
      TileType::get(builder.getContext(), shapeHiddenx16, f16);
  ...
  if (spec.hidden == 32) {
    auto w1Lo =
        builder.create<LoadMatrixOp>(loc, tile16x16, w1Ptr, 16, 16);
    ...
    auto outTile = builder.create<MaxOp>(
        loc, tile16x16, residual.getResult(), zero2.getResult());
    builder.create<StoreMatrixOp>(loc, out, outTile.getResult(), 16, 16);
    builder.create<ReturnOp>(loc);
  } else {
    auto w1 =
        builder.create<LoadMatrixOp>(loc, tile16xHidden, w1Ptr, 16,
                                     spec.hidden);
    auto conv1 =
        builder.create<DotOp>(loc, tile16xHidden, x.getResult(),
                              w1.getResult());
    auto zero1 = builder.create<ZeroOp>(loc, tile16xHidden, 16, spec.hidden);
    auto relu1 = builder.create<MaxOp>(
        loc, tile16xHidden, conv1.getResult(), zero1.getResult());
    auto w2 =
        builder.create<LoadMatrixOp>(loc, tileHiddenx16, w2Ptr, spec.hidden,
                                     16);
    auto conv2 =
        builder.create<DotOp>(loc, tile16x16, relu1.getResult(),
                              w2.getResult());
    auto residual = builder.create<AddOp>(
  ...
  if (spec.bottleneck == 32) {
    ...
    for (int64_t ky = 0; ky < 3; ++ky) {
      for (int64_t kx = 0; kx < 3; ++kx) {
        ...
        auto x =
            builder.create<LoadMatrixOp>(loc, tile16x16, xPtr, 16, 16);
        setExecutableMatrixOffsets(builder, x.getOperation(), xRow);
        auto conv1Lo = builder.create<DotOp>(
            loc, tile16x16, x.getResult(), w1Lo.getResult());
        auto zero1Lo = builder.create<ZeroOp>(loc, tile16x16, 16, 16);
        auto relu1Lo = builder.create<MaxOp>(
            loc, tile16x16, conv1Lo.getResult(), zero1Lo.getResult());
        auto conv1Hi = builder.create<DotOp>(
    ...
    auto relu2Lo = builder.create<MaxOp>(loc, tile16x16, conv2AccLo,
                                         zero2Lo.getResult());
    auto zero2Hi = builder.create<ZeroOp>(loc, tile16x16, 16, 16);
    auto relu2Hi = builder.create<MaxOp>(loc, tile16x16, conv2AccHi,
                                         zero2Hi.getResult());
    auto w3Lo =
        builder.create<LoadMatrixOp>(loc, tile16x16, w3Ptr, 16, 16);
    ...
    for (int64_t ky = 0; ky < 3; ++ky) {
      for (int64_t kx = 0; kx < 3; ++kx) {
        ...
        setExecutableMatrixOffsets(builder, x.getOperation(), xRow);
        auto conv1 =
            builder.create<DotOp>(loc, tile16x16, x.getResult(),
                                  w1.getResult());
    ...
    auto zero2 = builder.create<ZeroOp>(loc, tile16x16, 16, 16);
    auto relu2 = builder.create<MaxOp>(loc, tile16x16, conv2Acc,
                                       zero2.getResult());
    auto w3 = builder.create<LoadMatrixOp>(loc, tile16x16, w3Ptr, 16, 16);
    auto conv3 =
        builder.create<DotOp>(loc, tile16x16, relu2.getResult(),
                              w3.getResult());
    auto residual = builder.create<AddOp>(
...
LogicalResult mlir::rpu::exec::expandExecutableDotOp(
    OpBuilder &builder, DotOp dot, llvm::StringRef consumer) {
  (void)consumer;
...
LogicalResult mlir::rpu::exec::buildExecutableSoftmaxBody(
  ...
  builder.create<StoreContigOp>(
      loc, entry.getArgument(spec.outputArgIndex), y.getResult(), spec.nvec);
  builder.create<ReturnOp>(loc);
...
LogicalResult mlir::rpu::exec::buildExecutableGemmBody(
  ...
  auto dot = builder.create<DotOp>(loc, tileMN, lhs.getResult(),
                                   rhs.getResult());
  builder.create<StoreMatrixOp>(
      loc, entry.getArgument(spec.outputArgIndex), dot.getResult(), spec.m,
      spec.n);
  builder.create<ReturnOp>(loc);
...
LogicalResult mlir::rpu::exec::buildExecutableAddBody(
  ...
  auto sum = builder.create<AddOp>(loc, tileType, lhs.getResult(),
                                   rhs.getResult());
...
static LogicalResult verifyElementwise16ValueMapContract(
    Operation *op, OperandRange inputs, Type resultType,
    ArrayRef<int64_t> opcodes, ArrayRef<int64_t> lhsSlots,
    ArrayRef<int64_t> rhsSlots) {
  FailureOr<TileType> resultTile = requireTile(op, resultType);
...
LogicalResult mlir::rpu::exec::buildExecutableReduceSumAllBody(
  ...
  builder.create<StoreContigOp>(
      loc, entry.getArgument(spec.outputArgIndex), v.getResult(), spec.nvec);
  builder.create<ReturnOp>(loc);
  FailureOr<SmallVector<Operation *>> bodyOps =
      getCanonicalExecutableKernelBodyOps(kernel, consumer);
  if (failed(bodyOps))
...
LogicalResult mlir::rpu::exec::buildExecutableMaximumBody(
  ...
  auto rhs = builder.create<LoadContigOp>(
      loc, tileType, entry.getArgument(spec.rhsArgIndex), spec.n);
  auto m = builder.create<MaxOp>(loc, tileType, lhs.getResult(),
                                 rhs.getResult());
  builder.create<StoreContigOp>(
      loc, entry.getArgument(spec.outputArgIndex), m.getResult(), spec.n);
  builder.create<ReturnOp>(loc);
  FailureOr<SmallVector<Operation *>> bodyOps =
      getCanonicalExecutableKernelBodyOps(kernel, consumer);
  if (failed(bodyOps))
...
LogicalResult mlir::rpu::exec::buildExecutableBroadcastAddBody(
  ...
  builder.setInsertionPointToStart(&entry);
  auto lhs = builder.create<LoadMatrixOp>(
      loc, matTileType, entry.getArgument(spec.lhsArgIndex), spec.rows,
      spec.cols);
  auto rhs = builder.create<LoadContigOp>(
...
LogicalResult mlir::rpu::exec::expandExecutableElementwise16ValueMapOp(
    OpBuilder &builder, Elementwise16ValueMapOp map,
    llvm::StringRef consumer) {
  (void)consumer;
...
LogicalResult mlir::rpu::exec::expandExecutableCompactElementwise1DOp(
    OpBuilder &builder, CompactElementwise1DOp op,
    llvm::StringRef consumer) {
  (void)consumer;
...
LogicalResult mlir::rpu::exec::buildExecutableElementwise16ValueMapBody(
  ...
  for (unsigned argIndex : spec.inputArgIndices) {
    auto load = builder.create<LoadContigOp>(
        loc, tileType, entry.getArgument(argIndex), 16);
  ...
  builder.create<StoreContigOp>(loc, entry.getArgument(0),
                                valueMap.getResult(), 16);
  builder.create<ReturnOp>(loc);
...
LogicalResult mlir::rpu::exec::buildExecutableCompactElementwise1DBody(
  ...
  if (spec.outputArgIndex >= entry.getNumArguments())
    return kernel.emitError() << consumer << " output arg index is out of range";
  for (unsigned argIndex : spec.inputArgIndices) {
    if (argIndex >= entry.getNumArguments())
      return kernel.emitError() << consumer
                                << " input arg index is out of range";
  }
  ...
  for (const ExecutableCompactVectorBinaryBuildOp &op : spec.ops) {
    if (op.lhs < 0 || op.rhs < 0 ||
        op.lhs >= availableSlots || op.rhs >= availableSlots)
      return kernel.emitError()
  ...
  auto store = builder.create<StoreContigOp>(
      loc, entry.getArgument(spec.outputArgIndex), elementwise.getResult(),
      spec.n);
  if (spec.masked)
    setExecutableVectorMaskAttrs(builder, store.getOperation(), spec.logicalN,
                                 spec.n);
...
LogicalResult mlir::rpu::exec::verifyExecutableElementwise1DOpSequence(
  ...
  if (index + 2 != ops.size() || !isa<StoreContigOp>(ops[index]) ||
    ...
    kernel.emitError()
        << consumer << " requires compact load/binary/store/return body";
    return failure();
  }
  ...
  if (loads.size() < 2 || loads.size() > 4 || binaryOps.empty() ||
      binaryOps.size() > 3) {
    kernel.emitError()
        << consumer << " requires two to four loads and one to three binary ops";
    return failure();
  }
  ...
  if (block.getNumArguments() != loads.size() + 1) {
    kernel.emitError()
        << consumer << " requires output arg0 plus ordered input args";
    return failure();
  }
  ...
  for (Operation *op : binaryOps) {
    ...
    if (!availableValues.contains(lhs) || !availableValues.contains(rhs)) {
      kernel.emitError()
          << consumer << " requires ordered acyclic binary dataflow";
      return failure();
    }
  ...
  if (failed(verifyExecutableCompactVectorStore(
          store, block.getArgument(0), finalResult, n, *firstMask,
          consumer))) {
    return failure();
  }
...
mlir::rpu::exec::getGenericRenderableExecutableKernelBodyOps(
  ...
  if (failed(ops))
    return failure();
  if (failed(verifyGenericRenderableExecutableOpSequence(kernel, *ops,
                                                         consumer)))
    return failure();
  return *ops;
}
...
LogicalResult AddOp::verify() {
  ...
  FailureOr<TileType> result = requireTile(getOperation(), getResult().getType());
...
LogicalResult Elementwise16ValueMapOp::verify() {
  return verifyElementwise16ValueMapContract(
      getOperation(), getInputs(), getResult().getType(),
      getOpcodes(), getLhsSlots(), getRhsSlots());
}

LogicalResult CompactElementwise1DOp::verify() {
  return verifyCompactElementwise1DContract(
...
LogicalResult MmaOp::verify() {
  ...
  if ((*lhs).getRank() != 2 || (*rhs).getRank() != 2 ||
      (*acc).getRank() != 2 || (*result).getRank() != 2)
    return emitOpError(
