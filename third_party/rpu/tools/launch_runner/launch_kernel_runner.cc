...
ArgSpec parse_arg(const std::string &spec) {
  ...
  if (p1 == std::string::npos || p2 == std::string::npos) {
    std::cerr << "bad arg_spec: " << spec
              << " (expected role:size:path)" << std::endl;
    std::exit(2);
  }
...
int main(int argc, char *argv[]) {
  ...
  if (std::getenv("LK_USE_RPUBIN")) {
    ...
    kernel_ptr = std::make_unique<Kernel_t>(
        reinterpret_cast<const char *>(instr_bytes.data()),
        instr_bytes.size(), kernel_name.c_str());
  } else {
  ...
  for (size_t i = 0; i < args.size(); ++i) {
    ...
    if (alloc < kSpmBankSize) alloc = kSpmBankSize;
    ...
    uint32_t hi = kernel.set_regs(2 * i + 1,
                                  static_cast<uint16_t>(addr >> 16));
    std::cerr << "[runner]   arg " << i << " role="
              << (args[i].is_out ? "out" : "in ")
              << " size=" << args[i].size_bytes << " alloc=" << alloc
              << " rpu_addr=0x" << std::hex << addr
              << " set_regs(" << std::dec << (2 * i) << ",lo=0x" << std::hex
              << (addr & 0xFFFF) << ")/(" << std::dec << (2 * i + 1)
              << ",hi=0x" << std::hex << (addr >> 16) << ") rc=" << std::dec
              << lo << "/" << hi << std::endl;
    if (lo != 0 || hi != 0) return 7;
