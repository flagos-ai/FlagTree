...
void init_triton_rpu(py::module &&m) {
  ...
  passes.def("add_lower_supported_ttir_to_executable",
             [](mlir::PassManager &pm) {
               pm.addPass(
                   mlir::rpu::createLowerSupportedTTIRToRPUExecutablePass());
  ...
  passes.def("add_legalize_executable_compact_elementwise1d",
             [](mlir::PassManager &pm) {
               pm.addPass(
                   mlir::rpu::
  ...
  m.def("_rpuplan_executable_lowering_failure_reason",
        [](mlir::ModuleOp &module) {
          ...
          if (failed(reason))
            throw std::runtime_error(
                "RPU executable lowering failure reason unavailable; see MLIR diagnostics");
          return *reason;
        });
