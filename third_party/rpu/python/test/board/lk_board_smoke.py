...
def _build_cases():
    ...
    def rpu_gemm_kernel(out, lhs, rhs, M: tl.constexpr, K: tl.constexpr, N: tl.constexpr):
        ...
        tl.store(out + tl.arange(0, M)[:, None] * N + tl.arange(0, N)[None, :],
                 tl.dot(a, b).to(tl.float16))
    ...
    def add_case(name, kernel, n_ptr_args, constants, args, out_index, out_elems, golden, compare):
        cases.append(dict(name=name, kernel=kernel, n_ptr_args=n_ptr_args,
                          constants=constants, args=args, out_index=out_index,
                          out_elems=out_elems,
                          golden=np.asarray(golden, np.float16).flatten(),
                          compare=compare))
    ...
    add_case("add_mul_n16", rpu_add_mul_kernel, 4, {4: n}, [None, a, b, c], 0, n,
             ((a.astype(np.float32) + b) * c).astype(np.float16), "tol")
    add_case("relu_n16", rpu_relu_kernel, 2, {2: n}, [None, a], 0, n,
             np.maximum(a, np.float16(0.0)), "exact")
    add_case("maximum_n16", rpu_maximum_kernel, 3, {3: n}, [None, a, b], 0, n,
             np.maximum(a, b), "exact")
    ...
    add_case("softmax_n16", rpu_softmax_kernel, 2, {2: n}, [None, _v256_pad(sx, "-inf")],
             0, n, (se / se.sum()).astype(np.float16), "tol")
    ...
    add_case("reduce_sum_axis0_16x16", rpu_reduce_sum_axis0_kernel, 2, {2: M, 3: N},
             [None, rx.flatten()], 0, M * N, np.tile(col_sums, M), "tol")
    add_case("reduce_sum_axis1_16x16", rpu_reduce_sum_axis1_kernel, 2, {2: M, 3: N},
             [None, rx.flatten()], 0, M * N, np.repeat(row_sums, N), "tol")
    ...
    add_case("broadcast_add_16x16", rpu_bcast_kernel, 3, {3: N, 4: K},
             [None, ba.flatten(), b_buf], 0, N * K, bcast_gold, "exact")
...
def _compile_case(case):
    ...
    kernel = triton.compile(
        src=src, target=GPUTarget("rpu", "rpu-v1", 1), options={"num_warps": 1}
    )
    elf = kernel.asm.get("rpuelf")
    if not elf:
        raise RuntimeError(f"{case['name']}: kernel.asm has no 'rpuelf' "
                           "(RPU_OUTPUT_ELF=1 did not take effect)")
    if not isinstance(elf, (bytes, bytearray)) or not elf.startswith(b"\x7fELF"):
        raise RuntimeError(f"{case['name']}: rpuelf is not an ELF (head={bytes(elf[:8])!r})")
    return bytes(elf)
...
def _compare(out_path, golden, n_elements, compare):
    ...
    if compare == "exact":
        ...
        if np.allclose(out.astype(np.float32), gold.astype(np.float32),
                       rtol=FP16_RTOL, atol=FP16_ATOL):
...
def main():
    ...
    parser.add_argument(
        "--require-board", action="store_true",
