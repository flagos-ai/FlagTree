...
def test_rpu_mlir_plan_pass_emits_rpuplan_kernel_for_gemm(tmp_path, monkeypatch):
    ...
    plan_json, printed = _run_rpu_plan_pass_on_mlir(
...
def test_rpu_compile_rectangular_gemm_kernels_produce_binary(tmp_path, monkeypatch, m, n, k):
    ...
    assert (
        f"rpu::LayoutTile<half, {k}, {n}, rpu::layout::physical_b> tile1"
        in kernel.asm["rpurc"]
    )
...
def _compile_rpu_gemm_kernel_with_rpuplan_override(
    tmp_path,
    monkeypatch,
    override_text,
    m=16,
    ...
    override_path = override_dir / "rpu_gemm_kernel.rpuplan"
    ...
    class _OverrideManager:
        ...
        def has_file(self, filename):
            ...
            return filename == "rpu_gemm_kernel.rpuplan"

        def get_file(self, filename):
            self.calls.append(("get", filename))
            if filename != "rpu_gemm_kernel.rpuplan":
                return None
...
def _compile_rpu_gemm_kernel_with_rpuexec_override(
    tmp_path,
    monkeypatch,
    override_text,
    m=16,
    ...
    override_dir = tmp_path / "override"
    override_dir.mkdir(exist_ok=True)
    override_path = override_dir / "rpu_gemm_kernel.rpuexec"
    override_path.write_text(override_text)
    ...
    class _OverrideManager:
        ...
        def has_file(self, filename):
            self.calls.append(("has", filename))
            return filename == "rpu_gemm_kernel.rpuexec"

        def get_file(self, filename):
            self.calls.append(("get", filename))
            if filename != "rpu_gemm_kernel.rpuexec":
                return None
            return str(override_path)
...
def _recognized_plan_from_kernel(kernel, tmp_path, monkeypatch):
    plan_json, _printed = _run_rpu_plan_pass_on_mlir(
        kernel.asm["ttir"], tmp_path, monkeypatch
    )
