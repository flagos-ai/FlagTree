...
def test_rpu_compile_softmax16_direct_opt_out_uses_plan_kernel(
    tmp_path, monkeypatch, n_elements, nvec
):
    monkeypatch.setenv("RPU_USE_EXECUTABLE_DIALECT", "0")
    kernel = _compile_rpu_softmax_kernel(
        tmp_path, monkeypatch, n_elements=n_elements
    )
...
def _compile_rpu_softmax_kernel_with_rpuplan_override(
    tmp_path,
    monkeypatch,
    override_text,
    n_elements=16,
    ...
    override_path = override_dir / "rpu_softmax_kernel.rpuplan"
    ...
    class _OverrideManager:
        ...
        def has_file(self, filename):
            ...
            return filename == "rpu_softmax_kernel.rpuplan"

        def get_file(self, filename):
            self.calls.append(("get", filename))
            if filename != "rpu_softmax_kernel.rpuplan":
                return None
...
def _compile_rpu_softmax_kernel_with_rpuexec_override(
    tmp_path,
    monkeypatch,
    override_text,
    n_elements=16,
    ...
    override_dir = tmp_path / "override"
    override_dir.mkdir(exist_ok=True)
    override_path = override_dir / "rpu_softmax_kernel.rpuexec"
    override_path.write_text(override_text)
    ...
    class _OverrideManager:
        ...
        def has_file(self, filename):
            self.calls.append(("has", filename))
            return filename == "rpu_softmax_kernel.rpuexec"

        def get_file(self, filename):
            self.calls.append(("get", filename))
            if filename != "rpu_softmax_kernel.rpuexec":
                return None
            return str(override_path)
