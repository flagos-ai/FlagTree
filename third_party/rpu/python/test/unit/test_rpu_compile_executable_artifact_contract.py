...
def test_rpu_compile_supported_executable_artifacts_parse_to_summary(
    ...
    cases = [
        ...
        (
            ...
            "rpu_gemm_kernel",
            "gemm",
            lambda case_tmp: _compile_rpu_gemm_kernel(
                case_tmp, monkeypatch, m=16, n=16, k=16
            ),
        ),
        (
            "softmax",
            "rpu_softmax_kernel",
            "softmax",
            lambda case_tmp: _compile_rpu_softmax_kernel(
                case_tmp, monkeypatch, n_elements=16
            ),
        ),
        (
            "convkxk",
            "rpu_convkxk_kernel",
            "convkxk",
            lambda case_tmp: _compile_rpu_convkxk_kernel(
                case_tmp, monkeypatch, kernel_size=3
            ),
        ),
        (
            "resnet_block",
            ...
            "resnet_block",
            lambda case_tmp: _compile_rpu_resnet_block_kernel(
                case_tmp, monkeypatch, pixels=16, channels=16, hidden_channels=16
            ),
        ),
        (
            "resnet50_bottleneck",
            ...
            "resnet50_bottleneck",
            lambda case_tmp: _compile_rpu_resnet50_bottleneck_kernel(
                case_tmp, monkeypatch
            ),
        ),
    ]
