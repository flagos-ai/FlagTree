...
class _FakeMLIRModule:
    ...
    def __str__(self):
        ...
        return (
            "module attributes {rpu.plan.json = \"...\"} {\n"
            "  tt.func public @rpu_add_kernel() {\n"
            "    tt.return\n"
            "  }\n"
    ...
    return json.dumps(
        {
            ...
            "kernel_name": "rpu_softmax_kernel",
            "layout": {"access": "linear", "memory": "contiguous_vector"},
            "mask": {"masked": False},
            "pattern": "softmax",
            "required_dsl_features": [
...
def _make_rpurc_from_rpuplan_bundle(compiler, bundle, metadata):
    ...
    assert isinstance(exec_result.value, compiler.RPUExecutableStageBundle)
    return compiler.RPUBackend.make_rpurc(exec_result.value, metadata)
...
def _install_triton_backend_stubs(monkeypatch):
    ...
    compiler_mod.StageResult = StageResult
...
def test_rpuplan_json_contains_signature_and_stable_schema(monkeypatch):
    ...
    assert isinstance(result, compiler.StageResult)
    assert isinstance(result.value, compiler.RPUPlanStageBundle)
    assert result.value.kernel_name == "rpu_add_kernel"
    assert result.value.pattern == "add"
    assert "_rpu_plan_module" not in metadata
    plan = json.loads(result.artifact)
...
def test_rpuplan_mlir_pass_path_reads_module_attr_without_regex(monkeypatch):
    ...
    assert isinstance(result, compiler.StageResult)
    assert result.artifact == plan_json
    assert isinstance(result.value, compiler.RPUPlanStageBundle)
    assert result.value.module is module
    assert result.value.kernel_name == "rpu_add_kernel"
    assert result.value.pattern == "add"
...
def test_rpuplan_metadata_comes_from_dialect_summary_not_json(monkeypatch):
    ...
    if hasattr(compiler, "RPUKernelPlan"):
        monkeypatch.setattr(
            ...
            "from_json",
            staticmethod(lambda payload: (_ for _ in ()).throw(AssertionError("JSON parser used for rpuplan metadata"))),
        )
    monkeypatch.setattr(
        _FakeRPU,
        "summary",
    ...
    assert json.loads(result.artifact)["kernel_name"] == "json_kernel"
    assert json.loads(result.artifact)["pattern"] == "json_pattern"
    assert result.value.kernel_name == "dialect_kernel"
    assert result.value.pattern == "add"
    assert result.value.module is module
...
def test_rpurc_emits_from_stage_bundle(monkeypatch):
    ...
    assert isinstance(plan_result.value, compiler.RPUPlanStageBundle)
    assert plan_result.value.module is module
    assert "_rpu_plan_module" not in metadata
    assert "_rpu_direct_rpurc_result" not in metadata
    assert "rpu_rpurc_source_kind" not in metadata
    assert ("rpurc_emit", "fail") not in [
        (event["stage"], event["event"]) for event in metadata["rpu_pipeline_events"]
    ]
    assert ("rpurc_emit", "rpu_plan_kernel") not in [
        (event["stage"], event["event"]) for event in metadata["rpu_pipeline_events"]
    ]

    rpurc = _make_rpurc_from_rpuplan_bundle(compiler, plan_result.value, metadata)
...
def test_rpurc_fails_when_recognized_pattern_has_no_direct_emitter(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="does not have a .* rpurc stage emitter"):
        _make_rpurc_from_rpuplan_bundle(compiler, plan_result.value, metadata)
...
def test_rpu_debug_bundle_records_success_path(tmp_path, monkeypatch):
    ...
    plan_result = compiler.RPUBackend.make_rpuplan(module, metadata)
    plan_json = plan_result.artifact
    ...
    assert ("rpurc_emit", "rpu_plan_kernel") not in [
        (event["stage"], event["event"]) for event in manifest_before_rpurc["pipeline_events"]
    ]
    rpurc = _make_rpurc_from_rpuplan_bundle(compiler, plan_result.value, metadata)
    rpuinstr = compiler.RPUBackend.make_rpuinstr(
        rpurc,
...
def test_rpu_backend_rpuexec_mlir_debug_artifact_for_plan_lowering_success(
    ...
    assert isinstance(result.value, compiler.RPUExecutableStageBundle)
    assert result.value.module is executable_module
    debug_bundle, bundle_dir, manifest = _debug_bundle(metadata)
    assert "rpu.exec.mlir" in _debug_files(bundle_dir)
    assert (bundle_dir / "rpu.exec.mlir").read_text() == str(executable_module)
    assert "rpuexec_mlir" in debug_bundle["files"]
...
def test_rpu_backend_legacy_direct_fallback_has_no_rpuexec_mlir_debug_artifact(
    ...
    assert isinstance(result.value, compiler.RPUExecutableStageBundle)
    assert result.value.source_kind == "rpu_plan_kernel"
    assert result.value.fallback_reason == "missing_executable_hooks"
    assert "rpu_debug_bundle" not in metadata


def test_rpu_backend_direct_executable_success_has_own_pipeline_events(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
    ...
    direct_events = [
        ...
        if event["stage"] == "direct_executable_lower"
    ]
    ...
    compatibility_events = [
        event["event"]
        for event in metadata["rpu_pipeline_events"]
        if event["stage"] == "rpuplan_recognize"
    ]
    assert compatibility_events == []


def test_rpu_backend_direct_success_uses_python_pass_pipeline_not_private_helper(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
    ...
    assert isinstance(result.value, compiler.RPUExecutableStageBundle)
    assert result.value.module is source_module
    assert result.value.source_kind == "rpu_executable"
    assert source_module.has_rpuexec_kernel is True
    assert source_module.has_rpuplan_kernel is False
    assert all(
        event["stage"] != "rpuplan_recognize"
        for event in metadata["rpu_pipeline_events"]
    )


def test_rpu_backend_direct_executable_skip_falls_back_to_recognizer(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
...
def test_rpu_backend_direct_executable_summary_failure_has_own_fail_event(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
...
def test_rpu_backend_direct_artifact_prefers_native_executable_ir(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
    ...
    monkeypatch.setattr(
        ...
        staticmethod(
            lambda module: (_ for _ in ()).throw(
                AssertionError("stale text compatibility serializer must not run")
            )
        ),
        raising=False,
    )
    ...
    assert result.artifact == str(source_module)
    assert "rpu_direct_executable_sidecar_serializer" not in metadata
    artifact_event = [
        event
        for event in metadata["rpu_pipeline_events"]
        if event["stage"] == "direct_executable_lower"
        and event["event"] == "artifact"
    ][0]
    assert artifact_event["detail"]["artifact_kind"] == "rpu_exec_mlir"


def test_rpu_backend_missing_native_serializer_does_not_block_direct_artifact(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
...
def test_rpu_debug_bundle_records_recognizer_failure_trace(tmp_path, monkeypatch):
    ...
    assert ("rpuplan_recognize", "fail") in [
        (event["stage"], event["event"]) for event in manifest["pipeline_events"]
    ]
...
def test_rpu_debug_bundle_records_rpuinstr_failure(tmp_path, monkeypatch):
    ...
    _bundle, bundle_dir, manifest = _debug_bundle(metadata)
    assert {"rpurc.rc", "clang_command.json", "clang_stdout.txt", "clang_stderr.txt", "manifest.json"} <= _debug_files(bundle_dir)
    ...
    assert str(captured["cwd"]) not in json.dumps(command)
    assert str(captured["source"]) not in json.dumps(command)
    assert ("rpuinstr_compile", "fail") in [
        (event["stage"], event["event"]) for event in manifest["pipeline_events"]
    ]
...
def test_rpu_backend_uses_json_artifact_and_direct_source_without_op_traversal():
    ...
    assert "StageResult(" in recognizer_body
...
def test_rpu_backend_json_usage_is_artifact_or_diagnostic_only():
    ...
    assert 'rpu.get_module_str_attr(mod, "rpu.plan.json")' in recognizer_body
    assert "artifact=plan_json" in recognizer_body
    assert "value=RPUPlanStageBundle(" in recognizer_body
...
def test_rpu_backend_typed_replay_seed_is_not_replay_path():
    compiler_source = (_rpu_backend_path() / "compiler.py").read_text()
    generic_compiler_source = (
        _repo_root() / "python" / "triton" / "compiler" / "compiler.py"
    ).read_text()
    ...
    mlir_artifact_body = _source_between(
        compiler_source,
        "    def _record_rpuplan_mlir_artifact(",
        "    @staticmethod\n    def _make_rpuplan_from_mlir_module",
    )
    ...
    forbidden_backend_helpers = [
        ...
        "load_" + "rpuplan_mlir",
        "read_" + "rpuplan_mlir",
        "replay_" + "rpuplan_mlir",
        "rpuplan_mlir" + "_to_rpurc",
        "emit_rpurc_" + "from_mlir_file",
        "make_rpurc_" + "from_mlir",
        "make_rpurc_" + "from_path",
        "make_rpurc_" + "from_json",
    ]
    for forbidden in forbidden_backend_helpers:
        assert forbidden not in compiler_source

    forbidden_generic_helpers = [
        "parse_" + "rpuplan_mlir",
        "load_" + "rpuplan_mlir",
        "replay_" + "rpuplan_mlir",
        "rpuplan_mlir" + "_to_rpurc",
    ]
    for forbidden in forbidden_generic_helpers:
        assert forbidden not in generic_compiler_source

    assert "isinstance(src, RPUPlanStageBundle)" in make_rpuexec_body
    assert "isinstance(src, RPUExecutableStageBundle)" in make_rpurc_body
    assert "def _make_rpuplan_stage_bundle_from_mlir_file(" in compiler_source
    replay_helper_body = _source_between(
        compiler_source,
        "    def _parse_typed_replay_mlir_file(",
        "    @staticmethod\n    def make_rpuexec",
    )
    assert "ir.parse_mlir_module(str(path), context)" in replay_helper_body
    assert "RPUPlanStageBundle(" in replay_helper_body
    assert "RPUBackend._debug_bundle_root() is None" in mlir_artifact_body
    assert "str(mod)" in mlir_artifact_body
    assert "rpu.plan.mlir" in mlir_artifact_body
    assert "rpu.plan.mlir" not in make_rpuexec_body
    assert "rpu.plan.mlir" not in make_rpurc_body
    assert "rpu.plan.mlir" not in generic_compiler_source


def test_rpu_backend_replays_mlir_file_into_stage_bundle(tmp_path, monkeypatch):
    _install_triton_backend_stubs(monkeypatch)
    import compiler

    mlir_path = tmp_path / "rpu.plan.mlir"
    mlir_path.write_text(
        "module {\n"
        "  tt.func public @rpu_add_kernel() { tt.return }\n"
        "  rpu_plan.kernel @rpu_add_kernel_plan attributes {pattern = \"add\"}\n"
        "}\n"
    )
    metadata = {}
    ...
    bundle = compiler.RPUBackend._make_rpuplan_stage_bundle_from_mlir_file(
        mlir_path,
        context,
        metadata,
    )

    assert isinstance(bundle, compiler.RPUPlanStageBundle)
    assert bundle.module is _FakeIR.parsed_modules[-1]
    assert bundle.module.context is context
    assert bundle.kernel_name == "rpu_add_kernel"
    assert bundle.pattern == "add"
    assert [event["event"] for event in metadata["rpu_pipeline_events"]] == ["start", "end"]
    assert metadata["rpu_pipeline_events"][0]["stage"] == "rpuplan_replay"
    assert metadata["rpu_pipeline_events"][0]["detail"]["path"] == str(mlir_path)

    rpurc = _make_rpurc_from_rpuplan_bundle(compiler, bundle, metadata)
    assert "__rprog__ void rpu_add_kernel()" in rpurc


def test_rpu_backend_replay_records_parse_failure(tmp_path, monkeypatch):
    _install_triton_backend_stubs(monkeypatch)
    import compiler

    mlir_path = tmp_path / "rpu.plan.mlir"
    mlir_path.write_text("not mlir")
    _FakeIR.parse_error = "parse exploded"
    metadata = {}

    with pytest.raises(RuntimeError, match="failed to parse rpu.plan.mlir"):
        compiler.RPUBackend._make_rpuplan_stage_bundle_from_mlir_file(
            mlir_path,
            object(),
            metadata,
        )
    ...
    fail = metadata["rpu_pipeline_events"][-1]
    assert fail["stage"] == "rpuplan_replay"
    assert fail["event"] == "fail"
    assert fail["detail"]["path"] == str(mlir_path)
    assert "parse exploded" in fail["detail"]["error"]


def test_rpu_backend_replay_records_invalid_kernel_failure(tmp_path, monkeypatch):
    _install_triton_backend_stubs(monkeypatch)
    import compiler

    mlir_path = tmp_path / "rpu.plan.mlir"
    mlir_path.write_text("module { tt.func public @rpu_add_kernel() { tt.return } }\n")
    metadata = {}

    with pytest.raises(RuntimeError, match="failed to validate rpu_plan.kernel"):
        compiler.RPUBackend._make_rpuplan_stage_bundle_from_mlir_file(
            mlir_path,
            object(),
            metadata,
...
def test_rpu_backend_removes_transient_stage_metadata_bridge():
    ...
    forbidden = [
        ...
        if key.startswith("_rpu_plan_") or "module" in key
    ]
    assert forbidden == []


@pytest.mark.skip(reason="v3.6 inlined the _split_stage_result/_store_stage_artifact/_apply_stage_override/_update_compile_module_after_stage helpers; the same StageResult dispatch contract is now covered end-to-end by the StageResult dispatch integration tests")
def test_stage_result_split_helper_distinguishes_value_and_artifact(monkeypatch):
    backend_compiler, triton_compiler, _driver_mod = _load_generic_compiler_with_stage_stubs(monkeypatch)
    StageResult = backend_compiler.StageResult

    normal = object()
    assert triton_compiler._split_stage_result(normal) == (normal, normal, False)

    value = object()
    artifact = "artifact text"
    assert triton_compiler._split_stage_result(StageResult(value, artifact)) == (
        value,
        artifact,
        True,
    )
...
@pytest.mark.skip(reason="v3.6 inlined the _split_stage_result/_store_stage_artifact/_apply_stage_override/_update_compile_module_after_stage helpers; the same StageResult dispatch contract is now covered end-to-end by the StageResult dispatch integration tests")
def test_stage_result_normal_stage_keeps_existing_value_artifact_identity(monkeypatch):
    _backend_compiler, triton_compiler, driver_mod = _load_generic_compiler_with_stage_stubs(monkeypatch)
    ...
    class _Spec:
        def update_compile_module_after_stage(self, module, next_value, ext):
            calls.append((module, next_value, ext))
            return next_value
    ...
    class _CacheManager:
        def put(self, artifact, filename):
            cache_puts.append((artifact, filename))
            return f"/cache/{filename}"

    class _DumpManager:
        def put(self, artifact, filename):
            dump_puts.append((artifact, filename))

    normal = object()
    next_value, artifact, split_stage = triton_compiler._split_stage_result(normal)
    metadata_group = {}

    monkeypatch.setattr(driver_mod.driver, "active", _ActiveDriver())

    assert split_stage is False
    assert next_value is normal
    assert artifact is normal
    triton_compiler._store_stage_artifact(
        metadata_group,
        _CacheManager(),
        _DumpManager(),
        "kernel.ttir",
    ...
    assert metadata_group == {"kernel.ttir": "/cache/kernel.ttir"}
    assert cache_puts == [(normal, "kernel.ttir")]
    assert dump_puts == [(normal, "kernel.ttir")]
    assert triton_compiler._update_compile_module_after_stage("prev", next_value, "ttir") is normal
    assert calls == [("prev", normal, "ttir")]


@pytest.mark.skip(reason="v3.6 inlined the _split_stage_result/_store_stage_artifact/_apply_stage_override/_update_compile_module_after_stage helpers; the same StageResult dispatch contract is now covered end-to-end by the StageResult dispatch integration tests")
def test_stage_result_update_hook_receives_value_not_artifact(monkeypatch):
    backend_compiler, triton_compiler, driver_mod = _load_generic_compiler_with_stage_stubs(monkeypatch)
    StageResult = backend_compiler.StageResult
    ...
    value = object()
    artifact = "rpu.plan.json text"
    next_value, stage_artifact, split_stage = triton_compiler._split_stage_result(
        StageResult(value, artifact)
    )

    monkeypatch.setattr(driver_mod.driver, "active", _ActiveDriver())

    assert split_stage is True
    assert stage_artifact == artifact
    metadata_group = {}
    cache_puts = []
    ...
    assert metadata_group == {"kernel.rpuplan": "/cache/kernel.rpuplan"}
    assert cache_puts == [("rpu.plan.json text", "kernel.rpuplan")]
    assert triton_compiler._update_compile_module_after_stage("prev", next_value, "rpuplan") is value
    assert calls == [("prev", value, "rpuplan")]


@pytest.mark.skip(reason="v3.6 inlined the _split_stage_result/_store_stage_artifact/_apply_stage_override/_update_compile_module_after_stage helpers; the same StageResult dispatch contract is now covered end-to-end by the StageResult dispatch integration tests")
def test_stage_result_split_stage_override_rejects_before_parse_or_hook(monkeypatch):
    backend_compiler, triton_compiler, driver_mod = _load_generic_compiler_with_stage_stubs(monkeypatch)
    StageResult = backend_compiler.StageResult

    class _OverrideManager:
        def has_file(self, filename):
            assert filename == "kernel.rpuplan"
            return True

        def get_file(self, filename):
            raise AssertionError("split-stage override must fail before reading override file")

    class _Spec:
        def update_compile_module_after_stage(self, module, next_value, ext):
            raise AssertionError("split-stage override must fail before update hook")

    class _ActiveDriver:
        spec = _Spec()

    next_value, artifact, split_stage = triton_compiler._split_stage_result(
        StageResult(value=object(), artifact="rpu.plan.json text")
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("split-stage override must fail before parse")
        ),
    )

    with pytest.raises(RuntimeError, match="Stage override for split artifact kernel\\.rpuplan is not supported"):
        triton_compiler._apply_stage_override(
            _OverrideManager(),
            "kernel.rpuplan",
            "rpuplan",
            context=object(),
    ...
    assert split_stage is True
    assert artifact == "rpu.plan.json text"
    assert next_value is not artifact


@pytest.mark.skip(reason="v3.6 inlined the _split_stage_result/_store_stage_artifact/_apply_stage_override/_update_compile_module_after_stage helpers; the same StageResult dispatch contract is now covered end-to-end by the StageResult dispatch integration tests")
def test_stage_result_split_stage_override_uses_backend_hook(monkeypatch):
    backend_compiler, triton_compiler, _driver_mod = _load_generic_compiler_with_stage_stubs(monkeypatch)
    StageResult = backend_compiler.StageResult
    ...
    class _OverrideManager:
        ...
        def get_file(self, filename):
            assert filename == "kernel.rpuplan"
            return "/override/kernel.rpuplan"

    class _Backend:
        def __init__(self):
            self.calls = []

        def parse_split_stage_override(self, full_name, ext, context, metadata):
            self.calls.append((full_name, ext, context, metadata))
            return "typed override value"

    metadata = {}
    context = object()
    backend = _Backend()
    _next_value, _artifact, split_stage = triton_compiler._split_stage_result(
        StageResult(value=object(), artifact="rpu.plan.json text")
    )
    monkeypatch.setattr(
        triton_compiler,
        "parse",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("split-stage backend hook must bypass generic parse")
        ),
    )

    has_override, override_value = triton_compiler._apply_stage_override(
        _OverrideManager(),
        "kernel.rpuplan",
        "rpuplan",
        context=context,
    ...
    assert override_value == "typed override value"
    assert backend.calls == [("/override/kernel.rpuplan", "rpuplan", context, metadata)]


@pytest.mark.skip(reason="v3.6 inlined the _split_stage_result/_store_stage_artifact/_apply_stage_override/_update_compile_module_after_stage helpers; the same StageResult dispatch contract is now covered end-to-end by the StageResult dispatch integration tests")
def test_stage_result_split_stage_override_rejects_when_backend_hook_declines(monkeypatch):
    backend_compiler, triton_compiler, _driver_mod = _load_generic_compiler_with_stage_stubs(monkeypatch)
    StageResult = backend_compiler.StageResult
...
@pytest.mark.skip(reason="v3.6 inlined the _split_stage_result/_store_stage_artifact/_apply_stage_override/_update_compile_module_after_stage helpers; the same StageResult dispatch contract is now covered end-to-end by the StageResult dispatch integration tests")
def test_generic_compiler_split_stage_override_replaces_next_value_not_artifact(tmp_path, monkeypatch):
    backend_compiler, triton_compiler, driver_mod = _load_generic_compiler_with_stage_stubs(monkeypatch)
    StageResult = backend_compiler.StageResult
    GPUTarget = backend_compiler.GPUTarget
    ...
    cache_dir.mkdir()
    override_path = tmp_path / "kernel.rpuplan"
    override_path.write_text("module { rpu_plan.kernel @rpu_add_kernel_plan }\n")
    fresh_bundle = object()
    override_bundle = object()
    context = object()
    ...
    class _CacheManager:
        ...
        def put(self, artifact, filename, binary=True):
    ...
    class _OverrideManager:
        def has_file(self, filename):
            return filename == "kernel.rpuplan"
    ...
    class _Spec:
        def update_compile_module_after_stage(self, module, next_value, ext):
            update_calls.append((module, next_value, ext))
            return next_value
    ...
    class _Backend:
        ...
        def add_stages(self, stages, options):
            ...
            def make_rpuplan(src, metadata):
                stage_inputs.append(("rpuplan", src))
                return StageResult(value=fresh_bundle, artifact="fresh rpu.plan.json")

            def make_rpurc(src, metadata):
                stage_inputs.append(("rpurc", src))
                return "rc from override"

            stages["ttir"] = make_ttir
            stages["rpuplan"] = make_rpuplan
            stages["rpurc"] = make_rpurc
            stages["rpuinstr"] = lambda src, metadata: "instr"
            stages["rpubin"] = lambda src, metadata: b"bin"
        ...
        def parse_split_stage_override(self, full_name, ext, context_arg, metadata):
            override_calls.append((full_name, ext, context_arg))
    ...
    kernel.cache_key = "d18-cache-key"
    cache_manager = _CacheManager()
    monkeypatch.setattr(triton_compiler, "triton_key", lambda: "d18-triton")
    monkeypatch.setattr(triton_compiler, "get_cache_manager", lambda key: cache_manager)
    monkeypatch.setattr(triton_compiler, "get_dump_manager", lambda key: None)
    monkeypatch.setattr(triton_compiler, "get_override_manager", lambda key: _OverrideManager())
    monkeypatch.setattr(triton_compiler, "ast_to_ttir", lambda *args, **kwargs: "ast-ttir")
    triton_compiler.ir.context = lambda: context
    triton_compiler.ir.load_dialects = lambda context_arg: None
    triton_compiler.backends["d18"] = types.SimpleNamespace(compiler=_Backend)
    monkeypatch.setattr(driver_mod.driver, "active", _ActiveDriver())
    monkeypatch.setenv("TRITON_KERNEL_OVERRIDE", "1")
    monkeypatch.setenv("TRITON_ALWAYS_COMPILE", "1")

    src = triton_compiler.ASTSource(kernel, "fp16, fp16")
    triton_compiler.compile(src, target=GPUTarget("d18-rpu", "rpu-v1", 1), options={})

    assert ("fresh rpu.plan.json", "kernel.rpuplan") in cache_puts
    assert ("rpurc", override_bundle) in stage_inputs
    assert ("rpurc", fresh_bundle) not in stage_inputs
    assert any(next_value is override_bundle and ext == "rpuplan" for _, next_value, ext in update_calls)
    assert override_calls == [(str(override_path), "rpuplan", context)]
    assert cache_manager.group["kernel.rpuplan"].endswith("kernel.rpuplan")
    assert Path(cache_manager.group["kernel.rpuplan"]).read_text() == "fresh rpu.plan.json"


def test_rpu_backend_parse_split_stage_override_uses_typed_replay(tmp_path, monkeypatch):
    _install_triton_backend_stubs(monkeypatch)
    import compiler

    mlir_path = tmp_path / "kernel.rpuplan"
    mlir_path.write_text(
        "module {\n"
        "  tt.func public @rpu_add_kernel() { tt.return }\n"
        "  rpu_plan.kernel @rpu_add_kernel_plan attributes {pattern = \"add\"}\n"
    ...
    metadata = {}
    context = object()
    backend = compiler.RPUBackend(compiler.GPUTarget("rpu", "rpu-v1", 1))

    bundle = backend.parse_split_stage_override(str(mlir_path), "rpuplan", context, metadata).value
    ...
    assert _make_rpurc_from_rpuplan_bundle(compiler, bundle, metadata).startswith('#include "rpu_tile.h"')


def test_rpu_backend_native_exec_rpuplan_override_replays_as_executable_bundle(
    ...
    assert isinstance(bundle, compiler.RPUExecutableStageBundle)
    assert bundle.module is _FakeIR.parsed_modules[-1]
    assert bundle.kernel_name == "rpu_add_kernel"
    assert bundle.pattern == "add"
    assert bundle.source_kind == "rpu_executable"
    assert bundle.fallback_reason is None
    assert [
        (event["stage"], event["event"])
        for event in metadata["rpu_pipeline_events"]
    ] == [
        ("rpuexec_replay", "start"),
        ("rpuexec_replay", "end"),
    ]


def test_rpu_backend_native_exec_rpuplan_override_records_debug_artifact(
    tmp_path, monkeypatch
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
    ...
    monkeypatch.setenv("RPU_DEBUG_ARTIFACT_DIR", str(debug_root))
    mlir_path = tmp_path / "kernel.rpuplan"
    mlir_path.write_text(
        "module {\n"
        "  rpu.kernel @rpu_add_kernel() {\n"
    ...
    bundle = backend.parse_split_stage_override(str(mlir_path), "rpuplan", object(), metadata).value

    assert isinstance(bundle, compiler.RPUExecutableStageBundle)
    debug_bundle = metadata["rpu_debug_bundle"]
    bundle_dir = Path(debug_bundle["dir"])
    manifest = json.loads(Path(debug_bundle["manifest"]).read_text())
    assert "rpuexec_override_mlir" in debug_bundle["files"]
    override_info = debug_bundle["files"]["rpuexec_override_mlir"]
    override_path = Path(override_info["path"])
    assert override_path.name == "rpu.exec.override.mlir"
    assert "rpu.kernel @rpu_add_kernel" in override_path.read_text()
    assert manifest["files"]["rpuexec_override_mlir"] == override_info
    assert "rpu.exec.override.mlir" in _debug_files(bundle_dir)
    assert "rpuplan_override_mlir" not in debug_bundle["files"]


def test_rpu_backend_explicit_rpuexec_override_replays_executable_bundle(
    tmp_path, monkeypatch
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler

    mlir_path = tmp_path / "kernel.rpuexec"
    mlir_path.write_text(
        "module {\n"
        "  rpu.kernel @rpu_add_kernel() {\n"
        "    rpu.return\n"
    ...
    bundle = backend.parse_split_stage_override(str(mlir_path), "rpuexec", context, metadata).value
...
def test_rpu_backend_explicit_rpuexec_override_rejects_plan_kernel(
    tmp_path, monkeypatch
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
    ...
    with pytest.raises(
        RuntimeError,
        match="RPU executable replay failed to validate rpu.kernel",
    ):
        backend.parse_split_stage_override(str(mlir_path), "rpuexec", object(), metadata)
...
def test_rpu_backend_rpurc_consumes_lowered_stage_bundle():
    ...
    assert "isinstance(src, RPUPlanStageBundle)" in make_rpuexec_body
    assert "RPUBackend._make_rpuexec_stage_result_from_module" in make_rpuexec_body
...
def test_rpu_backend_executable_rpurc_route_ignores_direct_helper(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "_emit_rpurc_from_executable",
        lambda module: (_ for _ in ()).throw(
            AssertionError("direct executable helper must not run")
        ),
        raising=False,
    )
    metadata = {}
...
def test_rpu_backend_make_rpuexec_returns_executable_bundle(monkeypatch):
    ...
    monkeypatch.setattr(compiler.rpu, "_rpuplan_supports_executable_lowering", lambda module: module is plan_module, raising=False)
    monkeypatch.setattr(compiler.rpu, "_lower_rpuplan_to_executable_module", lambda module: executable_module, raising=False)
    monkeypatch.setattr(
        compiler.rpu,
        "_get_rpuexec_kernel_summary",
        lambda module: {"kernel_name": "rpu_add_kernel", "pattern": "add"},
    ...
    assert result.value.pattern == "add"
    assert result.value.source_kind == "rpu_executable"
    assert result.value.fallback_reason is None
    assert "rpu.kernel" in result.artifact
    assert 'kind = "add"' in result.artifact
...
def test_rpu_backend_direct_candidate_patterns_use_cpp_kind_contract(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
...
def test_rpu_backend_make_rpuplan_returns_executable_bundle_for_direct_supported(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
...
def test_rpu_backend_make_rpuexec_passes_through_executable_bundle(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
    ...
    monkeypatch.setattr(
        ...
        "_lower_rpuplan_to_executable_module",
        lambda module: (_ for _ in ()).throw(
            AssertionError("plan-owned lowering should not run")
        ),
        raising=False,
    )
    monkeypatch.setattr(
    ...
    assert result.value is bundle
    assert result.artifact == str(executable_module)
...
def test_rpu_backend_supported_direct_stage_chain_is_executable_trunk(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "_lower_rpuplan_to_executable_module",
        lambda module: (_ for _ in ()).throw(
            AssertionError("plan-to-executable lowering must not run")
        ),
        raising=False,
    )
    monkeypatch.setattr(
        ...
        lambda module: (_ for _ in ()).throw(
            AssertionError("plan rc emission must not run")
        ),
        raising=False,
    )
    monkeypatch.setattr(
    ...
    metadata = {}
    plan_result = compiler.RPUBackend._make_rpuplan_from_mlir_module(
        source_module, metadata
    )
    exec_result = compiler.RPUBackend.make_rpuexec(plan_result.value, metadata)
    rc_result = compiler.RPUBackend.make_rpurc(exec_result.value, metadata)

    assert isinstance(plan_result.value, compiler.RPUExecutableStageBundle)
    assert source_module.ran_pm.added == ["lower_supported_ttir_to_executable"]
    assert plan_result.value.module is source_module
    assert plan_result.artifact == str(source_module)
    assert exec_result.value is plan_result.value
    assert exec_result.artifact == str(source_module)
...
def test_rpu_backend_add_direct_artifact_skips_rpuplan_recognizer(monkeypatch):
    ...
    monkeypatch.setattr(
        compiler.rpu.passes,
        "add_recognize_plan",
        lambda pm: (_ for _ in ()).throw(
            AssertionError("rpu_plan recognizer must not run for executable Add sidecar")
        ),
    )
    metadata = {}
    ...
    assert any(
        event["stage"] == "direct_executable_lower"
        and event["event"] == "artifact"
        and event["detail"]["status"] == "available"
        and event["detail"]["artifact_kind"] == "rpu_exec_mlir"
        for event in metadata["rpu_pipeline_events"]
    )
...
def test_rpu_backend_direct_artifact_branch_has_no_fallback_to_rpuplan(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "add_recognize_plan",
        lambda pm: (_ for _ in ()).throw(
            AssertionError("rpu_plan recognizer must not recover direct sidecar failure")
        ),
    )
    metadata = {}
...
def test_rpu_backend_direct_executable_artifact_is_native_exec_mlir(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
...
def test_rpu_backend_missing_sidecar_serializer_does_not_block_direct_artifact(
    monkeypatch,
):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
...
def test_rpu_backend_direct_stage_result_uses_summary_factory(monkeypatch):
    ...
    assert isinstance(result.value, compiler.RPUExecutableStageBundle)
    assert result.value.module is source_module
    assert result.value.kernel_name == "rpu_custom_kernel"
    assert result.value.pattern == "gemm"
    assert result.value.source_kind == "rpu_executable"
    assert result.artifact == str(source_module)


def test_rpu_backend_executable_stage_result_factory_is_summary_verified():
    source = (_repo_root() / "third_party" / "rpu" / "backend" / "compiler.py").read_text()
    factory_body = _source_between(
        source,
        "    def _make_rpuexec_stage_result_from_module(",
        "    @staticmethod\n    def _try_make_direct_executable_stage_from_ttir",
    )
    direct_helper_body = _source_between(
        source,
    ...
    override_body = _source_between(
        source,
        "    def _try_make_rpuexec_stage_bundle_from_override_module(",
        "    @staticmethod\n    def _make_split_stage_bundle_from_mlir_file",
    )
    make_rpuexec_body = _source_between(
        source,
        "    def make_rpuexec(",
    ...
    assert "exec_summary = RPUBackend._get_rpuexec_kernel_summary(mod)" in factory_body
    assert "value=RPUExecutableStageBundle(" in factory_body
    assert 'source_kind="rpu_executable"' in factory_body
    assert "RPUBackend._make_rpuexec_stage_result_from_module" in direct_helper_body
    assert "direct_executable_module" in direct_helper_body
    assert "RPUExecutableStageBundle(" not in direct_helper_body
    assert "RPUBackend._make_rpuexec_stage_result_from_module(mod)" in override_body
    assert "RPUBackend._make_rpuexec_stage_result_from_module" in make_rpuexec_body
    assert "exec_module" in make_rpuexec_body
...
def test_rpu_backend_direct_pass_pipeline_source_has_no_sidecar_serializer():
    ...
    assert "direct_executable_artifact = str(direct_executable_module)" in direct_pass_body
    assert "return direct_stage" in direct_pass_body
...
def test_rpu_backend_executable_first_pass_pipeline_skips_recognizer(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "add_recognize_plan",
        lambda pm: (_ for _ in ()).throw(
            AssertionError("rpu_plan recognizer must not run from direct pass pipeline")
        ),
    )
    metadata = {}
...
def test_rpu_backend_gemm_direct_artifact_skips_rpuplan_recognizer(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "add_recognize_plan",
        lambda pm: (_ for _ in ()).throw(
            AssertionError("rpu_plan recognizer must not run for executable GEMM sidecar")
        ),
    )
    metadata = {}
...
def test_rpu_backend_softmax_direct_artifact_skips_rpuplan_recognizer(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "add_recognize_plan",
        lambda pm: (_ for _ in ()).throw(
            AssertionError("rpu_plan recognizer must not run for executable Softmax sidecar")
        ),
    )
    metadata = {}
    ...
    assert source_module.ran_pm.added == ["lower_supported_ttir_to_executable"]
    assert result.value.module is source_module
    assert result.value.pattern == "softmax"
    assert result.artifact == str(source_module)
    assert source_module.has_rpuplan_kernel is False
    assert "rpuplan_artifact_source_kind" not in metadata
...
def test_rpu_backend_convkxk_direct_artifact_skips_rpuplan_recognizer(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "add_recognize_plan",
        lambda pm: (_ for _ in ()).throw(
            AssertionError("rpu_plan recognizer must not run for executable ConvKxK sidecar")
        ),
    )
    metadata = {}
    ...
    assert source_module.ran_pm.added == ["lower_supported_ttir_to_executable"]
    assert result.value.module is source_module
    assert result.value.pattern == "convkxk"
    assert result.artifact == str(source_module)
    assert source_module.has_rpuplan_kernel is False
    assert "rpuplan_artifact_source_kind" not in metadata
...
def test_rpu_backend_residual_direct_artifact_skips_rpuplan_recognizer(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "add_recognize_plan",
        lambda pm: (_ for _ in ()).throw(
            AssertionError("rpu_plan recognizer must not run for executable residual sidecar")
        ),
    )
    metadata = {}
    ...
    assert source_module.ran_pm.added == ["lower_supported_ttir_to_executable"]
    assert result.value.module is source_module
    assert result.value.pattern == "resnet_block"
    assert result.artifact == str(source_module)
    assert source_module.has_rpuplan_kernel is False
    assert "rpuplan_artifact_source_kind" not in metadata
...
def test_rpu_backend_bottleneck_direct_artifact_skips_rpuplan_recognizer(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "add_recognize_plan",
        lambda pm: (_ for _ in ()).throw(
            AssertionError("rpu_plan recognizer must not run for executable bottleneck sidecar")
        ),
    )
    metadata = {}
    ...
    assert source_module.ran_pm.added == ["lower_supported_ttir_to_executable"]
    assert result.value.module is source_module
    assert result.value.pattern == "resnet50_bottleneck"
    assert result.artifact == str(source_module)
    assert source_module.has_rpuplan_kernel is False
    assert "rpuplan_artifact_source_kind" not in metadata
...
def test_rpu_backend_make_rpuexec_prefers_direct_candidate(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "_rpuplan_supports_executable_lowering",
        lambda module: (_ for _ in ()).throw(
            AssertionError("plan support query should not run")
        ),
        raising=False,
    )
    monkeypatch.setattr(
    ...
    assert result.value.module is direct_executable_module
    assert result.value.source_kind == "rpu_executable"
    assert result.value.fallback_reason is None
    assert metadata["rpu_rpuexec_source_kind"] == "rpu_executable"
    event = metadata["rpu_pipeline_events"][-1]
...
def test_rpu_backend_make_rpuexec_uses_executable_summary(monkeypatch):
    ...
    assert result.value.module is executable_module
    assert result.value.kernel_name == "exec_kernel"
    assert result.value.pattern == "softmax"
...
def test_rpu_backend_missing_summary_hook_falls_back(monkeypatch):
    ...
    assert result.value.module is plan_module
    assert result.value.source_kind == "rpu_plan_kernel"
    assert result.value.fallback_reason == "missing_executable_hooks"
    assert "reason: missing_executable_hooks" in result.artifact
...
def test_rpu_backend_make_rpuexec_direct_opt_out_returns_plan_bundle(monkeypatch):
    ...
    assert result.value.module is plan_module
    assert result.value.source_kind == "rpu_plan_kernel"
    assert result.value.fallback_reason == "env_opt_out"
    assert "source_kind: rpu_plan_kernel" in result.artifact
    assert "reason: env_opt_out" in result.artifact
    assert metadata["rpu_rpuexec_source_kind"] == "rpu_plan_kernel"
...
def test_rpu_backend_direct_fallback_requires_direct_hook(monkeypatch):
    ...
    assert metadata["rpu_pipeline_events"][-1]["event"] == "fail"
    assert (
        metadata["rpu_pipeline_events"][-1]["detail"]["fallback_reason"]
        == "env_opt_out"
    )
...
def test_rpu_backend_builder_fallback_detail_in_make_rpuexec(monkeypatch):
    ...
    metadata = {}
    detail = (
        "add requires unmasked contiguous linear f16 vector attrs with out=0 "
    ...
    assert isinstance(result.value, compiler.RPUExecutableStageBundle)
    assert result.value.source_kind == "rpu_plan_kernel"
    assert result.value.fallback_reason == "unsupported_executable_shape"
    assert "reason: unsupported_executable_shape" in result.artifact
    assert f"detail: {detail}" in result.artifact
    event = metadata["rpu_pipeline_events"][-1]
    assert event["stage"] == "rpuexec_lower"
    assert event["event"] == "rpu_plan_kernel"
...
def test_rpu_backend_split_stage_override_records_debug_artifact(tmp_path, monkeypatch):
    _install_triton_backend_stubs(monkeypatch)
    import compiler
    ...
    bundle_dir = Path(debug_bundle["dir"])
    manifest = json.loads(Path(debug_bundle["manifest"]).read_text())
    assert "rpuplan_override_mlir" in debug_bundle["files"]
    override_info = debug_bundle["files"]["rpuplan_override_mlir"]
    override_path = Path(override_info["path"])
    assert override_path.name == "rpu.plan.override.mlir"
    assert "rpu_plan.kernel @rpu_add_kernel_plan" in override_path.read_text()
    assert manifest["files"]["rpuplan_override_mlir"] == override_info
    assert "rpu.plan.override.mlir" in _debug_files(bundle_dir)


def test_rpu_backend_parse_split_stage_override_declines_non_rpuplan(monkeypatch):
    _install_triton_backend_stubs(monkeypatch)
    import compiler

    backend = compiler.RPUBackend(compiler.GPUTarget("rpu", "rpu-v1", 1))
    with pytest.raises(RuntimeError, match="only 'rpuexec' and 'rpuplan' are split stages"):
        backend.parse_split_stage_override("/tmp/kernel.ttir", "ttir", object(), {})


def test_generic_compiler_stage_result_contract_source_boundary():
    compiler_source = (_repo_root() / "python" / "triton" / "compiler" / "compiler.py").read_text()
    backend_source = (_repo_root() / "python" / "triton" / "backends" / "compiler.py").read_text()

    assert "class StageResult" in backend_source
    assert "isinstance(result, StageResult)" in compiler_source
    assert "result.value, result.artifact, True" in compiler_source
    assert "fn_cache_manager.put(artifact, ir_filename)" in compiler_source
    assert "fn_dump_manager.put(artifact, ir_filename)" in compiler_source
    assert "parse_split_stage_override" in compiler_source
    assert "is not supported" in compiler_source


def test_generic_compiler_json_usage_is_cache_metadata_not_rpu_lowering():
    compiler_source = (_repo_root() / "python" / "triton" / "compiler" / "compiler.py").read_text()

    assert "metadata = json.loads(metadata_path.read_text())" in compiler_source
    assert "json.dumps(metadata, default=vars)" in compiler_source
    assert "StageResult" in compiler_source
    assert "fn_cache_manager.put(artifact, ir_filename)" in compiler_source
    assert "rpu.plan.json" not in compiler_source
    assert "RPUPlanStageBundle" not in compiler_source
    assert "RPUKernelPlan" not in compiler_source
...
def test_rpu_direct_op_consumer_does_not_expose_broad_python_binding():
    ...
    suspicious_extra_bindings = [
        name
        for name in sorted(exported_bindings - required_bindings)
        if (
            name.startswith(("_get_rpuplan", "get_rpuplan", "walk_", "parse_"))
            or name.startswith(("rpuplan_", "emit_rpurc_"))
            or (
                any(token in name for token in ("attr", "op", "operand", "region"))
                and name != "get_module_str_attr"
            )
        )
    ]
