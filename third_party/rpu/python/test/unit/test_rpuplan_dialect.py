...
def test_rpuplan_kernel_verifier_rejects_invalid_plans(
    case, diagnostic, tmp_path, monkeypatch, capfd
):
    ir, context = _load_rpuplan_context(monkeypatch)
...
def test_rpuplan_trace_softmax_failure_anchor_allows_reduce_region():
    source_path = (
        _repo_root()
        / "third_party/rpu/lib/RPUTransforms/RPUTTIRPatternMatcher.cpp"
    )
    source = source_path.read_text()
    start = source.index("bool isAllowedSoftmaxTopLevelOp(")
    end = source.index("TraceAnchor rankedFailureAnchor", start)
...
def test_rpuplan_kernel_summary_reads_identity_attrs_without_dto_conversion(
