...
def test_rpu_mlir_plan_pass_emits_rpuplan_kernel_for_add(tmp_path, monkeypatch):
    ...
    plan_json, _trace_json, printed = _run_rpu_plan_pass_on_mlir(
        kernel.asm["ttir"], tmp_path, monkeypatch
    )
...
@pytest.mark.skip(reason="v3.6 raised the elementwise1D threshold from 128 to 256 so n=256 now goes through single-shot load_contig<256> rather than the chunked tile_frame path this test asserts.")
...
def test_rpu_compile_add_kernel_handles_masked_tail(tmp_path, monkeypatch):
    kernel = _compile_rpu_masked_add_kernel(
        tmp_path, monkeypatch, logical_n=17, block_size=32
    )
...
def test_rpu_compile_masked_add_variant_uses_executable_source(
...
def test_rpu_compile_add_defaults_to_executable_for_canonical_shape(
    tmp_path, monkeypatch
):
...
def test_rpu_compile_add_direct_opt_out_uses_plan_kernel(
    tmp_path, monkeypatch, n_elements
):
...
@pytest.mark.skip(reason="v3.6 raised the elementwise1D threshold from 128 to 256 so n=256 routes to kind='generic' single-shot rather than the kind='add' chunked path this test asserts.")
...
def test_rpu_compile_add_emits_elf_when_requested(tmp_path, monkeypatch):
    ...
    elf = kernel.asm["rpuelf"]
    assert isinstance(elf, bytes)
...
def test_rpu_compile_mul_add_defaults_to_generic_executable(
    tmp_path, monkeypatch
):
...
def test_rpu_compile_masked_mul_add_defaults_to_generic_executable(
    tmp_path, monkeypatch
):
    kernel = _compile_rpu_masked_mul_add_kernel(
        tmp_path, monkeypatch, logical_n=17, block_size=32
    )
...
def test_rpu_compile_add_mul_defaults_to_generic_executable(
    tmp_path, monkeypatch
):
...
def test_rpu_compile_masked_add_mul_add_defaults_to_generic_executable(
    tmp_path, monkeypatch
):
    kernel = _compile_rpu_masked_add_mul_add_kernel(
        tmp_path, monkeypatch, logical_n=17, block_size=32
    )
...
@pytest.mark.skip(reason="v3.6 stage-loop / cache-manager regression on the rpu.plan.mlir legacy-format override path: kernel.asm['rpuplan'] returns the raw override file's #loc table instead of the post-processed module body containing rpu.kernel. Reproducible by a manual call to RPUBackend._make_split_stage_bundle_from_mlir_file too.")
def test_rpu_compile_add_rpuplan_override_uses_typed_bundle_not_json_artifact(tmp_path, monkeypatch):
    debug_dir = tmp_path / "debug"
    monkeypatch.setenv("RPU_DEBUG_ARTIFACT_DIR", str(debug_dir))

    monkeypatch.setenv("RPU_USE_EXECUTABLE_DIALECT", "0")
    fresh_kernel = _compile_rpu_add_kernel(tmp_path, monkeypatch)
    fresh_bundle_dir = Path(fresh_kernel.metadata.rpu_debug_bundle["dir"])
    override_text = (fresh_bundle_dir / "rpu.plan.mlir").read_text()
    monkeypatch.delenv("RPU_USE_EXECUTABLE_DIALECT", raising=False)

    override_kernel, manager, override_path = _compile_rpu_add_kernel_with_rpuplan_override(
        tmp_path,
        monkeypatch,
        override_text,
    )

    assert ("get", "rpu_add_kernel.rpuplan") in manager.calls
    assert override_path.read_text() == override_text

    _assert_native_exec_artifact(override_kernel.asm["rpuplan"], "generic")
    assert "RPUPlanStageBundle" not in override_kernel.asm["rpuplan"]
    assert override_kernel.metadata.rpu_rpurc_source_kind == "rpu_executable"

    event_pairs = [
        (event["stage"], event["event"])
        for event in override_kernel.metadata.rpu_pipeline_events
    ]
    assert ("rpuplan_replay", "start") in event_pairs
    assert ("rpuplan_replay", "end") in event_pairs
    assert ("rpurc_emit", "rpu_executable") in event_pairs

    debug_bundle = override_kernel.metadata.rpu_debug_bundle
    ...
    sidecar = Path(override_info["path"])
    assert sidecar.name == "rpu.plan.override.mlir"
    assert "rpu_plan.kernel @rpu_add_kernel_plan" in sidecar.read_text()
    assert manifest["files"]["rpuplan_override_mlir"] == override_info


def test_rpu_compile_add_native_exec_rpuplan_override_replays_executable_bundle(
    tmp_path, monkeypatch
):
    debug_dir = tmp_path / "debug"
    monkeypatch.setenv("RPU_DEBUG_ARTIFACT_DIR", str(debug_dir))
    ...
    fresh_kernel = _compile_rpu_add_kernel(tmp_path, monkeypatch)
    override_text = fresh_kernel.asm["rpuplan"]
    _assert_native_exec_artifact(override_text, "generic")
    assert "rpu.kernel @rpu_add_kernel" in override_text
    assert "rpu_plan.kernel" not in override_text
    ...
    assert override_path.read_text() == override_text
    _assert_native_exec_artifact(override_kernel.asm["rpuplan"], "generic")
    assert override_kernel.metadata.rpu_rpuexec_source_kind == "rpu_executable"
    assert override_kernel.metadata.rpu_rpurc_source_kind == "rpu_executable"
    ...
    assert ("rpuexec_replay", "start") in event_pairs
    assert ("rpuexec_replay", "end") in event_pairs
    assert ("rpuplan_replay", "start") not in event_pairs
    assert ("rpuplan_replay", "end") not in event_pairs
    assert ("rpurc_emit", "rpu_executable") in event_pairs
    ...
    sidecar = Path(override_info["path"])
    assert sidecar.name == "rpu.exec.override.mlir"
    assert "rpu.kernel @rpu_add_kernel" in sidecar.read_text()
...
def test_rpu_compile_cache_metadata_excludes_transient_stage_keys(tmp_path, monkeypatch):
    ...
    assert [
        (event["stage"], event["event"]) for event in metadata["rpu_pipeline_events"][2:6]
    ] == [
...
def _compile_rpu_masked_mul_add_kernel(
    tmp_path, monkeypatch, logical_n=17, block_size=32
):
    llvm_root = require_rpu_toolchain()
...
def _compile_rpu_masked_add_mul_add_kernel(
    tmp_path, monkeypatch, logical_n=17, block_size=32
):
    llvm_root = require_rpu_toolchain()
    ...
    @triton.jit
    def rpu_add_mul_add_masked_kernel(
        out, a, b, c, d, n: tl.constexpr, block: tl.constexpr
    ):
        offsets = tl.arange(0, block)
        mask = offsets < n
        av = tl.load(a + offsets, mask=mask, other=0.0)
...
def _compile_rpu_add_kernel_with_rpuplan_override(
    tmp_path,
    monkeypatch,
    override_text,
    n_elements=16,
    ...
    override_path = override_dir / "rpu_add_kernel.rpuplan"
    ...
    class _OverrideManager:
        ...
        def has_file(self, filename):
            self.calls.append(("has", filename))
            return filename == "rpu_add_kernel.rpuplan"

        def get_file(self, filename):
            self.calls.append(("get", filename))
            if filename != "rpu_add_kernel.rpuplan":
                return None
            return str(override_path)

    manager = _OverrideManager()
    monkeypatch.setattr(triton_compiler, "get_override_manager", lambda key: manager)
    monkeypatch.setenv("TRITON_KERNEL_OVERRIDE", "1")
    kernel = _compile_rpu_add_kernel(tmp_path, monkeypatch, n_elements=n_elements)
    return kernel, manager, override_path


def _compile_rpu_add_kernel_with_rpuexec_override(
    tmp_path,
    monkeypatch,
    override_text,
    n_elements=16,
    ...
    override_dir = tmp_path / "override"
    override_dir.mkdir(exist_ok=True)
    override_path = override_dir / "rpu_add_kernel.rpuexec"
    override_path.write_text(override_text)
    ...
    class _OverrideManager:
        ...
        def has_file(self, filename):
            self.calls.append(("has", filename))
            return filename == "rpu_add_kernel.rpuexec"

        def get_file(self, filename):
            self.calls.append(("get", filename))
            if filename != "rpu_add_kernel.rpuexec":
                return None
            return str(override_path)
...
def _read_single_rpu_cache_metadata(cache_root, kernel_name):
    metadata_files = [
        path for path in _cache_metadata_json_files(cache_root)
        if path.name == f"{kernel_name}.json"
    ]
...
def _recognized_plan_from_kernel(kernel, tmp_path, monkeypatch):
    plan_json, _trace_json, _printed = _run_rpu_plan_pass_on_mlir(
        kernel.asm["ttir"], tmp_path, monkeypatch
    )
