...
from triton.backends.compiler import BaseBackend, GPUTarget, Language, StageResult
...
class RPUBackend(BaseBackend):
    ...
    def __init__(self, target: GPUTarget) -> None:
        ...
        # rpuelf stage emits the device ELF when RPU_OUTPUT_ELF=1.
        # Listing it here lets the cache round-trip surface kernel.asm["rpuelf"]
        # as raw bytes instead of attempting utf-8 decode.
        self.binary_exts = ("rpubin", "rpuelf")
    ...
    def parse_options(self, options: dict) -> RPUOptions:
        ...
        if "rpu_asm_path" not in accepted:
            ...
            accepted["rpu_asm_path"] = os.getenv("RPU_ASM_PATH") or str(
                Path(accepted["llvm_root"]) / "bin" / "rpuasm"
            )
    ...
    def add_stages(self, stages: dict, options: RPUOptions, language) -> None:
        if language != Language.TRITON:
            raise NotImplementedError(
                f"RPU backend does not yet support language={language}"
            )
    ...
    def parse_split_stage_override(self, full_name, ext, context, metadata):
        if ext == "rpuexec":
            bundle = RPUBackend._make_rpuexec_stage_bundle_from_mlir_file(
                full_name,
                context,
                metadata,
            )
            if metadata is not None:
                RPUBackend._record_debug_text_artifact(
                    metadata,
                    "rpuexec_override_mlir",
                    "rpu.exec.override.mlir",
                    str(bundle.module),
                )
            return StageResult(value=bundle, artifact=str(bundle.module))
        if ext != "rpuplan":
            raise RuntimeError(
                f"RPU parse_split_stage_override called with unexpected ext={ext!r}; "
                "only 'rpuexec' and 'rpuplan' are split stages"
            )
        bundle = RPUBackend._make_split_stage_bundle_from_mlir_file(
            full_name,
            context,
            metadata,
        )
        if metadata is not None:
            if isinstance(bundle, RPUExecutableStageBundle):
                RPUBackend._record_debug_text_artifact(
                    metadata,
                    "rpuexec_override_mlir",
                    "rpu.exec.override.mlir",
                ...
                RPUBackend._record_debug_text_artifact(
                    metadata,
                    "rpuplan_override_mlir",
                    "rpu.plan.override.mlir",
                    str(bundle.module),
                )
        return StageResult(value=bundle, artifact=str(bundle.module))
    ...
    @staticmethod
    def _record_rpurc_failure(metadata, failure_kind: str, error, summary: dict | None = None, detail: dict | None = None):
        event_detail = {
    ...
    @staticmethod
    def _make_rpuexec_stage_result_from_module(mod):
        exec_summary = RPUBackend._get_rpuexec_kernel_summary(mod)
        return (
            StageResult(
                value=RPUExecutableStageBundle(
                    module=mod,
    ...
    def _try_make_direct_executable_stage_from_ttir(mod, metadata):
        if os.environ.get("RPU_USE_EXECUTABLE_DIALECT", "1") == "0":
            return None
        if not hasattr(rpu, "passes") or not hasattr(
            rpu.passes, "add_lower_supported_ttir_to_executable"
        ):
            return None
        ...
        try:
            direct_stage, exec_summary = RPUBackend._make_rpuexec_stage_result_from_module(
            ...
            RPUBackend._write_debug_failure_manifest(metadata)
            raise RuntimeError(
                "RPU direct executable stage summary failed while creating executable-first stage value"
            ) from error

        direct_executable_artifact = str(direct_executable_module)
    ...
    def _make_rpuplan_stage_from_mlir_module_via_recognizer(mod, metadata):
        ...
        try:
            ...
            RPUBackend._write_debug_failure_manifest(metadata)
            raise RuntimeError(
                "RPU MLIR plan recognizer failed to produce rpu.plan.json artifact"
            ) from error
        try:
    ...
    def _parse_typed_replay_mlir_file(
        ...
        context,
        metadata=None,
        failure_stage="rpuplan_replay",
        parse_error_message="RPU typed replay failed to parse rpu.plan.mlir",
    ):
        if ir is None:
            raise RuntimeError("RPU typed replay requires triton._C.libtriton.ir") from _ir_import_error
        if rpu is None:
            raise RuntimeError("RPU typed replay requires triton._C.libtriton.rpu") from _rpu_import_error

        path = Path(path)
        try:
            mod = ir.parse_mlir_module(str(path), context)
            RPUBackend._remember_module_context(mod, context)
            return mod
        except Exception as error:
            if metadata is not None and failure_stage is not None:
                RPUBackend._record_pipeline_event(
                    metadata,
                    failure_stage,
                    "fail",
            ...
            raise RuntimeError(parse_error_message) from error

    @staticmethod
    def _try_make_rpuexec_stage_bundle_from_override_module(mod, path, metadata=None):
        try:
            stage_result, summary = RPUBackend._make_rpuexec_stage_result_from_module(mod)
        except Exception:
            return None
    ...
    @staticmethod
    def _make_rpuexec_stage_bundle_from_mlir_file(path, context, metadata=None):
        path = Path(path)
        mod = RPUBackend._parse_typed_replay_mlir_file(
            path,
            context,
            metadata,
            failure_stage="rpuexec_replay",
            parse_error_message="RPU executable replay failed to parse rpu.exec.mlir",
        )
        if metadata is not None:
            RPUBackend._record_pipeline_event(
                metadata,
        ...
        try:
            ...
            raise RuntimeError("RPU executable replay failed to validate rpu.kernel") from error
    ...
    @staticmethod
    def _make_split_stage_bundle_from_mlir_file(path, context, metadata=None):
        path = Path(path)
        mod = RPUBackend._parse_typed_replay_mlir_file(
            path,
            context,
        ...
        executable_bundle = RPUBackend._try_make_rpuexec_stage_bundle_from_override_module(
            mod,
            path,
            metadata,
        )
        ...
        return RPUBackend._make_rpuplan_stage_bundle_from_mlir_module(
            mod,
            path,
            metadata,
        )
    ...
    @staticmethod
    def _make_rpuplan_stage_bundle_from_mlir_module(
        mod,
        path,
        metadata=None,
        record_start=True,
        ...
        path = Path(path)
        if metadata is not None and record_start:
            RPUBackend._record_pipeline_event(
                metadata,
                "rpuplan_replay",
                "start",
        ...
        try:
            ...
            raise RuntimeError("RPU typed replay failed to validate rpu_plan.kernel") from error
    ...
    @staticmethod
    def _make_rpuplan_stage_bundle_from_mlir_file(path, context, metadata=None):
        path = Path(path)
        if metadata is not None:
            RPUBackend._record_pipeline_event(
                metadata,
    ...
    def make_rpuexec(src, metadata):
        if isinstance(src, RPUExecutableStageBundle):
            ...
            return StageResult(
                value=src,
                artifact=str(src.module),
            )

        if not isinstance(src, RPUPlanStageBundle):
            error = (
                "RPU make_rpuexec requires RPUPlanStageBundle or "
        ...
        has_executable_hooks = (
            hasattr(rpu, "_rpuplan_supports_executable_lowering")
            and hasattr(rpu, "_lower_rpuplan_to_executable_module")
            and hasattr(rpu, "get_module_str_attr")
            and hasattr(rpu, "passes")
        ...
        if reason is None:
            try:
                ...
                try:
                    ...
                    pass
                exec_stage, exec_summary = RPUBackend._make_rpuexec_stage_result_from_module(
                    exec_module
                )
            except Exception as error:
                RPUBackend._record_pipeline_event(
                ...
                RPUBackend._write_debug_failure_manifest(metadata)
                raise RuntimeError(
                    f"RPU executable stage lowering failed for pattern {src.pattern}"
                ) from error
        ...
        if reason not in LEGACY_DIRECT_FALLBACK_REASONS:
            error = (
                "RPU executable stage does not allow direct fallback for reason "
        ...
        if not hasattr(rpu, "_direct_rpurc_supported_patterns"):
            error = (
                "RPU direct fallback requires _direct_rpurc_supported_patterns "
        ...
        if summary["pattern"] not in direct_patterns:
            error = (
                f"RPU direct fallback for pattern {summary['pattern']} does not "
        ...
        return StageResult(
            ...
            artifact=RPUBackend._format_rpuexec_fallback_artifact(
                summary, reason, fallback_detail
            ),
        )
    ...
    def make_rpurc(src, metadata):
        ...
        if src.source_kind == "rpu_executable":
            try:
                ...
                raise RuntimeError(
                    f"RPU executable rpurc emission failed for pattern {summary['pattern']}"
                ) from error
        ...
        try:
            ...
            raise RuntimeError(
                f"RPU rpurc stage emission failed for pattern {summary['pattern']}"
            ) from error
    ...
    def _resolve_clang(options: RPUOptions) -> Path:
        ...
        for candidate in (root / "bin" / "clang", root / "build" / "bin" / "clang"):
            if candidate.exists():
                return candidate
        raise RuntimeError(
            f"RPU clang not found under {root}; tried bin/clang and build/bin/clang"
        )
    ...
    def _rpuinstr_text_from_rpubin(bin_bytes: bytes) -> str:
        ...
        if len(bin_bytes) % 8 != 0:
            raise RuntimeError(
                f"rpubin length {len(bin_bytes)} is not a multiple of 8; "
                "cannot derive rpuinstr listing"
            )
        return "".join(
            bin_bytes[offset:offset + 8][::-1].hex() + "\n"
    ...
    def make_rpuinstr(src, metadata, options: RPUOptions):
        ...
        with tempfile.TemporaryDirectory(prefix="triton_rpu_") as temp_dir:
            ...
            if os.getenv("RPU_OUTPUT_ELF") == "1":
                RPUBackend._emit_rpuelf(
                    kernel_name, src_path, temp_path, temp_dir, env, metadata, options
                )
            RPUBackend._record_pipeline_event(
                metadata,
                "rpuinstr_compile",
