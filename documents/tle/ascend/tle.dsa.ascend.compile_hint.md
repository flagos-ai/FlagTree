...
**不加 Hint**（`bit_ref.ttadapter.mlir`）— condition 以 i8 加载后直接经 `arith.cmpi ne` + `arith.select` 处理：

```
%13 = bufferization.to_tensor %alloc_6 restrict writable : memref<1024xi8>
%14 = arith.cmpi ne, %13, %1 : tensor<1024xi8>
%15 = arith.select %14, %12, %11 : tensor<1024xi1>, tensor<1024xf16>
```

**使用 bitwise_mask**（`bitwise.ttadapter.mlir`）— 在 condition 上附加 `annotation.mark {bitwise_mask}`：

```
%13 = bufferization.to_tensor %alloc_6 restrict writable : memref<1024xi8>
annotation.mark %13 {bitwise_mask} : tensor<1024xi8>
%14 = arith.cmpi ne, %13, %1 : tensor<1024xi8>
%15 = arith.select %14, %12, %11 : tensor<1024xi1>, tensor<1024xf16>
```
...
#### 问题 1：ttadapter 中 annotation.mark 位置不对

从上面 IR 可以看到，`compile_hint` 附加到了 condition 张量（`%13`）上，而非 select 的左值上。期望的 hivm pass 回溯模式要求 annotation 标记在 vsel 或其 vcast 结果上，而非 vsel 的输入上。

当前 hivm 中 NormalizeBitwiseSelect pass 期望的匹配模式：

```
%0 = ...                                    // 原始 i8 bitmask
%1 = vcast %0 → tensor<Nxf16>               // i8→f16
%2 = vcmp %1, %zero → tensor<Nxi1>          // 与零比较
%3 = vnot %2 → tensor<Nxi1>                 // 取反
%4 = vsel %3, %true, %false                 // 用取反后的条件做选择
%5 = vcast %4 (可选)
annotation.mark %5 (或 %4) {bitwise_mask}    // 标记必须在 select 的左值上
```

由于 hint 只加到 condition 上（`%0` 对应的位置），select 的左值上没有 `bitwise_mask` 标记，pass 无法匹配到该模式，导致 NormalizeBitwiseSelect pass 直接跳过。

#### 问题 2：尝试修复

若强行将 selOp 传入的 cond 直接用 int8，CANN 存在精度问题（bit-level mask vs byte-level boolean 语义差异）。

当前修复方案（参考 [AscendNPU-IR commit](https://gitee.com/Waynefeiran/ascendnpu-ir/commit/bc57d5043c5b7b4fef4de87f21731532539b750c)）：
- 修改 NormalizeBitwiseSelect pass：`vcast(i8→i1, rint)` → `vsel(i1)`，去除 f16 转换过程

---

[test_compile_hint.py](../../../python/test/tle/test_compile_hint.py) — 包含上述所有 hint 的完整测试用例