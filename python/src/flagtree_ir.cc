...
static std::optional<uint32_t> parseAddressSpaceFromTypeHint(const std::string &typeHint) {
  ...
  if (std::regex_match(typeHint, memref_match, memref_regex)) {
    try {
      ...
      llvm::errs() << "[ERROR] Failed to parse address space from memref param type: " << typeHint << "\n";
      return std::nullopt;
    }
  }
  ...
  if (std::regex_match(typeHint, ptr_match, ptr_regex)) {
    try {
      ...
      llvm::errs() << "[ERROR] Failed to parse address space from llvm.ptr param type: " << typeHint << "\n";
      return std::nullopt;
    }
  }
...
// 2. Create a DSLRegionOp with EDSL function parameter types stored in attributes
...
//    - EDSL function declarations (stored in edsl_param_types attribute) specify expected types
//    - LLVM function arguments are already in LLVM types
//    - We need to verify consistency: TT type -> EDSL param type -> LLVM func arg type
//
// Example type conversion for tensor:
//   - TT IR: tensor<128xi32> (RankedTensorType)
//   - EDSL param type: "memref<?xi32, 3>" (stored in edsl_param_types attribute)
...
flagtree::DSLRegionOp FlagTreeOpBuilder::createEdslRegionByLLVMFunc(
  ...
  // The edsl_param_types contain the EDSL function parameter type declarations (e.g., "memref<?xi32, 3>", "i32")
  // These are stored as an ArrayAttr of StringAttrs in the DSLRegionOp's "edsl_param_types" attribute
  // The edsl_param_names contain the EDSL function parameter names (e.g., "sum_buf", "indices")
  // These are stored as an ArrayAttr of StringAttrs in the DSLRegionOp's "edsl_param_names" attribute
  ...
  if (!arg_type_hints.empty()) {
    ...
    for (const auto &typeHint : arg_type_hints) {
      typeAttrs.push_back(StringAttr::get(getContext(), typeHint));
    }
    edslParamTypesAttr = ArrayAttr::get(getContext(), typeAttrs);
  }
  ...
  if (!arg_names.empty()) {
    ...
    for (const auto &name : arg_names) {
      nameAttrs.push_back(StringAttr::get(getContext(), name));
    }
    edslParamNamesAttr = ArrayAttr::get(getContext(), nameAttrs);
  }
  
  SmallVector<NamedAttribute> attrs;
  if (edslParamTypesAttr) {
    attrs.push_back(NamedAttribute(
        StringAttr::get(getContext(), "edsl_param_types"), edslParamTypesAttr));
  }
  if (edslParamNamesAttr) {
    attrs.push_back(NamedAttribute(
        StringAttr::get(getContext(), "edsl_param_names"), edslParamNamesAttr));
  }
  ...
  for (auto [idx, oldBlock] : llvm::enumerate(func.getBlocks())) {
    if (idx == 0) {
      ...
      // Note: operands = [outputs..., inputs...], edsl_param_types corresponds to all operands
      // Extract edsl_param_names from attribute
      SmallVector<std::string> edsl_param_names_from_attr;
      if (auto edslParamNamesAttr = dslRegionOp->getAttrOfType<ArrayAttr>("edsl_param_names")) {
      ...
      for (size_t operand_idx = 0; operand_idx < operands.size(); ++operand_idx) {
        ...
        std::string arg_name = operand_idx < edsl_param_names_from_attr.size() ? edsl_param_names_from_attr[operand_idx] : "";
        ...
        if (RankedTensorType tensorTy = dyn_cast<RankedTensorType>(blockArg.getType())) {
          // Type consistency check: verify EDSL parameter type matches actual Triton type
          // If EDSL param type is "llvm.ptr<...>", it requires triton::PointerType, not RankedTensorType
          if (!arg_type.empty()) {
            std::regex ptr_regex(R"(!?llvm\.ptr<\d+>)");
            if (std::regex_match(arg_type, ptr_regex)) {
              llvm::errs() << "[ERROR] Type mismatch for operand " << operand_idx;
              if (!arg_name.empty()) {
                llvm::errs() << " (parameter: " << arg_name << ")";
              }
              llvm::errs() << "\n";
              llvm::errs() << "[ERROR] EDSL param type: " << arg_type << " (expects triton::PointerType)\n";
              llvm::errs() << "[ERROR] Actual Triton type: " << blockArg.getType() << " (RankedTensorType)\n";
              assert(false && "EDSL type hint mismatch: llvm.ptr requires triton::PointerType, but got RankedTensorType");
            }
          }
          ...
          // Parse expected address space from EDSL parameter type (e.g., "memref<?xi32, 3>" -> 3)
        ...
        // EDSL param type: "llvm.ptr<1>" (stored in edsl_param_types attribute)
        ...
        // Conversion: Create ExtractPtrOp to convert TT pointer to LLVM pointer
          ...
          // Type consistency check: verify EDSL parameter type matches actual Triton type
          // If EDSL param type is "memref<...>", it requires RankedTensorType, not triton::PointerType
          if (!arg_type.empty()) {
            std::regex memref_regex(R"(!?memref<[^>]*,\s*\d+\s*>)");
            if (std::regex_match(arg_type, memref_regex)) {
              ...
              llvm::errs() << "\n";
              llvm::errs() << "[ERROR] EDSL param type: " << arg_type << " (expects RankedTensorType)\n";
              llvm::errs() << "[ERROR] Actual Triton type: " << blockArg.getType() << " (triton::PointerType)\n";
              assert(false && "EDSL type hint mismatch: memref requires RankedTensorType, but got triton::PointerType");
            }
          }
          ...
          // Parse expected address space from EDSL parameter type (e.g., "llvm.ptr<1>" -> 1)
          uint32_t expected_as = llvm_as;  // Default to LLVM's address space
          if (auto parsed_as = parseAddressSpaceFromTypeHint(arg_type)) {
            expected_as = *parsed_as;
        ...
        // Conversion: Use block argument directly (no extract operation needed)
          ...
          // Type consistency check: verify EDSL parameter type matches actual Triton type
          // Scalars should not have memref or llvm.ptr param types
          if (!arg_type.empty()) {
            std::regex memref_regex(R"(!?memref<[^>]*,\s*\d+\s*>)");
            std::regex ptr_regex(R"(!?llvm\.ptr<\d+>)");
            ...
            if (is_memref_hint || is_ptr_hint) {
              ...
              llvm::errs() << "\n";
              llvm::errs() << "[ERROR] EDSL param type: " << arg_type << " (expects tensor or pointer type)\n";
              llvm::errs() << "[ERROR] Actual Triton type: " << blockArg.getType() << " (scalar type)\n";
              assert(false && "EDSL type hint mismatch: memref/llvm.ptr requires tensor/pointer type, but got scalar");
            }
          }
          size_t expected_llvm_args = 1;
