...
class DSASemantic:
    ...
    def validate_alloc_shape(shape: Sequence) -> Tuple[int, ...]:
        ...
        if not isinstance(shape, (tuple, list)):
            if hasattr(shape, "__iter__"):
                ...
                raise DSASemanticError(
                    f"alloc: shape must be a tuple or list, got {type(shape).__name__}")
        ...
        for i, dim in enumerate(shape):
            ...
            if not isinstance(dim, int) or dim <= 0:
                raise DSASemanticError(
                    f"alloc: shape[{i}] must be a positive integer, got {dim!r}")
    ...
    def validate_alloc_dtype(dtype: tl.dtype) -> tl.dtype:
        ...
        if not isinstance(dtype, tl.dtype):
            raise DSASemanticError(
                f"alloc: dtype must be a tl.dtype instance, got {type(dtype).__name__}")
        if dtype not in _SUPPORTED_ALLOC_DTYPES:
            supported = ", ".join(str(d) for d in sorted(_SUPPORTED_ALLOC_DTYPES, key=str))
            raise DSASemanticError(
                f"alloc: unsupported dtype {dtype}. Supported types: {supported}")
        return dtype
    ...
    def validate_alloc_scope(scope) -> tle.scope:
        ...
        if not isinstance(scope, tle.scope):
            raise DSASemanticError(
                f"alloc: scope must be a tle.scope instance, got {type(scope).__name__}")
        return scope
    ...
    def validate_copy_operands(src, dst) -> str:
        ...
        if (not src_is_buf) and dst_is_buf:
            if not isinstance(src, tl.tensor):
                raise DSASemanticError(
                    f"copy: src must be tl.tensor or buffered_tensor, got {type(src).__name__}")
            return "GM_TO_SPM"
        if src_is_buf and (not dst_is_buf):
            if not isinstance(dst, tl.tensor):
                raise DSASemanticError(
                    f"copy: dst must be tl.tensor or buffered_tensor, got {type(dst).__name__}")
            return "SPM_TO_GM"
        raise DSASemanticError(
            "copy: at least one operand must be a buffered_tensor. "
    ...
    def validate_copy_dtype_compat(src_dtype, dst_dtype) -> None:
        ...
        if src_dtype != dst_dtype:
            raise DSASemanticError(
                f"copy: element type mismatch – src has {src_dtype}, dst has {dst_dtype}")
    ...
    def validate_local_ptr_buffer(buffer) -> None:
        ...
        if not isinstance(buffer, tle.buffered_tensor):
            raise DSASemanticError(
                f"local_ptr: buffer must be a buffered_tensor, got {type(buffer).__name__}")
        if buffer.type.shape is None:
            raise DSASemanticError(
                "local_ptr: buffer shape is None (deferred shapes not yet supported)")
    ...
    def validate_local_ptr_indices(
        ...
        if indices is None:
            raise DSASemanticError(
                "local_ptr: indices must be provided as a tuple of tensors")
        if len(indices) != buffer_rank:
            raise DSASemanticError(
                f"local_ptr: expected {buffer_rank} index tensors, got {len(indices)}")
        ...
        for i, idx in enumerate(indices):
            if not isinstance(idx, tl.tensor):
                raise DSASemanticError(
                    f"local_ptr: indices[{i}] must be a tl.tensor, "
                    f"got {type(idx).__name__}")
            if not idx.dtype.is_int():
                raise DSASemanticError(
                    f"local_ptr: indices[{i}] must have integer dtype, "
                    f"got {idx.dtype}")
            ...
            if is_scalar:
                ...
                if view_shape is None:
                    ...
                    raise DSASemanticError(
                        f"local_ptr: index tensor shape mismatch at dim {i}: "
                        f"expected {view_shape}, got {tuple(idx.shape)}")

        if has_scalar and has_tensor:
            raise DSASemanticError(
                "local_ptr: indices must be either all scalar or all "
