...
V33 keeps v31 intact and adopts two measured dispatch changes: CTA-SMEM expert
counting and the CUDA-shaped pre-pull NVLink barrier (reuse D3 sync and signal
all peers in parallel).  `W_D8_PULL_STREAMS=2` retains the three attempted
dual-stream implementations as compiler diagnostics; PR837 currently rejects
their multi-route/stage ownership during TritonGPUCoalesce, so the live default
remains one D8 stream.
...
HERE = Path(__file__).resolve().parent   # raw .cu helpers live beside v33
...
def dispatch_nvlink_barrier(sig_tab, slot, ctr, sm_idx,
    ...
    if sm_idx == 0:
        if FAST_NVLINK_BARRIER:
            pe = tl.arange(0, 8)
            tl.atomic_add(peer_i32(sig_tab, pe) + slot, 1, mask=pe < NPES,
                          sem="release", scope="sys")
        else:
            for pe in tl.static_range(0, NPES):
                tl.atomic_add(peer_i32(sig_tab, pe) + slot, 1,
                              sem="release", scope="sys")
...
def dispatch_dual_d8_pull(
    sf_tab, w_tab, rsum_l, recv_l, q_l, meta_l, l1_arrival_count,
    l1_token, l1_sf, l1_w, d8_token_s, d8_state_s,
    tp0, tp1, tp2, tp3, tp4, tp5, tp6, tp7,
    MY_PE: tl.constexpr, NPES: tl.constexpr, NUM_SMS: tl.constexpr,
    EPR: tl.constexpr, R_POW2: tl.constexpr, MAX_RECV: tl.constexpr,
    BLOCK_M: tl.constexpr, K: tl.constexpr, NSF: tl.constexpr,
    POOL_TOKENS: tl.constexpr, TOPK: tl.constexpr,
    NSF_POW2: tl.constexpr, USE_D8_TMA1D: tl.constexpr,
):
    ...
    tle_raw.call(d8_tma1d_edsl,
                 [stage0, state0, init_addr, d8_bytes, inactive, d8_init_op])
    tle_raw.call(d8_tma1d_edsl,
                 [stage1, state1, init_addr, d8_bytes, inactive, d8_init_op])
    dispatch_sync(USE_D8_TMA1D)
    tle_raw.call(d8_tma1d_edsl,
                 [stage0, state0, init_addr, d8_bytes, inactive, d8_fence_op])
    tle_raw.call(d8_tma1d_edsl,
                 [stage1, state1, init_addr, d8_bytes, inactive, d8_fence_op])
    dispatch_sync(USE_D8_TMA1D)
    ...
    while tl.sum((done == 0).to(tl.int32), axis=0) > 0:
        ...
        while tl.sum(need.to(tl.int32), axis=0) > 0:             # D6, two cursors
            ...
            n = (tl.load(rsum_l + safe_cur, mask=need & ~ex, other=0) &
        ...
        remaining = tl.load(
            ...
            mask=live[:, None] & rank_ok[None, :], other=0,
        ).to(tl.int32)
        slot = tok_in_e
        ...
        while tl.sum((found == 0).to(tl.int32), axis=0) > 0:     # D7, two routes
            ...
            selected_rank = tl.sum(
                tl.where(sel, rank_v[None, :], 0), axis=1)
        ...
        peer_tok = tl.multiple_of(
            peer_base.to(tl.pointer_type(tl.float8e4nv)), 16)
        ...
        tle_raw.call(d8_tma1d_edsl,
                     [stage0, state0, _pick_stream2(src_addr, 0), d8_bytes,
                      _pick_stream2(active_i32, 0), d8_load_op])
        tle_raw.call(d8_tma1d_edsl,
                     [stage1, state1, _pick_stream2(src_addr, 1), d8_bytes,
        ...
        sfv = tl.load(
            ...
            mask=live[:, None] & sf_ok[None, :], other=0.0)
        tl.store(l1_sf + offs_sf[None, :] * POOL_TOKENS + pt[:, None], sfv,
        ...
        tle_raw.call(d8_tma1d_edsl,
                     [stage0, state0, _pick_stream2(dst_addr, 0), d8_bytes,
                      _pick_stream2(active_i32, 0), d8_store_op])
        tle_raw.call(d8_tma1d_edsl,
                     [stage1, state1, _pick_stream2(dst_addr, 1), d8_bytes,
...
def _resolve_one_d8_route(
    token_idx, cur, e_start, e_end, pool_blk, done, rsum_l, recv_l, q_l,
    NPES: tl.constexpr, EPR: tl.constexpr, R_POW2: tl.constexpr,
    MAX_RECV: tl.constexpr, BLOCK_M: tl.constexpr, TOPK: tl.constexpr,
):
    ...
    while found == 0:
        ...
        src_rank = tl.where(
            hit, tl.sum(tl.where(sel, rank_v, 0), axis=0), src_rank)
        tir = tl.where(hit, offset + slot // denom, tir)
        slot = tl.where(hit, slot, slot - nrt)
        offset = tl.where(hit, offset, offset + length)
        remaining = tl.where(
    ...
    src_topk = stt % TOPK
    pt = tl.where(live, pool_blk * BLOCK_M + tok_in_e, 0)
    return (cur, e_start, e_end, pool_blk, done, live, tok_in_e,
            src_rank, stt, src_tok, src_topk, pt)
...
def dispatch_dual_d8_pull_scalar(
    ...
    init_addr = tl.cast(l1_token, tl.int64)
    tle_raw.call(d8_tma1d_edsl,
                 [stage0, state0, init_addr, d8_bytes, inactive, init_op])
    tle_raw.call(d8_tma1d_edsl,
                 [stage1, state1, init_addr, d8_bytes, inactive, init_op])
    dispatch_sync(USE_D8_TMA1D)
    tle_raw.call(d8_tma1d_edsl,
                 [stage0, state0, init_addr, d8_bytes, inactive, fence_op])
    tle_raw.call(d8_tma1d_edsl,
                 [stage1, state1, init_addr, d8_bytes, inactive, fence_op])
    dispatch_sync(USE_D8_TMA1D)
    ...
    while (done0 == 0) | (done1 == 0):
        (cur0, start0, end0, pool0, done0, live0, tie0,
         rank0, stt0, src_tok0, src_topk0, pt0) = _resolve_one_d8_route(
            token0, cur0, start0, end0, pool0, done0, rsum_l, recv_l, q_l,
            NPES, EPR, R_POW2, MAX_RECV, BLOCK_M, TOPK)
        (cur1, start1, end1, pool1, done1, live1, tie1,
         rank1, stt1, src_tok1, src_topk1, pt1) = _resolve_one_d8_route(
            token1, cur1, start1, end1, pool1, done1, rsum_l, recv_l, q_l,
            NPES, EPR, R_POW2, MAX_RECV, BLOCK_M, TOPK)
        ...
        tle_raw.call(d8_tma1d_edsl,
                     [stage0, state0, src_addr0, d8_bytes, active0, load_op])
        tle_raw.call(d8_tma1d_edsl,
                     [stage1, state1, src_addr1, d8_bytes, active1, load_op])
        dispatch_sync(USE_D8_TMA1D)

        sfv0 = tl.load(peer_f32(sf_tab, rank0) + src_tok0 * NSF + offs_sf,
                       mask=live0 & sf_ok, other=0.0)
        sfv1 = tl.load(peer_f32(sf_tab, rank1) + src_tok1 * NSF + offs_sf,
                       mask=live1 & sf_ok, other=0.0)
        ...
        tle_raw.call(d8_tma1d_edsl,
                     [stage0, state0, dst_addr0, d8_bytes, active0, store_op])
        tle_raw.call(d8_tma1d_edsl,
                     [stage1, state1, dst_addr1, d8_bytes, active1, store_op])
        dispatch_sync(USE_D8_TMA1D)
        ...
        tl.atomic_add(l1_arrival_count + (pool0 + tie0 // BLOCK_M), 1,
                      mask=live0, sem="release")
        tl.atomic_add(l1_arrival_count + (pool1 + tie1 // BLOCK_M), 1,
                      mask=live1, sem="release")
...
def dispatch_d8_pingpong_pull(
    ...
    while (done == 0) | (pending != 0):
        (cur, e_start, e_end, pool_blk, done, live, tok_in_e,
         src_rank, stt, src_tok, src_topk, pt) = _resolve_one_d8_route(
            token_idx, cur, e_start, e_end, pool_blk, done,
            rsum_l, recv_l, q_l, NPES, EPR, R_POW2, MAX_RECV, BLOCK_M, TOPK)

        stage = tl.where(next_slot == 0, stage0, stage1)
        state = tl.where(next_slot == 0, state0, state1)
        peer_base = _select_d8_peer(
            src_rank, tp0, tp1, tp2, tp3, tp4, tp5, tp6, tp7)
        peer_tok = tl.multiple_of(
            peer_base.to(tl.pointer_type(tl.float8e4nv)), 16)
        src_addr = tl.cast(peer_tok + src_tok * K, tl.int64)
        ...
        tle_raw.call(d8_tma1d_edsl,
                     [stage, state, src_addr, d8_bytes, active, load_op])
        dispatch_sync(USE_D8_TMA1D)
        sfv = tl.load(peer_f32(sf_tab, src_rank) + src_tok * NSF + offs_sf,
                      mask=live & sf_ok, other=0.0)
        tl.store(l1_sf + offs_sf * POOL_TOKENS + pt, sfv,
                 mask=live & sf_ok)
        weight = tl.load(peer_f32(w_tab, src_rank) + stt,
                         mask=live, other=0.0)
        tl.store(l1_w + pt, weight, mask=live)
        dispatch_sync(USE_D8_TMA1D)
        ...
        tle_raw.call(d8_tma1d_edsl,
                     [pending_stage, pending_state, pending_dst,
                      d8_bytes, pending, store_op])
        dispatch_sync(USE_D8_TMA1D)
        ...
        tl.atomic_add(
            l1_arrival_count + (pending_pool + pending_tie // BLOCK_M), 1,
            mask=pending != 0, sem="release")
...
def dispatch_role(
    ...
    USE_D8_TMA1D: tl.constexpr, USE_SMEM_EXPERT_COUNT: tl.constexpr,
    FAST_NVLINK_BARRIER: tl.constexpr,
    ...
    if USE_SMEM_EXPERT_COUNT:
        ...
        ec_s = tle.gpu.alloc([NEXP_POW2], dtype=tl.int32, layout=None,
    ...
    for it in tl.static_range(0, ITERS):                        # D1 count
        ...
        ok1 = m & (e >= 0)
        if USE_SMEM_EXPERT_COUNT:
            tl.atomic_add(tle.gpu.local_ptr(ec_s, (tl.where(ok1, e, 0),)), 1,
                          mask=ok1, sem="relaxed", scope="cta")
        else:
    ...
    for it in tl.static_range(0, ITERS):                        # D3 -> OWNER's queue
        ...
        if USE_SMEM_EXPERT_COUNT:
            d3_slot = tl.atomic_add(
                tle.gpu.local_ptr(ec_s, (tl.where(valid, e, 0),)), 1,
                mask=valid, sem="relaxed", scope="cta")
        else:
    ...
    if D8_PULL_STREAMS == 2:
        dispatch_d8_pingpong_pull(
            ...
            l1_token, l1_sf, l1_w, d8_token_s, d8_state_s,
            tp0, tp1, tp2, tp3, tp4, tp5, tp6, tp7,
            MY_PE, NPES, NUM_SMS, EPR, R_POW2, MAX_RECV, BLOCK_M, K, NSF,
            POOL_TOKENS, TOPK, NSF_POW2, USE_D8_TMA1D,
        )
...
def loader_role(
    ...
    full_mask = ((1 << NL1N) - 1) | (((1 << NL1N) - 1) << 32)   # v31: WG0 low/ WG1 high bits
...
@triton.jit
def math_role(
    reader, cd_s, dst_rows_s, l1o_s, l1_st_desc,
    ...
    w1_sf, w2_sf,
    WRITE_L2_OUT: tl.constexpr, USE_L2_TMA: tl.constexpr, WG: tl.constexpr,
        ...
        if stop == 0:
            ...
            for kb in tl.range(0, nk):
                ...
                if phase == 1:
                    ...
                    sf_lo = tl.reshape(
                        tl.load(tle.gpu.local_ptr(sl.sfa.slot(0).subslice(row_base, WG_M, 0))), (WG_M,))
                    sf_hi = sf_lo
                    ...
                    acc_lo = tle.gpu.wgmma(a_my.subslice(0, HK, -1), sl.b.subslice(0, HK, -1),
                                           out_dtype=tl.float32, trans_b=True)
                    acc_hi = tle.gpu.wgmma(a_my.subslice(HK, HK, -1), sl.b.subslice(HK, HK, -1),
                                           out_dtype=tl.float32, trans_b=True)
                    sf_lo = tl.reshape(
                        tl.load(tle.gpu.local_ptr(sl.sfa.slot(0).subslice(row_base, WG_M, 0))), (WG_M,))
                    sf_hi = tl.reshape(
                        tl.load(tle.gpu.local_ptr(sl.sfa.slot(1).subslice(row_base, WG_M, 0))), (WG_M,))
                    acc_lo = tle.gpu.wgmma_wait(0, acc_lo)
            ...
            if phase == 1:
                ...
                tl.debug_barrier()
                tl.atomic_or(l2_arrival_mask + pool_block, (tl.full((), 1, tl.int64) << (n_block + WG * 32)), sem="release")
            else:
...
def ws_megakernel(
    ...
    USE_D8_TMA1D: tl.constexpr, USE_L1_STORE_PIPE: tl.constexpr,
    USE_SMEM_EXPERT_COUNT: tl.constexpr, FAST_NVLINK_BARRIER: tl.constexpr,
    D8_PULL_STREAMS: tl.constexpr,
    MY_PE: tl.constexpr, NPES: tl.constexpr, NUM_SMS: tl.constexpr,
    NTOK: tl.constexpr, TOPK: tl.constexpr, NEXP: tl.constexpr, EPR: tl.constexpr,
    ...
    # v31: per-WG scatter staging (2 x 64 rows) so the two math partitions don't
    # clobber each other's linear tile / dest-row buffers.
    cd_s = tle.gpu.alloc([2, BLOCK_M // 2, BLOCK_N], dtype=tl.bfloat16, layout=None,
                         scope=tle.gpu.smem, nv_mma_shared_layout=False)
    dst_rows_s = tle.gpu.alloc([2, BLOCK_M // 2], dtype=tl.uint64, layout=None,
                               scope=tle.gpu.smem, nv_mma_shared_layout=False)
    # v32: per-WG L1-output SMEM stage for the async TMA store pipeline.
    l1o_s = tle.gpu.alloc([2, BLOCK_M // 2, L1_OUT_N], dtype=tl.float8e4nv,
                          layout=None, scope=tle.gpu.smem)
    ...
    d8_token_s = tle.gpu.alloc([D8_PULL_STREAMS * K], dtype=tl.float8e4nv, layout=None,
                               scope=tle.gpu.smem, nv_mma_shared_layout=False)
    d8_state_s = tle.gpu.alloc([D8_PULL_STREAMS * 2], dtype=tl.uint64, layout=None,
                               scope=tle.gpu.smem, nv_mma_shared_layout=False)
    # every field is TMA -> ONE pipe (no mixed TMA / tl.store commit)
    p = tle.pipe(capacity=STAGES, scope="cta", name="megamoe_gemm",
                 readers=["m0", "m1"], a=a_s, b=b_s, sfa=sfa_s)
    ...
    tle.gpu.warp_specialize(
        [
            ...
            (dispatch_role, (
                ...
                USE_SMEM_EXPERT_COUNT, FAST_NVLINK_BARRIER, D8_PULL_STREAMS)),
            (math_role, (
                p.reader(name="m0"), cd_s, dst_rows_s, l1o_s, l1_st_desc,
                ...
                WRITE_L2_OUT, USE_L2_TMA, 0, USE_L1_STORE_PIPE,     # WG=0 -> rows [0:64]
                MY_PE, NPES, NUM_SMS, EPR, EPR_POW2, EPW, BLOCK_M, BLOCK_N, BLOCK_K,
                INTER, L1_N, L2_N, NK1, NK2, NL1N, NL2N, L1_OUT_N, NPAIR, POOL_TOKENS,
                NTOK, TOPK, K, K_POW2)),
            (math_role, (
                p.reader(name="m1"), cd_s, dst_rows_s, l1o_s, l1_st_desc,
                meta_ready, rsum_tab, cb_tab, sig_tab,
                meta_tab, l2out_tab, cb_local, l1_w, l2_acts, l2_sf, l2_arrival_mask,
                math_blocks, topk_local, final_y, gctr, w1_sf, w2_sf,
                WRITE_L2_OUT, USE_L2_TMA, 1, USE_L1_STORE_PIPE,     # WG=1 -> rows [64:128]
                MY_PE, NPES, NUM_SMS, EPR, EPR_POW2, EPW, BLOCK_M, BLOCK_N, BLOCK_K,
                INTER, L1_N, L2_N, NK1, NK2, NL1N, NL2N, L1_OUT_N, NPAIR, POOL_TOKENS,
                NTOK, TOPK, K, K_POW2)),
        ],
        [DISPATCH_WARPS, MATH_WARPS, MATH_WARPS],
        [24, 232, 232],
    )
...
def run_worker() -> int:
    ...
    USE_L1_STORE_PIPE = int(os.environ.get("W_L1_STORE_PIPE", "0"))   # v32: L1 async store (opt-in; wash on MoE-7)
    USE_D8_TMA1D = int(os.environ.get(
        "W_D8_TMA1D_LEVEL",
        "2" if int(os.environ.get("W_D8_TMA1D", "1")) else "0",   # v31: default ON (= v25 D8 raw TMA1D) + dual-math
    ))
    # Adopted after matched 8-rank A/B; either can still be disabled for the
    # exact v31 control path.
    USE_SMEM_EXPERT_COUNT = int(os.environ.get("W_SMEM_EXPERT_COUNT", "1"))
    FAST_NVLINK_BARRIER = int(os.environ.get("W_FAST_NVLINK_BARRIER", "1"))
    D8_PULL_STREAMS = int(os.environ.get("W_D8_PULL_STREAMS", "1"))
    ...
    pos = (topk_local, tok_tab, sf_tab, w_tab, q_tab, recv_tab, rsum_tab, sig_tab,
           ...
           final_y, a1_desc, b1_desc, a2_desc, b2_desc, sfa1_desc, sfa2_desc, *_tp, l1_st_desc)
    ...
    try:
        ...
        print('V31_FULLERR>>> '+repr(c)[:300]+' FILENAME='+str(getattr(c,'filename',None))+' args='+str(getattr(c,'args',None)), flush=True); raise
    barrier()
