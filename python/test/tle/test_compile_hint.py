...
@triton.jit
def matmul_ref(mat_a, mat_b, mat_c, M: tl.constexpr, N: tl.constexpr, K: tl.constexpr,
...
def test_may_discrete_ub_overflow():
    ...
    ref_time = do_bench_npu(lambda: matmul_ref[grid](mat_a, mat_b, ref, M, N, K,
                                                      BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N, BLOCK_K=BLOCK_K,
                                                      multibuffer=False),
                            clear_l2_cache=True, collect_prof=False)
    hint_time = do_bench_npu(lambda: triton_compile_hint_may_discrete[grid](mat_a, mat_b, mat_c, M, N, K,
                                                                            BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N, BLOCK_K=BLOCK_K,
                                                                            multibuffer=False),
                             clear_l2_cache=True, collect_prof=False)
    print(f"[BENCH] mayDiscretememaccess (hint): {hint_time:.2f} us, (no hint): {ref_time:.2f} us, ratio={hint_time / ref_time:.3f}")
...
# ═══════════════════════════════════════════════════════════════════════════════
@triton.jit
def triton_dot_pad_only_k(mat_a, mat_b, mat_c, M: tl.constexpr, N: tl.constexpr, K: tl.constexpr,
                         BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, BLOCK_K: tl.constexpr):
    pid = tl.program_id(0)
    num_blocks_n = triton.cdiv(N, BLOCK_N)
    block_m = pid // num_blocks_n
...
def test_dot_pad_only_k():
    ...
    mat_c = torch.zeros((M, N), dtype=dtype).npu()
    triton_dot_pad_only_k[grid](mat_a, mat_b, mat_c, M, N, K,
                                 BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N, BLOCK_K=BLOCK_K)
    # print(mat_c)
    torch.testing.assert_close(mat_c, ref, rtol=1e-2, atol=1e-2)
    ...
    ref_time = do_bench_npu(lambda: matmul_ref[grid](mat_a, mat_b, ref, M, N, K,
                                                      ...
                                                      multibuffer=False),
                            clear_l2_cache=True, collect_prof=False)
    hint_time = do_bench_npu(lambda: triton_dot_pad_only_k[grid](mat_a, mat_b, mat_c, M, N, K,
                                                                            BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N, BLOCK_K=BLOCK_K,
                                                                            multibuffer=False),
                             clear_l2_cache=True, collect_prof=False)
    print(f"[BENCH] dot_pad_only_k (hint): {hint_time:.2f} us, (no hint): {ref_time:.2f} us, ratio={hint_time / ref_time:.3f}")
...
# ═══════════════════════════════════════════════════════════════════════════════
@triton.jit
def triton_flash_attn_ref(Q_ptr, K_ptr, V_ptr, O_ptr,
                          stride_q_m, stride_q_k,
                          stride_k_n, stride_k_k,
                          stride_v_n, stride_v_d,
                          stride_o_m, stride_o_d,
                          M: tl.constexpr, N: tl.constexpr, K: tl.constexpr, D: tl.constexpr,
...
@triton.jit
def triton_flash_attn_tile_mix_cube_num(Q_ptr, K_ptr, V_ptr, O_ptr,
                                        stride_q_m, stride_q_k,
                                        stride_k_n, stride_k_k,
                                        stride_v_n, stride_v_d,
                                        stride_o_m, stride_o_d,
...
def test_tile_mix_cube_num():
    ...
    print(f"[BENCH] tile_mix_cube_num (hint): {hint_time:.2f} us, (no hint): {ref_time:.2f} us, ratio={hint_time / ref_time:.3f}")
...
# ═══════════════════════════════════════════════════════════════════════════════
@triton.jit
def triton_rms_slice_ref(x_ptr, w_ptr, out_ptr, M: tl.constexpr, N: tl.constexpr,
...
@triton.jit
def triton_rms_slice_disable_bubble_up(x_ptr, w_ptr, out_ptr, M: tl.constexpr,
...
def test_disable_bubble_up():
    ...
    ref_time = do_bench_npu(
        lambda: triton_rms_slice_ref[grid](
            x, w, ref, M, N, BLOCK_SIZE, eps, multibuffer=False,
        ),
        clear_l2_cache=True, collect_prof=False,
    )
    hint_time = do_bench_npu(
        lambda: triton_rms_slice_disable_bubble_up[grid](
            x, w, out, M, N, BLOCK_SIZE, eps, multibuffer=False,
        ),
        clear_l2_cache=True, collect_prof=False,
    )
    print(f"[BENCH] disable_bubble_up (hint): {hint_time:.2f} us, (no hint): {ref_time:.2f} us, ratio={hint_time / ref_time:.3f}")
...
# ═══════════════════════════════════════════════════════════════════════════════
@triton.jit
def triton_where_ref(in_ptr0, in_ptr1, cond_ptr, out_ptr0, xnumel,
...
@triton.jit
def triton_where_bitwise_mask(in_ptr0, in_ptr1, cond_ptr, out_ptr0, xnumel,
                               XBLOCK: tl.constexpr, XBLOCK_SUB: tl.constexpr):
...
def test_bitwise_mask():
    ...
    ref_time = do_bench_npu(
        lambda: triton_where_ref[grid](
            in0, in1, cond, ref, xnumel, XBLOCK, XBLOCK_SUB, multibuffer=False,
        ),
        clear_l2_cache=True, collect_prof=False,
    )
    hint_time = do_bench_npu(
        lambda: triton_where_bitwise_mask[grid](
            in0, in1, cond, out, xnumel, XBLOCK, XBLOCK_SUB, multibuffer=False,
        ),
        clear_l2_cache=True, collect_prof=False,
    )
    print(f"[BENCH] bitwise_mask (hint): {hint_time:.2f} us, (no hint): {ref_time:.2f} us, ratio={hint_time / ref_time:.3f}")
