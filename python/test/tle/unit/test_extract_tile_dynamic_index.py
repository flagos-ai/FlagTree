...
@triton.jit
def simple_extract_kernel(x_ptr, out_ptr, stride_xb, stride_xm, stride_xn, stride_ob, stride_om,
                          stride_on,  # Strides for the output tensor
    ...
    extracted_tile = tle.extract_tile(bg_tile, index=[tile_idx_m, tile_idx_n], tile_shape=[TILE_M, TILE_N])
...
def main():
    ...
    simple_extract_kernel[grid](x, out, x.stride(0), x.stride(1), x.stride(2), out.stride(0), out.stride(1),
                                out.stride(2), BLOCK_M=32, BLOCK_N=32, TILE_M=16, TILE_N=16)
