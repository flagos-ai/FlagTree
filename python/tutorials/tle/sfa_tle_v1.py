...
def fused_pa_rope_to_sparse_kernel(
    k_pa_ptr, k_rope_pa_ptr, v_pa_ptr,           # PA_BSND input [block_num, block_size, n, d]
    block_table_ptr,                               # block_table [B, max_blocks]
    sparse_indices_ptr,                            # sparse_indices [B, N, TOPK]
    k_sparse_out_ptr, v_sparse_out_ptr,            # BNSD output [B, N, TOPK, d]
    stride_k_pa_bn, stride_k_pa_bs, stride_k_pa_n, stride_k_pa_d,   # K PA strides
    stride_k_rope_pa_bn, stride_k_rope_pa_bs, stride_k_rope_pa_n, stride_k_rope_pa_d,  # K_rope PA strides
    stride_v_pa_bn, stride_v_pa_bs, stride_v_pa_n, stride_v_pa_d,   # V PA strides
    stride_bt_b, stride_bt_blk,                    # block_table strides
    stride_si_b, stride_si_n, stride_si_topk,     # sparse_indices strides
    stride_out_b, stride_out_n, stride_out_topk, stride_out_d,     # output strides
    stride_v_b, stride_v_n, stride_v_topk, stride_v_d,
    ...
    for b_idx in range(B):
        ...
        for idx in range(pid, TOPK, num_programs):
            ...
            k_pa_offset = (actual_block_id * stride_k_pa_bn +
                        bs_offset * stride_k_pa_bs +
                        n * stride_k_pa_n)

            # Compute PA_BSND offset for K_rope
            k_rope_pa_offset = (actual_block_id * stride_k_rope_pa_bn +
                                bs_offset * stride_k_rope_pa_bs +
                                n * stride_k_rope_pa_n)

            # Compute PA_BSND offset for V
            v_pa_offset = (actual_block_id * stride_v_pa_bn +
                        bs_offset * stride_v_pa_bs +
                        n * stride_v_pa_n)
            # Load K vector (no rope part)
            k_vec = tl.load(
                k_pa_ptr + k_pa_offset +
                tl.arange(0, BLOCK_DK) * stride_k_pa_d
            )

            # Load V vector
            v_vec = tl.load(
                v_pa_ptr + v_pa_offset +
                tl.arange(0, BLOCK_DV) * stride_v_pa_d
            )
            ...
            if BLOCK_DK_ROPE > 0:
                ...
                k_rope_vec = tl.load(
                    k_rope_pa_ptr + k_rope_pa_offset +
                    tl.arange(0, BLOCK_DK_ROPE) * stride_k_rope_pa_d
                )
                full_k = tle.dsa.insert_slice(full_k, k_vec, offsets=(0,), sizes=(BLOCK_DK,), strides=(1,))
                full_k = tle.dsa.insert_slice(full_k, k_rope_vec, offsets=(BLOCK_DK,), sizes=(BLOCK_DK_ROPE,), strides=(1,))
                tl.store(
                    k_sparse_out_ptr + out_offset +
            ...
            # Store V
            tl.store(
                v_sparse_out_ptr + out_offset_v +
                tl.arange(0, BLOCK_DV) * stride_v_d,
                v_vec
            )
...
def triton_fused_pa_rope_to_sparse(k_pa, k_rope_pa, v_pa, block_table, sparse_indices, block_size):
    ...
    fused_pa_rope_to_sparse_kernel[grid](
        k_pa, k_rope_pa_input, v_pa,
        block_table,
        sparse_indices_input,
        k_sparse, v_sparse,
        k_pa.stride(0), k_pa.stride(1), k_pa.stride(2), k_pa.stride(3),
        k_rope_pa_input.stride(0), k_rope_pa_input.stride(1), k_rope_pa_input.stride(2), k_rope_pa_input.stride(3),
        v_pa.stride(0), v_pa.stride(1), v_pa.stride(2), v_pa.stride(3),
        block_table.stride(0), block_table.stride(1),
        sparse_indices_input.stride(0), sparse_indices_input.stride(1), sparse_indices_input.stride(2),
...
def gather_kv_bnsd_vec_kernel(
    ...
    stride_vb, stride_vn, stride_vs, stride_vd,
    stride_ob, stride_on, stride_os, stride_od,
    stride_ovb, stride_ovn, stride_ovs, stride_ovd,
    BLOCK_DK: tl.constexpr,
    BLOCK_DV: tl.constexpr,
    TOPK: tl.constexpr,
    B: tl.constexpr,
...
def triton_gather_kv_bnsd_vec(k, v, indices):
    ...
    gather_kv_bnsd_vec_kernel[grid](
        k, v, indices.squeeze(0),  # [B, N, SK, D] -> [N, SK, D]
        k_sparse, v_sparse,
...
def _attn_fwd(
    Q, K, V, O, scale_value,
    stride_qb: tl.constexpr, stride_qs: tl.constexpr, stride_qn: tl.constexpr, stride_qd: tl.constexpr,
    stride_kb: tl.constexpr, stride_kn: tl.constexpr, stride_ks: tl.constexpr, stride_kd: tl.constexpr,
    stride_vb: tl.constexpr, stride_vn: tl.constexpr, stride_vs: tl.constexpr, stride_vd: tl.constexpr,
    stride_ob: tl.constexpr, stride_os: tl.constexpr, stride_on: tl.constexpr, stride_od: tl.constexpr,
    B: tl.constexpr,
    Q_N: tl.constexpr, Q_D: tl.constexpr, Q_S: tl.constexpr,
    KV_S: tl.constexpr, K_D: tl.constexpr, V_D: tl.constexpr,
    sparse_mode: tl.constexpr, # 0 or 3
    O_N:tl.constexpr, O_D: tl.constexpr,
    ...
    for block_idx in range(pid, NUM_BLOCKS, num_cores): # 并行
        ...
        for i  in range(cur_act_s_q):
            ...
            k_block_ptr = tl.make_block_ptr(base = K + k_offset,
                                            ...
                                            order = (1, 0),)
            v_block_ptr = tl.make_block_ptr(base = V + v_offset,
                                            shape = (KV_S, V_D),
            ...
            for k_idx  in range(KV_S // blk_size):
                ...
                qk  = tl.dot(q_vec.to(tl.float16), tl.trans(k_vec).to(tl.float16)) * scale_value  # [q_block_size, blk_size]
            ...
            o_block_ptr = tl.make_block_ptr(base = O + o_offset,
                            shape = (O_N, O_D),
...
class _attention(torch.autograd.Function):
    ...
    def forward(
        ...
        if layout_query == 'TND':
            # t = B*act_q_s
            output = torch.empty((total_len, out_shape_bsnd[2], out_shape_bsnd[3]), device=query.device, dtype=torch.float32)
            _attn_fwd_fused_bsnd_to_tnd[(num_cores,)](
                ...
                B = B, Q_N = Q_N, Q_D = Q_D, Q_S = Q_S,
                KV_S = KV_S, K_D = K_D, V_D = v_sparse.shape[3],
                sparse_mode = sparse_mode, O_N = output.shape[1], O_D = output.shape[2],
            ...
            _attn_fwd[(num_cores,)](
                ...
                B = B, Q_N = Q_N, Q_D = Q_D, Q_S = Q_S,
                KV_S = KV_S, K_D = K_D, V_D = v_sparse.shape[3],
                sparse_mode = sparse_mode, O_N = output.shape[2], O_D = output.shape[3],
                actual_seq_lengths_query = actual_seq_lengths_query,
                actual_seq_lengths_kv = actual_seq_lengths_kv,
...
def trans_tnd_to_bsnd_fused_kernel(
    query_ptr, query_rope_ptr, sparse_ptr,
    query_out_ptr, sparse_out_ptr,  # query_out 已经拼接了 rope
    act_s,
    stride_q_t, stride_q_tn, stride_q_td,
    stride_qr_t, stride_qr_tn, stride_qr_td,
    stride_s_t, stride_s_tn, stride_s_td,
    stride_qob, stride_qobs, stride_qon, stride_qod,  # query_out strides
    stride_sb, stride_sbs, stride_sbn, stride_sbd,
    B: tl.constexpr,
    N: tl.constexpr,
    ...
    for tn_id in range(B):
        ...
        if pid == 0:
            sparse_block_ptr = tl.make_block_ptr(base = sparse_ptr + t_idx * stride_s_t,
                                            shape = (1, D_SPARSE),
            ...
            sparse_out_block_ptr = tl.make_block_ptr(base = sparse_out_ptr + t_idx * stride_sb,
                                            shape = (1, D_SPARSE),
        ...
        for head_block_id in range(pid, num_head_blocks, num_programs):
            ...
            # Load q and q_ro
            q_block_ptr = tl.make_block_ptr(base = query_ptr + t_idx * stride_q_t,
                                            shape = (N, D_QUERY),
                                            strides = (stride_q_tn, stride_q_td),
                                            offsets = (n_offset, 0),
                                            block_shape = (BLOCK_N, D_QUERY),
                                            order = (1, 0))
            q_ro_block_ptr = tl.make_block_ptr(base = query_rope_ptr + t_idx * stride_qr_t,
                                    shape = (N, D_ROPE),
            ...
            q_out_block_ptr = tl.make_block_ptr(base = query_out_ptr + t_idx * stride_qob,
...
def trans_tnd_to_bsnd_fused(query, query_rope, sparse_indices, shape, act_seq, grid=(16,)):
    ...
    trans_tnd_to_bsnd_fused_kernel[num_programs,](
        query, query_rope, sparse_indices,
        query_out, sparse_out,
        act_seq,
        ...
        sparse_out.stride(0), sparse_out.stride(1), sparse_out.stride(2), sparse_out.stride(3),
        B=b,
        N=n,
        D_QUERY=d_query,
        D_ROPE=d_rope,
...
def sparse_attention(query, key, value,
                     sparse_indices, scale_value, sparse_block_size = 1,
...
if __name__ == "__main__":
    ...
    i += 1
    print(f"====================第{i}次测试=================")
    test_op(T = 4, B = 4, KV_S = 6400, Q_N = 128, KV_N = 1, D = 512, D_rope = 64,
        sparse_size = 2048, scale_value = 0.5, sparse_block_size = 1, sparse_mode = 0,
        block_size = 128, act_kv_s = 2560)
    i += 1
    print(f"====================第{i}次测试=================")
    test_op(T = 8, B = 8, KV_S = 48000, Q_N = 128, KV_N = 1, D = 512, D_rope = 64,
        sparse_size = 2048, scale_value = 0.5, sparse_block_size = 1, sparse_mode = 0,
        block_size = 128, act_kv_s = 2560)
    i += 1
    print(f"====================第{i}次测试=================")
    test_op(T = 16, B = 16, KV_S = 48000, Q_N = 128, KV_N = 1, D = 512, D_rope = 64,
        sparse_size = 2048, scale_value = 0.5, sparse_block_size = 1, sparse_mode = 0,
        block_size = 128, act_kv_s = 2560)
