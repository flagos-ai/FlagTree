...
ar_multimem_ar_vector_multicast_store(
    __attribute__((address_space(1))) __half *mc_ptr,
    __attribute__((address_space(1))) __half *mc_out_ptr, int offset) {
    ...
    asm volatile(
      ...
      :: "l"(mc_out_ptr + offset), "r"(v0), "r"(v1), "r"(v2), "r"(v3)
      : "memory");
}
...
ar_load_acquire_sys_u64(
    __attribute__((address_space(1))) const uint64_t *ptr) {
    uint64_t value;
...
extern "C" __device__ __attribute__((always_inline)) void
ar_mark_tile_ready(__attribute__((address_space(1))) uint64_t *ready, int tile_id,
                   int rank, int num_tiles, uint64_t epoch) {
...
extern "C" __device__ __attribute__((always_inline)) void
ar_wait_tile_ready(
    __attribute__((address_space(1))) const uint64_t *peer_ready_ptrs,
...
extern "C" __device__ __attribute__((always_inline)) void
ar_multimem_store_barrier(
    __attribute__((address_space(1))) uint64_t *barrier,
    __attribute__((address_space(1))) const uint64_t *peer_barrier_ptrs,
