...
# ==================================================================
# dialect 注册
# 每个 device 函数对应一个 dialect，供 triton kernel 通过 tle_raw.call 调用
# ==================================================================
...
def no_overlap_kernel(
    ...
    expected: triton.language.constexpr,
    mat_A,
    mat_B,
    mat_C,
...
def run_experiment():
    ...
    # 让不同 block 的 matmul 独立，按 block 偏移寻址
    ...
    no_overlap_put_cycles = torch.zeros(num_blocks, NUM_CHUNKS, dtype=torch.uint64, device=device)
    no_overlap_wait_cycles = torch.zeros(num_blocks, NUM_CHUNKS, dtype=torch.uint64, device=device)
    ...
    # ---- 预热（warmup） ----
    with torch.cuda.stream(curr_stream):
        no_overlap_kernel[(num_blocks,)](
            dst_tensor, src_tensor, size, signal_tensor,
            no_overlap_put_cycles, no_overlap_wait_cycles,
    ...
    reset_tensors_on_stream(curr_stream, no_overlap_put_cycles, no_overlap_wait_cycles)

    evt_start_no = torch.cuda.Event(enable_timing=True)
    evt_end_no   = torch.cuda.Event(enable_timing=True)
    with torch.cuda.stream(curr_stream):
        evt_start_no.record(curr_stream)
        no_overlap_kernel[(num_blocks,)](
            dst_tensor, src_tensor, size, signal_tensor,
            no_overlap_put_cycles, no_overlap_wait_cycles,
            chunk_size, NUM_CHUNKS,
        ...
        evt_end_no.record(curr_stream)

    curr_stream.synchronize()
    time_no_overlap = evt_start_no.elapsed_time(evt_end_no)  # ms
    lib.ring_reduce_barrier_all()

    clock_rate_khz = torch.cuda.get_device_properties(device).clock_rate
    no_overlap_put_stats = format_cycle_stats(no_overlap_put_cycles, clock_rate_khz)
    no_overlap_wait_stats = format_cycle_stats(no_overlap_wait_cycles, clock_rate_khz)
    ...
    if pe == 0 or pe == 1:
        ...
        print("-" * 60)
        print(f"  方案二 no-overlap: {time_no_overlap:.3f} ms")
        print("-" * 60)
        print("  device clock64 统计:")
        print(f"    no-overlap put_nbi 调用     : {no_overlap_put_stats}")
        print(f"    no-overlap quiet + wait/chunk : {no_overlap_wait_stats}")
        print()
