...
_QUALIFIERS = re.compile(r"\b(?:const|volatile|restrict|__restrict__|__restrict|__const__|"
...
def parse_cuda_functions(source: str) -> tuple[Function, ...]:
    ...
    for match in _FUNCTION.finditer(source):
        ...
        parameters = tuple(
            _parse_parameter(parameter, match.group("name")) for parameter in _split_parameters(match.group("params")))
...
def generate(
    ...
    if required_function is not None and required_function not in names:
        raise ValueError(f"extern function {required_function!r} was not found in {cuda_file}; "
