...
load_pack128(const Pack128 *ptr) {
  ...
  asm volatile("ld.global.v4.u32 {%0, %1, %2, %3}, [%4];"
               : "=r"(value.data[0]), "=r"(value.data[1]), "=r"(value.data[2]),
                 "=r"(value.data[3])
               : "l"(ptr)
               : "memory");
  return value;
...
static __device__ __attribute__((always_inline)) Pack128 poison_pack128() {
  Pack128 value;
#pragma unroll
  for (int word = 0; word < 4; ++word) {
    value.data[word] = kPoisonWord;
  }
  return value;
}

static __device__ __attribute__((always_inline)) void
canonicalize_bf16_negative_zero(Pack128 &value) {
#pragma unroll
...
static __device__ __attribute__((always_inline)) Pack128
reduce_pack128_fp32(const Pack128 *values, int world_size) {
...
#pragma unroll
    for (int pair = 0; pair < 4; ++pair) {
      ...
      float2 pair_value = __bfloat1622float2(__nv_bfloat162(bits.raw));
...
fused_ar_rmsnorm_allreduce_impl(const __nv_bfloat16 *input,
                                const uint64_t *peer_scatter_ptrs,
                                void *multicast_broadcast_raw, int num_tokens,
                                int packs_per_token, int rank, int world_size) {
  ...
  int cluster_size = static_cast<int>(gridDim.y);
  int block_rank = static_cast<int>(blockIdx.y);
  ...
  Pack128 *multicast_broadcast =
      reinterpret_cast<Pack128 *>(multicast_broadcast_raw);
  ...
  for (int pack = pack_begin + tid; pack < pack_end; pack += nthreads) {
    ...
    canonicalize_bf16_negative_zero(contribution);
    Pack128 *scatter_destination =
        reinterpret_cast<Pack128 *>((uintptr_t)peer_scatter_ptrs[owner]);
  ...
  if (owner == rank) {
    Pack128 *local_scatter =
        reinterpret_cast<Pack128 *>((uintptr_t)peer_scatter_ptrs[rank]);
    for (int pack = pack_begin + tid; pack < pack_end; pack += nthreads) {
      ...
      while (!all_ready) {
        ...
        for (int source = 0; source < world_size; ++source) {
          size_t source_offset =
              ...
              pack;
          values[source] = load_pack128_volatile(local_scatter + source_offset);
          all_ready &= values[source].data[0] != kPoisonWord;
          all_ready &= values[source].data[1] != kPoisonWord;
          all_ready &= values[source].data[2] != kPoisonWord;
          all_ready &= values[source].data[3] != kPoisonWord;
        }
      }

      Pack128 poison = poison_pack128();
      for (int source = 0; source < world_size; ++source) {
        size_t source_offset =
            ((size_t)local_token * world_size + source) * packs_per_token +
            pack;
      ...
      Pack128 result = reduce_pack128_fp32(values, world_size);
...
fused_ar_rmsnorm_allreduce_bf16(
    ...
    __attribute__((address_space(1))) void *multicast_broadcast_raw,
    int num_tokens, int packs_per_token, int rank, int world_size) {
  fused_ar_rmsnorm_allreduce_impl(input, peer_scatter_ptrs,
                                  multicast_broadcast_raw, num_tokens,
