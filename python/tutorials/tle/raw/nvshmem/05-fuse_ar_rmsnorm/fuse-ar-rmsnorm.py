...
def _device_dialect(function_name):
    @dialect(name="cuda", compiler="clang", file=CU_FILE,
...
@triton.jit
def allreduce_kernel(input_ptr, peer_scatter_ptrs, multicast_broadcast,
                     num_tokens, packs_per_token,
                     rank: tl.constexpr, world_size: tl.constexpr,
                     CLUSTER: tl.constexpr):
    tle_raw.call(_ALLREDUCE_FUNC, [
        input_ptr, peer_scatter_ptrs, multicast_broadcast,
...
@triton.jit
def _poison_broadcast_pack(bc_ptr, token, packs_per_token, pack_idx, m,
                           BLOCK_PACKS: tl.constexpr):
    wptr = bc_ptr + ((token * packs_per_token + pack_idx)[:, None] * 4
                     + tl.arange(0, 4)[None, :])
    tl.store(wptr, tl.full([BLOCK_PACKS, 4], 0x80000000, tl.uint32),
             mask=m[:, None])


@triton.jit
def _spin_broadcast_pack(bc_ptr, token, packs_per_token, pack_idx, m,
                         BLOCK_PACKS: tl.constexpr):
    wptr = bc_ptr + ((token * packs_per_token + pack_idx)[:, None] * 4
                     + tl.arange(0, 4)[None, :])
    words = tle.load(wptr, mask=m[:, None], other=0, volatile=True)
    ready = tl.sum(tl.sum(
        tl.where(m[:, None], words != 0x80000000, True).to(tl.int32), axis=1),
...
def fused_ar_rmsnorm_norm_bf16(
    ...
    lane = tl.arange(0, BLOCK_PACKS)
    elem = tl.arange(0, 8)
    row = token * packs_per_token

    s3 = tl.arange(0, PREFETCH)
    p3 = pack_begin + s3[:, None, None] * BLOCK_PACKS + lane[None, :, None]
    m3 = p3 < pack_end
    e3 = elem[None, None, :]
    pref_r = tl.load(residual_ptr + (row + p3) * 8 + e3, mask=m3, other=0.0)
    pref_g = tl.load(gamma_ptr + p3 * 8 + e3, mask=m3, other=0.0)
    ...
    for slot in tl.static_range(PREFETCH):
        pack_idx = pack_begin + slot * BLOCK_PACKS + lane
        m = pack_idx < pack_end
        if slot < n_iters:
            words = _spin_broadcast_pack(local_broadcast_ptr, token,
                                         packs_per_token, pack_idx, m,
                                         BLOCK_PACKS)
            x = _pack_words_to_bf16(words).to(tl.float32)

            r = tl.reshape(tle.extract_tile(pref_r, index=[slot, 0, 0],
                                            tile_shape=(1, BLOCK_PACKS, 8)),
                           (BLOCK_PACKS, 8)).to(tl.float32)
            y = x + r
            pre_bits = _float_to_bf16_bits(y)
            tl.store(prenorm_out_ptr + (row + pack_idx)[:, None] * 8 + elem[None, :],
                     pre_bits.to(tl.bfloat16, bitcast=True), mask=m[:, None])
            if not PDL_CHAIN:
                _poison_broadcast_pack(local_broadcast_ptr, token,
                                       packs_per_token, pack_idx, m,
                                       BLOCK_PACKS)
            acc += tl.where(m[:, None], y * y, 0.0)

    for it in range(PREFETCH, n_iters):
        pack_idx = pack_begin + it * BLOCK_PACKS + lane
        m = pack_idx < pack_end
        r = tl.load(residual_ptr + (row + pack_idx)[:, None] * 8 + elem[None, :],
                    mask=m[:, None], other=0.0)
        words = _spin_broadcast_pack(local_broadcast_ptr, token, packs_per_token,
                                     pack_idx, m, BLOCK_PACKS)
        x = _pack_words_to_bf16(words).to(tl.float32)
        y = x + r.to(tl.float32)
        pre_bits = _float_to_bf16_bits(y)
        tl.store(prenorm_out_ptr + (row + pack_idx)[:, None] * 8 + elem[None, :],
        ...
        if not PDL_CHAIN:
            _poison_broadcast_pack(local_broadcast_ptr, token,
                                   packs_per_token, pack_idx, m, BLOCK_PACKS)
        acc += tl.where(m[:, None], y * y, 0.0)
    ...
    if CLUSTER > 1:
        partial_smem = tle.gpu.alloc([CLUSTER], dtype=tl.float32, layout=None,
                                     scope=tle.gpu.smem,
                                     nv_mma_shared_layout=False)
        tle.distributed_barrier()
        for dst in tl.static_range(CLUSTER):
        ...
        for i in tl.static_range(CLUSTER):
    ...
    for slot in tl.static_range(PREFETCH):
        ...
        m = pack_idx < pack_end
        if slot < n_iters:
            pre = tl.load(prenorm_out_ptr + (row + pack_idx)[:, None] * 8 + elem[None, :],
                          mask=m[:, None], other=0.0).to(tl.float32)
            g = tl.reshape(tle.extract_tile(pref_g, index=[slot, 0, 0],
                                            tile_shape=(1, BLOCK_PACKS, 8)),
                           (BLOCK_PACKS, 8)).to(tl.float32)
            z = pre * g * rcp_rms
    ...
    for it in range(PREFETCH, n_iters):
        ...
        pre = tl.load(prenorm_out_ptr + (row + pack_idx)[:, None] * 8 + elem[None, :],
                      mask=m[:, None], other=0.0).to(tl.float32)
        g = tl.load(gamma_ptr + pack_idx[:, None] * 8 + elem[None, :],
                    mask=m[:, None], other=0.0).to(tl.float32)
        z = pre * g * rcp_rms
        out_bits = _float_to_bf16_bits(z)
        tl.store(norm_out_ptr + (row + pack_idx)[:, None] * 8 + elem[None, :],
...
class SymmetricWorkspace:
    ...
    def __init__(self, world_size, rank, max_token_num, hidden_dim, dtype,
                 group):
        ...
        self.local_slots = symm_mem.empty(shape, dtype=torch.uint32,
                                          device="cuda")
        ...
        self.multicast_slots = tensor_from_pointer(
            ctypes.c_void_p(self.handle.multicast_ptr), shape, torch.uint32,
            self.local_slots.device)
        self.peer_slots = tuple(
            self.handle.get_buffer(p, shape, torch.uint32)
            for p in range(world_size))
        self.peer_pointer_tables = torch.tensor(
            [[self.peer_slots[p][0, 0].data_ptr() for p in range(world_size)]],
            dtype=torch.uint64, device=self.local_slots.device)

        cluster = min(CLUSTER_MAX, packs_per_token)
        self.cluster_scratch = torch.zeros(
            (max_token_num, cluster + 1), dtype=torch.float32,
...
def fused_allreduce_residual_rmsnorm(x, residual, gamma, eps, workspace,
                                     rank, world_size,
                                     norm_out=None, residual_out=None):
    ...
    allreduce_kernel[(num_tokens, cluster)](
        x, workspace.peer_pointer_tables[0],
        workspace.multicast_slots[0, 1],
        num_tokens, packs_per_token,
        rank=rank, world_size=world_size, CLUSTER=cluster,
        num_warps=BLOCK // 32,
        launch_pdl=PDL_CHAIN,
    )

    fused_ar_rmsnorm_norm_bf16[(num_tokens,)](
        norm_out, residual_out, residual, gamma,
        workspace.local_slots[0, 1], workspace.cluster_scratch,
        num_tokens, packs_per_token, hidden, eps,
        CLUSTER=cluster, BLOCK_PACKS=block_packs, PREFETCH=1,
        PDL_CHAIN=PDL_CHAIN,
        num_warps=triton_warps, cluster_dims=(1, cluster, 1),
        launch_pdl=True,
    )
...
def torch_reference(x, residual, gamma, eps, group):
    ...
    norm = (prenorm.to(torch.float32) * gamma.to(torch.float32)
            * rcp_rms).to(torch.bfloat16)
    return prenorm, norm


def check(x, residual, gamma, eps, workspace, rank, world_size, group):
    norm_out, residual_out = fused_allreduce_residual_rmsnorm(
        x, residual, gamma, eps, workspace, rank, world_size)
    torch.cuda.synchronize()
    ref_prenorm, ref_norm = torch_reference(x, residual, gamma, eps, group)

    pre_bit = torch.equal(residual_out, ref_prenorm)
    pre_err = (residual_out.float() - ref_prenorm.float()).abs().max().item()
    norm_err = (norm_out.float() - ref_norm.float()).abs().max().item()
    n_bit_diff = (norm_out.view(torch.int16)
                  != ref_norm.view(torch.int16)).sum().item()

    # one bf16 ulp at the observed magnitude
    scale = max(ref_norm.float().abs().max().item(), 1.0)
    ulp = 2.0 ** (math.floor(math.log2(scale)) - 7)
    ok = pre_bit and norm_err <= ulp

    if rank == 0:
        print(f"  prenorm bit-exact : {pre_bit} (max |err| = {pre_err:.3e})",
              flush=True)
        print(f"  norm    max |err| : {norm_err:.6f}  ({norm_err / ulp:.2f} ulp)"
              f"  bit-different: {n_bit_diff}/{norm_out.numel()}", flush=True)
...
def _time_launches(launch, iters, warmup, x, group):
    ...
    worst = torch.tensor([local], device=x.device, dtype=torch.float64)
    dist.all_reduce(worst, op=dist.ReduceOp.MAX, group=group)
    return worst.item()


def benchmark(x, residual, gamma, eps, workspace, rank, world_size,
              warmup, iters, group, use_cuda_graph=True):
    norm_out = torch.empty_like(x)
    residual_out = torch.empty_like(x)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="two-kernel fused AllReduce + Residual + RMSNorm")
    parser.add_argument("--hidden", type=int, default=8192,
                        help="hidden size (must be divisible by 8)")
    parser.add_argument("--tokens", type=int, default=32,
                        help="decode batch size M")
    parser.add_argument("--eps", type=float, default=1e-5)
    parser.add_argument("--iters", type=int, default=200)
    parser.add_argument("--warmup", type=int, default=20)
    parser.add_argument("--skip-check", action="store_true",
                        help="skip the torch reference comparison")
    parser.add_argument("--no-cuda-graph", action="store_true",
                        help="only time the eager launch loop")
...
def main():
    ...
    residual = (torch.randn((M, H), dtype=dtype, device=device) * 0.1)
    gamma = torch.randn((H,), dtype=dtype, device=device)

    workspace = SymmetricWorkspace(world_size, rank, M, H, dtype, group)
    cluster = pick_cluster(M, H * dtype.itemsize // PACK_BYTES)
    if rank == 0:
        print(f"=== fused AllReduce + Residual + RMSNorm ===", flush=True)
        print(f"  world_size={world_size} M={M} hidden={H} "
              f"dtype={dtype} pdl_chain={PDL_CHAIN}", flush=True)
        print(f"  cluster={cluster} "
              f"(SM count {_sm_count()}, numTokens*cluster="
              f"{M * cluster})", flush=True)
        print(f"  workspace: scatter+broadcast slots "
              f"{tuple(workspace.local_slots.shape)}, "
              f"multicast={'yes' if workspace.handle.multicast_ptr else 'no'}",
              flush=True)
    ...
    if not args.skip_check:
        ...
        ok = check(x, residual, gamma, args.eps, workspace, rank, world_size,
                   group)
    ...
    eager_us, graph_us = benchmark(
        x, residual, gamma, args.eps, workspace, rank, world_size,
        args.warmup, args.iters, group,
        use_cuda_graph=not args.no_cuda_graph)
    payload = H * M * dtype.itemsize          # one rank's all-reduce shard
    if rank == 0:
        print(f"  eager launch loop (p50, max over ranks): {eager_us:8.2f} us",
              flush=True)
        if graph_us is not None:
            print(f"  CUDA graph replay (p50, max over ranks): {graph_us:8.2f} us"
