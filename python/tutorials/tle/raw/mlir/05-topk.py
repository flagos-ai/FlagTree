...
def kernel_bucket_sort_topk_edsl(  # grid(B,)
    ...
    s_histogram_smem = tle.alloc(
        shape=[HISTOGRAM_SIZE],
        dtype=tl.int32,
        layout=None,
        scope=tle.smem,
        init_value=s_histogram_init,
        nv_mma_shared_layout=False
    )

    # Kernel2: Call edsl1 for topk selection (threshold calculated in edsl1)
    thre_bin_sum_buf_init = tl.zeros([1], dtype=tl.int32)
    thre_bin_sum_buf_smem = tle.alloc(
        shape=[1],
        dtype=tl.int32,
        layout=None,
        scope=tle.smem,
        init_value=thre_bin_sum_buf_init,
        nv_mma_shared_layout=False
    )

    l_new_topk_buf_init = tl.zeros([1], dtype=tl.int32)
    l_new_topk_buf_smem = tle.alloc(
        shape=[1],
        dtype=tl.int32,
        layout=None,
        scope=tle.smem,
        init_value=l_new_topk_buf_init,
        nv_mma_shared_layout=False
    )

    s_threshold_bin_id_init = tl.zeros([1], dtype=tl.int32)
    s_threshold_bin_id_smem = tle.alloc(
        shape=[1],
        dtype=tl.int32,
        layout=None,
        scope=tle.smem,
        init_value=s_threshold_bin_id_init,
        nv_mma_shared_layout=False
    )
    s = S
    bs = BS
    k_tensor_init = tl.full([1], K, dtype=tl.int32)  # Convert constexpr to tensor
    k_tensor_smem = tle.alloc(
        shape=[1],
        dtype=tl.int32,
        layout=None,
        scope=tle.smem,
        init_value=k_tensor_init,
        nv_mma_shared_layout=False
    )
