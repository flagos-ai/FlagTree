...
def _fused_inv_rope_fp8_quant_per_head(
    ...
    for i in tl.range(0, HEADS_PER_PROGRAM, loop_unroll_factor=HEADS_PER_PROGRAM, reorder=True):
        global_head = pid_gh_base + i
        g = global_head // heads_per_group
        head_in_group = global_head % heads_per_group
        qb_start = head_in_group * CHUNKS_PER_HEAD
...
def fused_inv_rope_fp8_quant_reorder(
    ...
    Uses tl.range with reorder=True to cluster loads across multiple heads,
    improving memory latency hiding via software pipelining after unroll.
